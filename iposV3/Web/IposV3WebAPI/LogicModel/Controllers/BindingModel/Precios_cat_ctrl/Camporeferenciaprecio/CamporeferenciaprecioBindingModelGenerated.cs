...
namespace BindingModels
{
    ...
    public class CamporeferenciaprecioBindingModelGenerated : Validatable, IBaseBindingModel
    
    {
        
        public CamporeferenciaprecioBindingModelGenerated()
        {
        }
        
        #if PROYECTO_WEB
        public CamporeferenciaprecioBindingModelGenerated(Camporeferenciaprecio item)
        {
            FillFromEntity(item);
        }
        ...
        private BoolCN? _Listacliente;
        [XmlAttribute][Required(ErrorMessage = "Es requerido")]
        public BoolCN? Listacliente { get => _Listacliente?? BoolCN.N; set { if (RaiseAcceptPendingChange(value, _Listacliente)) { _Listacliente = value?? BoolCN.N; OnPropertyChanged(); } } }

        private BoolCN? _Listatraslado;
        [XmlAttribute][Required(ErrorMessage = "Es requerido")]
        public BoolCN? Listatraslado { get => _Listatraslado?? BoolCN.N; set { if (RaiseAcceptPendingChange(value, _Listatraslado)) { _Listatraslado = value?? BoolCN.N; OnPropertyChanged(); } } }

        public static string GetBaseQuery()
        {

            return $@"SELECT * FROM ""Camporeferenciaprecio"" Where ""EmpresaId"" = @1 and ""SucursalId"" = @2 ";
        ...
        #if PROYECTO_WEB
        public void FillCatalogRelatedFields(Camporeferenciaprecio item)
        {
        ...
        public void FillEntityValues(ref Camporeferenciaprecio item)
        {
            ...
            item.Listacliente = Listacliente ?? BoolCN.N;


            item.Listatraslado = Listatraslado ?? BoolCN.N;



        }

        public void FillFromEntity(Camporeferenciaprecio item)
        {
            ...
            Listacliente = item.Listacliente;

            Listatraslado = item.Listatraslado;
