...
namespace BindingModels
{
    ...
    public class AlmacenBindingModelGenerated : Validatable, IBaseBindingModel
    
    {
        
        public AlmacenBindingModelGenerated()
        {
        }
        
        #if PROYECTO_WEB
        public AlmacenBindingModelGenerated(Almacen item)
        {
            FillFromEntity(item);
        }
        ...
        private BoolCS? _Esprincipal;
        [XmlAttribute][Required(ErrorMessage = "Es requerido")]
        public BoolCS? Esprincipal { get => _Esprincipal?? BoolCS.S; set { if (RaiseAcceptPendingChange(value, _Esprincipal)) { _Esprincipal = value?? BoolCS.S; OnPropertyChanged(); } } }

        public static string GetBaseQuery()
        {

            return $@"SELECT * FROM ""Almacen"" Where ""EmpresaId"" = @1 and ""SucursalId"" = @2 ";
        ...
        #if PROYECTO_WEB
        public void FillCatalogRelatedFields(Almacen item)
        {
        ...
        public void FillEntityValues(ref Almacen item)
        {
            ...
            item.Esprincipal = Esprincipal ?? BoolCS.S;



        }

        public void FillFromEntity(Almacen item)
        {
