...
namespace BindingModels
{
    ...
    public class Sat_paisBindingModelGenerated : Validatable, IBaseBindingModel
    
    {
        
        public Sat_paisBindingModelGenerated()
        {
        }

        #if PROYECTO_WEB
        public Sat_paisBindingModelGenerated(Sat_pais item)
        {
            FillFromEntity(item);
        }
        ...
        private string? _Sat_formatocp;
        [XmlAttribute]
        public string? Sat_formatocp { get => _Sat_formatocp; set { if (RaiseAcceptPendingChange(value, _Sat_formatocp)) { _Sat_formatocp = value; OnPropertyChanged(); } } }

        private string? _Sat_formatorit;
        [XmlAttribute]
        public string? Sat_formatorit { get => _Sat_formatorit; set { if (RaiseAcceptPendingChange(value, _Sat_formatorit)) { _Sat_formatorit = value; OnPropertyChanged(); } } }

        private string? _Sat_validacionrit;
        [XmlAttribute]
        public string? Sat_validacionrit { get => _Sat_validacionrit; set { if (RaiseAcceptPendingChange(value, _Sat_validacionrit)) { _Sat_validacionrit = value; OnPropertyChanged(); } } }

        private string? _Sat_agrupaciones;
        [XmlAttribute]
        public string? Sat_agrupaciones { get => _Sat_agrupaciones; set { if (RaiseAcceptPendingChange(value, _Sat_agrupaciones)) { _Sat_agrupaciones = value; OnPropertyChanged(); } } }

        public static string GetBaseQuery()
        {

            return $@"SELECT * FROM ""Sat_pais""  ";
        ...
        {
            return @"Clave|Nombre|Sat_clave|Sat_descripcion|Sat_formatocp|Sat_formatorit|Sat_validacionrit|Sat_agrupaciones";
        }



        #if PROYECTO_WEB
        public void FillCatalogRelatedFields(Sat_pais item)
        {
        ...
        public void FillEntityValues(ref Sat_pais item)
        {
            ...
            item.Sat_formatocp = Sat_formatocp ;


            item.Sat_formatorit = Sat_formatorit ;


            item.Sat_validacionrit = Sat_validacionrit ;


            item.Sat_agrupaciones = Sat_agrupaciones ;



        }

        public void FillFromEntity(Sat_pais item)
        {
            ...
            Sat_formatocp = item.Sat_formatocp;

            Sat_formatorit = item.Sat_formatorit;

            Sat_validacionrit = item.Sat_validacionrit;

            Sat_agrupaciones = item.Sat_agrupaciones;
