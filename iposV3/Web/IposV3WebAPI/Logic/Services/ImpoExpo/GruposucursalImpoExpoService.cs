...
namespace IposV3.Services
{
    ...
    public class GruposucursalImpoExpoService : BaseImpoExpoService
    {
        ...
        {
            var exportValores = localContext.Gruposucursal.AsNoTracking()
                                               ...
                                               OleDbDataReader dataReader, List<ImpoExpoField> fields, ApplicationDbContext localContext)
        {
            ...
            var itemSaved = localContext.Gruposucursal
                                        .FirstOrDefault(i => i.EmpresaId == empresaId && i.SucursalId == sucursalId &&
                                                             i.Clave == clave);

            var item = itemSaved != null ? itemSaved : new Gruposucursal();
		...
		item.Activo = dataReader["activo"] != System.DBNull.Value && ((string)dataReader["activo"]).Trim() == "N" ? BoolCS.N : BoolCS.S; 
            ...
            if (existItem)
                localContext.Gruposucursal.Update(item);
            else
                localContext.Gruposucursal.Add(item);
                                               ...
                                               FbDataReader dataReader, ApplicationDbContext localContext)
        {
            ...
            var itemSaved = localContext.Gruposucursal.AsNoTracking()
                                        .FirstOrDefault(i => i.EmpresaId == empresaId && i.SucursalId == sucursalId &&
                                                             i.Clave == clave);
