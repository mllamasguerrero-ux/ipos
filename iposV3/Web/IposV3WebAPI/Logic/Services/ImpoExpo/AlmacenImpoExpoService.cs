...
namespace IposV3.Services
{
    ...
    public class AlmacenImpoExpoService : BaseImpoExpoService
    {
                                               ...
                                               FbDataReader dataReader, ApplicationDbContext localContext)
        {
            ...
            var itemSaved = localContext.Almacen.AsNoTracking()
                                        .FirstOrDefault(i => i.EmpresaId == empresaId && i.SucursalId == sucursalId &&
                                                             i.Clave == clave);

            var item = itemSaved != null ? itemSaved : new Almacen();
            ...
            if (existItem)
                localContext.Almacen.Update(item);
            else
                localContext.Almacen.Add(item);
