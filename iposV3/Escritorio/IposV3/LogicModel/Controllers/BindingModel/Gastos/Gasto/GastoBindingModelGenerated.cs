...
namespace BindingModels
{
    ...
    public class GastoBindingModelGenerated : Validatable, IBaseBindingModel
    
    {
        
        public GastoBindingModelGenerated()
        {
        }
        
        #if PROYECTO_WEB
        public GastoBindingModelGenerated(Gasto item)
        {
            FillFromEntity(item);
        }
        ...
        private string? _CuentaClave;
        [XmlAttribute]
        public string? CuentaClave { get => _CuentaClave; set { if (RaiseAcceptPendingChange(value, _CuentaClave)) { _CuentaClave = value; OnPropertyChanged(); } } }

        private string? _CuentaNombre;
        [XmlAttribute]
        public string? CuentaNombre { get => _CuentaNombre; set { if (RaiseAcceptPendingChange(value, _CuentaNombre)) { _CuentaNombre = value; OnPropertyChanged(); } } }

        public static string GetBaseQuery()
        {

            return $@"SELECT * FROM ""Gasto"" Where ""EmpresaId"" = @1 and ""SucursalId"" = @2 ";
        ...
        {
            return @"Clave|Nombre|Cuenta.Clave|Cuenta.Nombre";
        }


        #if PROYECTO_WEB
        public void FillCatalogRelatedFields(Gasto item)
        {

             this._CuentaClave = item.Cuenta?.Clave;

             this._CuentaNombre = item.Cuenta?.Nombre;


        }


        public void FillEntityValues(ref Gasto item)
        {
        ...
        public void FillFromEntity(Gasto item)
        {
