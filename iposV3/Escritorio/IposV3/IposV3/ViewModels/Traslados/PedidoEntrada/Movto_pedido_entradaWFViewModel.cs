...
namespace IposV3.ViewModels
{
    ...
    public class Movto_pedido_entradaWFViewModel : BaseOperationalScreen
    {
        ...
        public ObservableCollection<Movto_pedido_entradaWFBindingModel> MovtoItems { get; set; }
        ...
        private readonly PuntoPedidoEntradaWebController puntoPedidoEntradaControllerProvider;
        protected readonly ImpoExpoWebController impoExpoProvider;
        //protected readonly IEventAggregator aggregator;


        //public Movto_pedido_entradaWFBindingModel? Movto_pedido_entradaItem { get; set; }
        ...
        public Movto_pedido_entradaWFViewModel(
            ...
            PuntoPedidoEntradaWebController puntoPedidoEntradaControllerProvider,
            ...
            base(selectorProvider, aggregator, winManager, messageBoxService, usuarioControllerProvider, corteControllerProvider)
        {
            ...
            this.puntoPedidoEntradaControllerProvider = puntoPedidoEntradaControllerProvider;
            ...
            MovtoItems = new ObservableCollection<Movto_pedido_entradaWFBindingModel>();
            ...
            //Movto_pedido_entradaItem = new Movto_pedido_entradaWFBindingModel();
            //Movto_pedido_entradaItem!.PendingChange += CatalogField_PropertyChanging;
        ...
        public void NuevaPedidoEntrada()
        {

            ClearInfo();
            clearingOrLoadingDoctoInfo = true;
            ...
            clearingOrLoadingDoctoInfo = false;
            //AssignProveedorInfo();
        ...
        public void LoadPedidoEntrada(long doctoId)
        {
            ClearInfo();
            ClearMovtoItems();
        ...
        //loading movto items
        public async Task<List<Movto_pedido_entradaWFBindingModel>> DoGetMovtoItems(long doctoId)
        {
            List<Movto_pedido_entradaWFBindingModel> results = new List<Movto_pedido_entradaWFBindingModel>();

            results = await puntoPedidoEntradaControllerProvider.Select_movto_pedido_entrada_List(new Movto_ped_entr_lstParamWFBindingModel(
        ...
        public void ReloadMovtoItems(long doctoId)
        {
            ...
            try
            {
                ...
                var comentario = "";
                System.Collections.Generic.List<Movto_pedido_entradaWFBindingModel> items = new System.Collections.Generic.List<Movto_pedido_entradaWFBindingModel>();

                
                Task.Run<List<Movto_pedido_entradaWFBindingModel>>(async () =>
        ...
        public void CompleteSucursalTInfoInDocto()
        {
        ...
        public void SaveMovtoRecord(MovtoPedidoEntradaTrans movto)
        {
            ...
            Task.Run(async () =>
            {
                Fnc_movto_prov_insertResult = await this.puntoPedidoEntradaControllerProvider.Movto_pedido_entrada_insert(movto);
            }).Wait();
        ...
        public void SaveMovto()
        {
            ...
            var Fnc_movto_prov_insertParam = new MovtoPedidoEntradaTrans();
        ...
        public bool SaveMovto_(MovtoPedidoEntradaTrans movto)
        {
        ...
        public bool SaveDoctoRecord()
        {
            ...
            var Fnc_docto_prov_insertParam = new DoctoPedidoEntradaTrans();
            ...
            Task.Run(async () =>
            {
                ...
                Fnc_docto_prov_insertResult = await puntoPedidoEntradaControllerProvider.Docto_pedido_entrada_insert(Fnc_docto_prov_insertParam);
        ...
        public void Docto_update_datos_ajustables()
        {
            ...
            Task.Run<BaseResultBindingModel?>(async () =>
                                                  await puntoPedidoEntradaControllerProvider.Docto_update_datos_ajustables(
                                                           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid!.Value,
                                                           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid!.Value,
                                                           CurrentDocto!.Id!.Value,
                                                           GlobalVariable.CurrentSession!.CurrentUsuario!.Id,
                                                           CurrentDocto!.Almacenid,
                                                           CurrentDocto!.Referencias
            ...
            //        result = puntoPedidoEntradaControllerProvider.Docto_update_datos_ajustables(
            //           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid!.Value,
            //           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid!.Value,
            //           CurrentDocto!.Id!.Value,
            //           GlobalVariable.CurrentSession!.CurrentUsuario!.Id,
        ...
        public void SurtirPedido()
        {
            ...
            BaseResultBindingModel? result = null;
            ...
            Task.Run(async () =>
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            result = await puntoPedidoEntradaControllerProvider.Docto_surtir_pedido(
                                                   GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid!.Value,
                                                   GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid!.Value,
                                                   CurrentDocto!.Id!.Value,
                                                   GlobalVariable.CurrentSession!.CurrentUsuario!.Id,
                                            ...
                                            if (bSuccess)
                                                doctoSurtidoId = result!.Result;

                                            if (bSuccess && doctoSurtidoId != null && CurrentDocto!.Docto_traslado_Sucursaltid != GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid)
                                            {
        ...
        public void AjustarExistencias()
        {
            ...
            Task.Run<BaseResultBindingModel?>(async () =>
                                                  await puntoPedidoEntradaControllerProvider.Movto_pedido_entrada_ajustarExistencias(
                                                           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid!.Value,
                                                           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid!.Value,
                                                           CurrentDocto!.Id!.Value,
            ...
            //        result = puntoPedidoEntradaControllerProvider.Movto_pedido_entrada_ajustarExistencias(
            //           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid!.Value,
            //           GlobalVariable.CurrentSession!.CurrentConfiguracion!.Sucursalid!.Value,
            //           CurrentDocto!.Id!.Value,
            //           GlobalVariable.CurrentSession!.CurrentUsuario!.Id,
        ...
        {
            Movto_pedido_entradaWFBindingModel movtoItem = (Movto_pedido_entradaWFBindingModel)sender;
            if (e.PropertyName == "Cantidadsurtida")
            {
                ...
                e.CancelPendingChange = SaveMovtoFromGrid(movtoItem, "Cantidadsurtida", e.NewValue);
            }
        }
        ...
        {
            if (e.NewItems != null)
                foreach (Movto_pedido_entradaWFBindingModel item in e.NewItems)
                    item.PendingChange += MovtoItem_PendingChange;

            if (e.OldItems != null)
                foreach (Movto_pedido_entradaWFBindingModel item in e.OldItems)
                    item.PendingChange -= MovtoItem_PendingChange;
        }

        public bool SaveMovtoFromGrid(Movto_pedido_entradaWFBindingModel movtoItem, string propertyChanged, object newVal)
        {
            if (propertyChanged != "Cantidadsurtida")
                return false;
            ...
            Fnc_movto_prov_insertParam.Empresaid = (movtoItem.EmpresaId ?? GlobalVariable.CurrentSession!.Empresaid) ?? 0;
            Fnc_movto_prov_insertParam.Sucursalid = (movtoItem.SucursalId ?? GlobalVariable.CurrentSession!.Sucursalid) ?? 0;

            Fnc_movto_prov_insertParam.Id = movtoItem.Id;
            Fnc_movto_prov_insertParam.Productoid = movtoItem.Productoid!.Value;
            Fnc_movto_prov_insertParam.Cantidad = movtoItem.Cantidad ?? 0m;
            Fnc_movto_prov_insertParam.Cantidadsurtida = (decimal?)newVal ?? 0m;
            ...
            Task.Run(async () =>
            {
                ...
                Fnc_movto_prov_insertResult = await this.puntoPedidoEntradaControllerProvider.Movto_pedido_entrada_ponercantidadsurtida(Fnc_movto_prov_insertParam);
            ...
            if (Fnc_movto_prov_insertResult.Result == null || Fnc_movto_prov_insertResult.Result.Value <= 0)
            {
                ...
                throw new Exception("Error al cambiar el movimiento " +
                                        (Fnc_movto_prov_insertResult.Result == null ? "" : Fnc_movto_prov_insertResult.Usermessage));
            }
        ...
        public void MovtoDeleteSelected()
        {
            ...
            if (ShowSiNoMessageBox("Realmente desea eliminar el(los) registro(s)", "Confirmacion"))
            {
                try
                {
                    var listIds = SelectedMovtoItems.Select(item => ((Movto_pedido_entradaWFBindingModel)item).Id!.Value).ToList();
