...
namespace IposV3.ViewModels.BaseScreen
{
    public class BaseScreen : Screen
    {
        ...
        protected SelectorWebController selectorProvider;
        ...
        public bool CatalogValueChanged(string? strCatalogo, string? strClave, out string strNombre, out long? lId, out string strRealClave)
        {
            ...
            //string tempStrNombre = "", tempStrRealClave = "";
            //long? tempLId = null;

            var bRetorno = false;

            BaseSelector? catalogItem = null;
            Task.Run(async () =>
            {

                catalogItem = await selectorProvider.obtainItemByClave(strCatalogo, strClave, new BaseParam(GlobalVariable.CurrentSession?.Empresaid, GlobalVariable.CurrentSession?.Sucursalid));
        ...
        public bool CatalogValueChangedById(string? strCatalogo, long? lId, out string strNombre, out string strClave, out long? realId)
        {
            ...
            //string tempStrNombre = "", tempStrClave = "";
            //long? tempRealId = null;
            ...
            Task.Run(async () =>
            {
                ...
                catalogItem = await selectorProvider.obtainItemById(strCatalogo, lId.Value, new BaseParam(GlobalVariable.CurrentSession?.Empresaid, GlobalVariable.CurrentSession?.Sucursalid));
                    ...
                    case "byId":
                        {
                            ...
                            if (!e.CancelPendingChange)
                            {
                                AfterCatalogFieldChanging(catalogRelated, lIdValue, strClave ?? "", strNombre ?? "");
                            }
                        }
                        break;
        ...
        public virtual void ShowPopUpSelector(string catalogClaveField)
        {
        ...
        public bool ShowPopUpSelectorCatalogRelated(CatalogRelatedFields catalogRelated, string? textSearch = null, Dictionary<string,object?>? specialProperties = null)
        {
            ...
            try
            {
                ...
                if(specialProperties != null)
                {
                    ...
                    {
                        if (type!.GetProperty(prop.Key) != null)
                                type!.GetProperty(prop.Key)?.SetValue(rootModel, prop.Value);
                    }
                }
            }
            catch { }

            if (catalogRelated.ListVMName == "IposV3.ViewModels.Catalogo_genericoListViewModel")
            {
            ...
            winManager.ShowDialogAsync(rootModel, null, CreateWinSettings(selectorProvider.obtainCatalogTitle(catalogRelated) , 0.65, 0.65));
        ...
        protected dynamic CreateWinSettingsWithPixels(string strTitle, double widthPixels, double heightPixels)
        {
            var settings = CreateWinSettings(strTitle, 0.99, 0.99);

            settings.Width = widthPixels;
            settings.Height = heightPixels;
