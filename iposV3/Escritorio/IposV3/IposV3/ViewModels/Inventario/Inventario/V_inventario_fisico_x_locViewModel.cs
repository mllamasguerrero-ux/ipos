...
namespace IposV3.ViewModels
{
    ...
    public class V_inventario_fisico_x_locViewModel : BaseOperationalScreen, IHandle<V_inventario_listItemVMInitialParameters>
    {
        ...
        public V_inventario_fisico_x_locViewModel(
        ...
        protected override void fillCatalogConfiguration()
        {
            ...
            lstCatalogDef = new List<CatalogDef>() { 
                     ...
                     new CatalogDef("LoteInventario"),
                     new CatalogDef("AnaquelLibre", null, null,new KendoParams("","anaquel", new KendoFilters(new List<KendoFilter>() { new KendoFilter("0", "eq", "Almacenid", "S") }, "and"))),
                     new CatalogDef("ProductolocationsLibre", null, null,new KendoParams("","localidad", new KendoFilters(new List<KendoFilter>() { new KendoFilter("0", "eq", "Anaquelid", "S") }, "and")))
            };
        }
        ...
        public void ReloadMovtoItems()
        {
            ...
            try
            {
                ...
                Task.Run(async () =>
                {
                    items = await this.inventarioController.V_inventario_movto_detalles_x_locList(ParamList!) ?? items;
                }).Wait();
            ...
            switch (e.PropertyName)
            {
                ...
                case "Almacenid":
                    ...
                    if (!e.CancelPendingChange && e.NewValue != e.OldValue)
                    {
                        ...
                        string almacenIdStr = "0";
                        if (e.NewValue != null && e.NewValue != System.DBNull.Value && (long?)e.NewValue != -2)
                        {
                            almacenIdStr = ((long?)e.NewValue).Value.ToString();
                        }



                        string almacenIdOldStr = "0";
                        if (e.OldValue != null && e.OldValue != System.DBNull.Value && (long?)e.OldValue != -2)
                        {
                            almacenIdOldStr = ((long?)e.OldValue).Value.ToString();
                        }
                        
                        if(almacenIdOldStr!= almacenIdStr)
                        {

                            CatalogDef defAnaquelLibre = new CatalogDef("AnaquelLibre");
                            defAnaquelLibre.ExtraCondition = new KendoParams("", "anaquel", new KendoFilters(new List<KendoFilter>() { new KendoFilter(almacenIdStr, "eq", "Almacenid", "S") }, "and"));


                            Task.Run(async () =>
                            {
                                this.Catalogs!["AnaquelLibre"] = await selectorProvider.obtainCatalog(defAnaquelLibre, new BaseParam(GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid, GlobalVariable.CurrentSession.CurrentConfiguracion.Sucursalid));
                ...
                case "Anaquelid":
                    ...
                    if (!e.CancelPendingChange && e.NewValue != e.OldValue)
                    {
                        ...
                        Task.Run(async () =>
                        {
                            Catalogs!["ProductolocationsLibre"] = await selectorProvider.obtainCatalog(defProductLocations, new BaseParam(GlobalVariable.CurrentSession!.CurrentConfiguracion!.Empresaid, GlobalVariable.CurrentSession.CurrentConfiguracion.Sucursalid));
        ...
        private void InsertDocto()
        {
            ...
            tempParam.Tipodoctoid = DBValues._DOCTO_TIPO_INVENTARIO_FISICOXLOC;
        ...
        public void Accept()
        {
            try
            {
                ...
                if (Inventario_movto_insert_param.Anaquelid == null)
                {

                    IsBusy = false;
                    showPopUpMessage("Ocurrio un error ", "No se ha definido el anaquel", "Error");
                    return;
                }

                if (Inventario_movto_insert_param.Localidad == null)
                {

                    IsBusy = false;
                    showPopUpMessage("Ocurrio un error ", "No se ha definido la localidad", "Error");
                    return;
                }
                ...
                if (CurrentProducto.Producto_inventario_Manejalote != null && CurrentProducto.Producto_inventario_Manejalote == BoolCN.S)
                {
                    ...
                    if (LoteNuevo == 0)
                    {
                        ...
                        DateTime fechaVenceBuffer = DateTime.ParseExact(strSplitBuffer[1], "d/M/yyyy", null);
        ...
        public void AnaquelPrev()
        {
            if (Inventario_movto_insert_param == null || 
                    ( Catalogs == null ||  Catalogs["AnaquelLibre"] == null || Catalogs["AnaquelLibre"]!.Count() == 0)
                )
                return;

            var primerAnaquelId = Catalogs["AnaquelLibre"]!.First().Id;

            if (Inventario_movto_insert_param.Anaquelid == null)
                Inventario_movto_insert_param.Anaquelid = primerAnaquelId;
            else 
            {
                var indexCurrentValue = Catalogs["AnaquelLibre"]!.FindIndex(p => p.Id == Inventario_movto_insert_param.Anaquelid);
                if (indexCurrentValue > 0)
                    Inventario_movto_insert_param.Anaquelid = Catalogs["AnaquelLibre"]![indexCurrentValue - 1].Id;
            }
            

        }

        public void AnaquelNext()
        {
            if (Inventario_movto_insert_param == null || 
                    ( Catalogs == null ||  Catalogs["AnaquelLibre"] == null || Catalogs["AnaquelLibre"]!.Count() == 0)
                )
            ...
            {
                var indexCurrentValue = Catalogs["AnaquelLibre"]!.FindIndex(p => p.Id == Inventario_movto_insert_param.Anaquelid);
                if (indexCurrentValue < Catalogs["AnaquelLibre"]!.Count() - 1)
                    Inventario_movto_insert_param.Anaquelid = Catalogs["AnaquelLibre"]![indexCurrentValue + 1].Id;
            }
        ...
        public void LocalidadPrev()
        {

            if (Inventario_movto_insert_param == null ||
                    (Catalogs == null || Catalogs["ProductolocationsLibre"] == null || Catalogs["ProductolocationsLibre"]!.Count() == 0)
                )
                return;

            var primerLocalidad = Catalogs["ProductolocationsLibre"]!.First().Clave;

            if (Inventario_movto_insert_param.Localidad == null)
                Inventario_movto_insert_param.Localidad = primerLocalidad;
            else
            {
                var indexCurrentValue = Catalogs["ProductolocationsLibre"]!.FindIndex(p => p.Clave == Inventario_movto_insert_param.Localidad);
                if (indexCurrentValue > 0)
                    Inventario_movto_insert_param.Localidad = Catalogs["ProductolocationsLibre"]![indexCurrentValue - 1].Clave;
            }

        }

        public void LocalidadNext()
        {
            ...
            {
                var indexCurrentValue = Catalogs["ProductolocationsLibre"]!.FindIndex(p => p.Clave == Inventario_movto_insert_param.Localidad);
                if (indexCurrentValue < Catalogs["ProductolocationsLibre"]!.Count() - 1)
                    Inventario_movto_insert_param.Localidad = Catalogs["ProductolocationsLibre"]![indexCurrentValue + 1].Clave;
            }
        }
