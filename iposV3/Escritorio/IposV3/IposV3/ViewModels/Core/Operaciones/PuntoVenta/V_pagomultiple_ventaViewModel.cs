...
namespace IposV3.ViewModels
{
    ...
    public class V_pagomultiple_ventaViewModel : BaseGenericViewModel
    {


        public bool BIsReadOnly { get; protected set; }

        private V_pagomultiple_ventaWFBindingModel? pagoItem;
        public V_pagomultiple_ventaWFBindingModel? PagoItem
        {
            get => pagoItem;
            set
            {
        ...
        public V_pagomultiple_ventaViewModel(SelectorWebController selectorProvider, IEventAggregator aggregator, IWindowManager winManager, IMessageBoxService messageBoxService) :
            base(selectorProvider, aggregator, winManager, messageBoxService)
        {
            PagoItem = new V_pagomultiple_ventaWFBindingModel();
            PagoItem!.PendingChange += CatalogField_PropertyChanging;
        ...
        public void Accept()
        {
            ...
            bool bGenerarcartaporte = (PagoItem?.Generarcartaporte?.Equals("S") ?? false);
            bool bGenerarcomprobantetraslado = (PagoItem?.Generarcomprobantetraslado?.Equals("S") ?? false);

            if (bGenerarcartaporte && !bGenerarcomprobantetraslado)
            {
            ...
            if (bGenerarcartaporte)
            {
                ...
                WFGuiaPopUpWFViewModel vm = IoC.Get<WFGuiaPopUpWFViewModel>();
                winManager.ShowDialogAsync(vm, null, CreateWinSettings("Guia", 0.80, 0.90));

                if (!vm.BProcessSuccess || vm.GuiaItem == null)
                {
                    showPopUpMessage(new MessageToUserSimple("Error", "Si requiere carta porte necesita ingresar la informacion de guia/vehiculo/fechas", "Error"));
                    return;

                }

                PagoItem!.Guia = vm.GuiaItem;
                //vm.Dispose();
