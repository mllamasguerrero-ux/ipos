...
namespace IposV3.Services.FacturaElectronica
{
    public class VirtualXmlHelper
    {
        ...
        public VirtualXmlHelper(string versionFacturacion, DispatcherDelegate? dispatcherMethodCall, VirtualXmlHelperModel? model) :this(dispatcherMethodCall)
        {
            version = versionFacturacion;

            if(model != null)
            {
                VirtualPacInfo = model.VirtualPacInfo;
                CiberSatInfo = model.CiberSatInfo;
                ICARTAPORTE = model.ICARTAPORTE;
                ComprobanteInfo = model.ComprobanteInfo;
                ComprobanteInfoEx = model.ComprobanteInfoEx;
                ComprobanteCfdInfo = model.ComprobanteCfdInfo;
                EmisorInfo = model.EmisorInfo;
                ReceptorInfo = model.ReceptorInfo;
                InformacionGlobal = model.InformacionGlobal;
                Conceptos = model.Conceptos;
                PagosSat = model.PagosSat;
                ImpuestosTrasladados = model.ImpuestosTrasladados;
                ImpuestosRetenidos = model.ImpuestosRetenidos;
                ImpuestosInfo = model.ImpuestosInfo;
                ImpuestosLocalesInfo = model.ImpuestosLocalesInfo;
                LocalesTrasladados = model.LocalesTrasladados;
                LocalesRetenidos = model.LocalesRetenidos;
                Addendas = model.Addendas;
                EmisorExpedidoEn = model.EmisorExpedidoEn;
                RutaCsd = model.RutaCsd;
                Llave = model.Llave;
                ClaveLlave = model.ClaveLlave;
                RutaXml = model.RutaXml;
                RutaPdf = model.RutaPdf;
                Version = model.Version;
                ManejaAddenda = model.ManejaAddenda;
                ManejaImpuestosLocales = model.ManejaImpuestosLocales;
                ManejaTrasladados = model.ManejaTrasladados;
                ManejaRetenidos = model.ManejaRetenidos;
                CfdiRelacionados = model.CfdiRelacionados;
            }
        }
                        ...
                        if (c.DoctosImpRelacionados != null)
                        {
                                                                                   ...
                                                                                   (y.Tipoimpuesto == "CONCEPTO_IVA" || y.Tipoimpuesto == "CONCEPTO_IEPS")))
                            {
                                ...
                                VirtualXML_Pagos20AddPagoDoctoRelacionadoTrasladoDR(hXmlPtr,
                                                ...
                                                impuesto.Impuesto,
                                                impuesto.Tipofactor,
                                                ((impuesto.Tasa ?? 0m) / 100.00m).ToString("N6", nfi),
