...
namespace IposV3.Services
{
    ...
    public class ContenidoImpoExpoService : BaseImpoExpoService
    {
        ...
        {
            var exportValores = localContext.Contenido.AsNoTracking()
        ...
        public override List<ImpoExpoField> ObtainImpoExpoFields()
        {
            return new List<ImpoExpoField>() {
                ...
                new ImpoExpoField("Clave","clave",1,0, NpgsqlDbType.Char, typeof(string)),
                                               ...
                                               OleDbDataReader dataReader, List<ImpoExpoField> fields, ApplicationDbContext localContext)
        {
            ...
            var itemSaved = localContext.Contenido
                                        .FirstOrDefault(i => i.EmpresaId == empresaId && i.SucursalId == sucursalId &&
                                                             i.Clave == clave);

            var item = itemSaved != null ? itemSaved : new Contenido();
		...
		item.Activo = dataReader["activo"] != System.DBNull.Value && ((string)dataReader["activo"]).Trim() == "N" ? BoolCS.N : BoolCS.S; 
            ...
            if (existItem)
                localContext.Contenido.Update(item);
            else
                localContext.Contenido.Add(item);
                                               ...
                                               FbDataReader dataReader, ApplicationDbContext localContext)
        {
            ...
            var itemSaved = localContext.Contenido.AsNoTracking()
                                        .FirstOrDefault(i => i.EmpresaId == empresaId && i.SucursalId == sucursalId &&
                                                             i.Clave == clave);
