...
namespace IposV3.Services
{
    public class FacturaElectronicaService
    {
        ...
        public bool Factura_Electronica_Generar(
                    long empresaId, long sucursalId, long doctoId,
                    string documentoTimbrado,bool silentMode,bool overwritte, string? tipocomprobanteespecial, bool generarCartaPorte,
                    ApplicationDbContext localContext, DispatcherDelegate? dispatcherMethodCall, out string message)
        {
            ...
            if (File.Exists(documentoTimbrado))
            {
                ...
                {
                    message = "Archivo timbrado existente en la carpeta";
                }
            }

            VirtualXmlHelper virtualXmlHelper = new VirtualXmlHelper(dispatcherMethodCall);
            Factura_Electronica_FillAll(empresaId,  sucursalId,  doctoId,
                     documentoTimbrado, silentMode,  tipocomprobanteespecial, generarCartaPorte,
                    localContext, dispatcherMethodCall, out message, out virtualXmlHelper);


            if(virtualXmlHelper.ComprobanteInfo?.TipoComprobante == "P")
                virtualXmlHelper.GenerarPago();
            else
                virtualXmlHelper.GenerarDocumento();
        ...
        public bool Factura_Electronica_FillAll(long empresaId, long sucursalId, long doctoId,
                    ...
                    ApplicationDbContext localContext, DispatcherDelegate? dispatcherMethodCall, out string message,  out VirtualXmlHelper virtualXmlHelper)
        {



            virtualXmlHelper = new VirtualXmlHelper(dispatcherMethodCall);
            message = "";
        ...
        public void FillComprobanteInfo(
                    ...
                    ref VirtualXmlHelper virtualXmlHelper)
        {
        ...
        public bool Facturacion_GuardarDatosTimbrado(long empresaId, long sucursalId, long doctoId,
                                                      string documentoTimbrado, string? tipoComprobanteEspecial, ApplicationDbContext localContext, out string message)
        {
            ...
            if (tipoComprobanteEspecial == null || tipoComprobanteEspecial != "T")
            {
                if (doctoAFacturar.Docto_fact_elect != null && elmTimbreFiscalNode != null && comprobanteNode != null)
                {

                    doctoAFacturar.Docto_fact_elect.Timbradouuid = elmTimbreFiscalNode.GetAttribute("UUID");
                    doctoAFacturar.Docto_fact_elect.Timbradofecha = elmTimbreFiscalNode.GetAttribute("FechaTimbrado");
                    doctoAFacturar.Docto_fact_elect.Timbradocertsat = elmTimbreFiscalNode.GetAttribute("NoCertificadoSAT");
                    doctoAFacturar.Docto_fact_elect.Timbradosellosat = elmTimbreFiscalNode.GetAttribute("SelloSAT");
                    doctoAFacturar.Docto_fact_elect.Timbradosellocfdi = elmTimbreFiscalNode.GetAttribute("SelloCFD");
                    doctoAFacturar.Docto_fact_elect.Timbradoformapagosat = comprobanteNode.GetAttribute("FormaPago");
                ...
                else
                {
                    doctoAFacturar.Docto_fact_elect = new Docto_fact_elect()
                    {
                        ...
                        SucursalId = sucursalId,
                        Doctoid = doctoId,
                        Timbradouuid = elmTimbreFiscalNode!.GetAttribute("UUID"),
                        Timbradofecha = elmTimbreFiscalNode.GetAttribute("FechaTimbrado"),
                        Timbradocertsat = elmTimbreFiscalNode.GetAttribute("NoCertificadoSAT"),
                        Timbradosellosat = elmTimbreFiscalNode.GetAttribute("SelloSAT"),
                        Timbradosellocfdi = elmTimbreFiscalNode.GetAttribute("SelloCFD"),
                        Timbradoformapagosat = comprobanteNode!.GetAttribute("FormaPago"),
                ...
                DateTimeOffset fechaFactura = DateTime.Parse(elmTimbreFiscalNode.GetAttribute("FechaTimbrado"));
            ...
            {
                var doctoComprobante = doctoAFacturar?.Docto_comprobantes?.FirstOrDefault(y => y.Tipocomprobante == tipoComprobanteEspecial);
                if (doctoComprobante != null && elmTimbreFiscalNode != null)
                {

                    doctoComprobante.Timbradouuid = elmTimbreFiscalNode.GetAttribute("UUID");
                    doctoComprobante.Timbradofecha = elmTimbreFiscalNode.GetAttribute("FechaTimbrado");
                    doctoComprobante.Timbradocertsat = elmTimbreFiscalNode.GetAttribute("NoCertificadoSAT");
                    doctoComprobante.Timbradosellosat = elmTimbreFiscalNode.GetAttribute("SelloSAT");
                    doctoComprobante.Timbradosellocfdi = elmTimbreFiscalNode.GetAttribute("SelloCFD");

                    localContext.Doctocomprobante.Update(doctoComprobante);
                }
                else if ( elmTimbreFiscalNode != null)
                {
                    var doctoComprobanteElectronico = new Doctocomprobante()
                    {
                        EmpresaId = empresaId,
        ...
        public bool Factura_Electronica_GuardarCfdiInfo(
                    long empresaId, long sucursalId, long doctoId,
                    bool silentMode, string? tipocomprobanteespecial, bool generarCartaPorte,
                    ApplicationDbContext localContext, DispatcherDelegate? dispatcherMethodCall, out string message)
        {
            ...
            VirtualXmlHelper virtualXmlHelper = new VirtualXmlHelper(dispatcherMethodCall);
            Factura_Electronica_FillAll(empresaId, sucursalId, doctoId,
                    "", silentMode, tipocomprobanteespecial, generarCartaPorte,
                    localContext, dispatcherMethodCall, out message, out virtualXmlHelper);
            ...
            if (tipocomprobanteespecial != "T" && virtualXmlHelper.Conceptos != null)
            {
                int consecutivoConcepto = 0;
                foreach (var concepto in virtualXmlHelper.Conceptos)
                {
                    Cfdi_conc cfdi_conc = new Cfdi_conc();
                    cfdi_conc.EmpresaId = empresaId;
                    cfdi_conc.SucursalId = sucursalId;
                    ...
                    if (concepto.ConceptoPartes != null)
                    {
                        foreach (var parte in concepto.ConceptoPartes)
                        {
                            ...
                            cfdi_conc_part.EmpresaId = empresaId;
                            cfdi_conc_part.SucursalId = sucursalId;
                    ...
                    if (concepto.ImpuestosRetenidos != null)
                    {
                        foreach (var impuesto in concepto.ImpuestosRetenidos)
                        {
                            ...
                            conc_imp.EmpresaId = empresaId;
                            conc_imp.SucursalId = sucursalId;
            ...
            if (virtualXmlHelper.PagosSat != null)
            {
                foreach (var pago in virtualXmlHelper.PagosSat)
                {
                    ...
                    var pagoSat = localContext.Pagosat.FirstOrDefault(p => p.EmpresaId == empresaId && p.SucursalId == sucursalId &&
                                                             p.Id == pago.PagoSatId);
            ...
            if (virtualXmlHelper.CfdiRelacionados != null && tipocomprobanteespecial != "T")
            {
                foreach (var relacionadoList in virtualXmlHelper.CfdiRelacionados)
                {
                    foreach (var relacionado in relacionadoList.Value)
                    {
                        ...
                        cfdi_rel.EmpresaId = empresaId;
                        cfdi_rel.SucursalId = sucursalId;
            ...
            if (virtualXmlHelper.ImpuestosTrasladados != null && tipocomprobanteespecial != "T")
            {
                foreach (var impuesto in virtualXmlHelper.ImpuestosTrasladados)
                {
                    Cfdi_imp cfdi_imp = new Cfdi_imp();
                    cfdi_imp.EmpresaId = empresaId;
                    cfdi_imp.SucursalId = sucursalId;
            ...
            if (virtualXmlHelper.ImpuestosRetenidos != null && tipocomprobanteespecial != "T")
            {
            ...
            if (decimal.TryParse(virtualXmlHelper.ImpuestosInfo?.TotalTraslados, out bufferCantidad) && tipocomprobanteespecial != "T")
                cfdi.Totaltraslados = bufferCantidad;


            if (decimal.TryParse(virtualXmlHelper.ImpuestosInfo?.TotalRetenciones, out bufferCantidad) && tipocomprobanteespecial != "T")
        ...
        {
            VirtualXmlHelper.ShowError(hXml);
        }
