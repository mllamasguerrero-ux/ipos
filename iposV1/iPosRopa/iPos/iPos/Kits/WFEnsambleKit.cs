...
namespace iPos
{
    public partial class WFEnsambleKit : IposForm
    {
        CDOCTOBE m_Docto;
        CPRODUCTOBE m_prod;
        ...
        public WFEnsambleKit()
        {
            InitializeComponent();
            m_prod = new CPRODUCTOBE();
            m_Docto = new CDOCTOBE();
            m_Docto.ITIPODOCTOID = DBValues._DOCTO_TIPO_ENSAMBLE;
            LlenarGrid();
        ...
        private void LlenarGrid()
        {
            try
            {
                ...
                if (P_IDDOCTO == 0)
                {
                    this.dSKits.KitDetalle.Clear();
                    return;
                }

                this.kitDetalleTableAdapter.Fill(this.dSKits.KitDetalle, P_IDDOCTO);
            }
        ...
        private void ClearDocto()
        {
            ...
            this.ALMACENFuenteComboBox.Enabled = this.m_manejaAlmacen;
        }
        ...
        private void Process()
        {
            ...
            if (m_manejaAlmacen && (this.ALMACENDestinoComboBox.SelectedDataValue == null && this.ALMACENFuenteComboBox.SelectedDataValue == null && int.Parse(this.ALMACENDestinoComboBox.SelectedValue.ToString()) <= 0 || int.Parse(this.ALMACENFuenteComboBox.SelectedValue.ToString()) <= 0))
            {
            ...
            long? SUCURSALTID = 0;
            long? ALMACENTID =  m_manejaAlmacen ? int.Parse(this.ALMACENFuenteComboBox.SelectedValue.ToString()) : (int)DBValues._DOCTO_ALMACEN_DEFAULT; 
            ...
            if (m_prod.IESKIT == null || !m_prod.IESKIT.Equals("S"))
            {
                MessageBox.Show("No es un kit valido");
                return;
            }
            ...
            try
            {
                ...
                if (EjecutarAddMOVTO(ref P_IDDOCTO, P_CANTIDAD, P_IDPRODUCTO, P_VD_VENDEDOR, P_USERID, P_LOTE, US_SUPERVISORID, ALMACENID, SUCURSALID, P_FECHAVENCE, m_Docto.ITIPODOCTOID, SUCURSALTID, ALMACENTID, "N", P_TIPODIFERENCIAINVENTARIOID, P_CANTIDAD_SURTIDA,  null, fTrans))
                {
                    ...
                    this.ALMACENFuenteComboBox.Enabled = false;
        ...
        private void ObtenerDoctoDeBD(long lDoctoID)
        {
            ...
            if (!(bool)m_Docto.isnull["IALMACENTID"])
                this.ALMACENFuenteComboBox.SelectedDataValue = m_Docto.IALMACENTID;
            else
                this.ALMACENFuenteComboBox.SelectedIndex = -1;
        ...
        {
            if (kitDetalleDataGridView.Columns["DGCANTIDADFISICA"].Index != e.ColumnIndex)
                return;
            ...
            try
            {
                ...
                decimal dViejaCantidad = decimal.Parse(kitDetalleDataGridView.Rows[e.RowIndex].Cells["DGCANTIDADFISICA"].Value.ToString());
                ...
                TBCodigo.Text = kitDetalleDataGridView.Rows[e.RowIndex].Cells["DGPRODUCTOCLAVE"].Value.ToString();
                LlenarDatos();
        ...
        {
            WFListaEnsamblados look = new WFListaEnsamblados();
            look.ShowDialog();
        ...
        private void kitDetalleDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1 )
            {
                ...
                if (kitDetalleDataGridView.Columns[e.ColumnIndex].Name == "BTELIMINAR")
                {
                    CPuntoDeVentaD pvd = new CPuntoDeVentaD();
                    CMOVTOBE movtoBE = new CMOVTOBE();

                    movtoBE.IID = (long)this.kitDetalleDataGridView.CurrentRow.Cells["MOVTOID"].Value;
        ...
        private bool KIT_APLICAR(long lDoctoID, FbTransaction st)
        {
            ...
            try
            {
                ...
                string commandText = @"KIT_APLICAR";
        ...
        private bool KIT_HAYEXISTENCIAPARAENSAMBLAR(long lDoctoID, FbTransaction st)
        {
            ...
            try
            {
                ...
                string commandText = @"KIT_HAYEXISTENCIAPARAENSAMBLAR";
        ...
        private bool ProcessAplicar()
        {
            ...
            try
            {
                ...
                if(!KIT_HAYEXISTENCIAPARAENSAMBLAR(m_Docto.IID, null))
                {
                    MessageBox.Show("No hay existencias suficientes para hacer los kits");

                    WFListaFaltantesEnsamble listaEnsamble = new WFListaFaltantesEnsamble((int)m_Docto.IID);
                    listaEnsamble.ShowDialog();

                    listaEnsamble.Dispose();
                    GC.SuppressFinalize(listaEnsamble);
                ...
                if (KIT_APLICAR(m_Docto.IID, fTrans) == false)
                {
                    fTrans.Rollback();
                    fConn.Close();
            ...
            MessageBox.Show("La finalizacion del ensamble se realizo correctamente");
            return bRes;
        }

        private void WFEnsambleKit_Load(object sender, EventArgs e)
        {
            ...
            this.ALMACENFuenteComboBox.llenarDatos();

            this.ALMACENDestinoComboBox.Enabled = m_manejaAlmacen;
            this.ALMACENFuenteComboBox.Enabled = m_manejaAlmacen;
