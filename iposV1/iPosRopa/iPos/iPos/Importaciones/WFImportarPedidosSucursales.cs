...
namespace iPos
{
    public partial class WFImportarPedidosSucursales : IposForm
    {
        private long m_TipoDoctoID;



        public WFImportarPedidosSucursales()
        {
            InitializeComponent();
            m_TipoDoctoID = 11;
        }

        public WFImportarPedidosSucursales(long TipoDoctoID) : this()
        {
            m_TipoDoctoID = TipoDoctoID;
        }
        private void WFImportarPedidosSucursales_Load(object sender, EventArgs e)
        {
        ...
        private void iMP_FILESDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (this.iMP_FILESDataGridView[e.ColumnIndex, e.RowIndex].Value.ToString() == "Recibir")
                {
                    WFProcesarPedidoSucursalDet fic;

                    fic = new WFProcesarPedidoSucursalDet((long)iMP_FILESDataGridView["GR_ID", e.RowIndex].Value, m_TipoDoctoID
                        , (string)iMP_FILESDataGridView["DGCLAVESUCURSAL", e.RowIndex].Value, (string)iMP_FILESDataGridView["DGDESCRIPCION", e.RowIndex].Value);
                ...
                else if (this.iMP_FILESDataGridView[e.ColumnIndex, e.RowIndex].Value.ToString() == "Eliminar")
                {
