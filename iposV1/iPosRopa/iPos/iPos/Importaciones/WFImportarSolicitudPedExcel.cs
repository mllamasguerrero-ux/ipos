...
namespace iPos
{
    public partial class WFImportarSolicitudPedExcel : IposForm
    {
        ...
        public WFImportarSolicitudPedExcel()
        {
            InitializeComponent();
        ...
        private void ImportarCatalogos()
        {
            ...
            if ((new Invoker(openFileDialog1).Invoke())  == DialogResult.OK)
            {
                try
                {
        ...
        {
            //Thread hilo = new Thread(ImportarCatalogos);
        ...
        public bool ImportarCompraFromExcel(string archivoExcel, ref string comentario)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        item.Add("CAJAS", decimal.Parse(dr["CAJAS"].ToString()));
                    }
                    else
                    {
                        item.Add("CAJAS", 0);
                    ...
                    {
                        item.Add("PIEZAS", decimal.Parse(dr["PIEZAS"].ToString()));
                    }
                    else
                    {
                        item.Add("PIEZAS", 0);
