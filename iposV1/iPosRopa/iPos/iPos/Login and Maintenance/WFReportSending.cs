    ...
    public partial class WFReportSending : IposForm
    {
        ...
        private void WFReportSending_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                export.Account.EnableSSL = CurrentUserID.CurrentParameters.ISMTPSSL.Equals("S");

                // set up email properties...

                string emailDestinosStr = m_mailEnvio == "" ? CurrentUserID.CurrentParameters.IMAILTOINVENTARIO : m_mailEnvio.Replace(";", ",");
                



                export.Subject = m_asunto == null || m_asunto == "" ? "Exportacion de informacion" : m_asunto;
                export.MessageBody = m_body == null || m_body == "" ? "Exportacion de informacion" : m_body;

                //if(m_mailEnvio == "")
                //{
                //    m_mailEnvio = CurrentUserID.CurrentParameters.IMAILTOINVENTARIO;
                //}
                
                //if(m_mailEnvio.Contains(","))
                //{
                //    export.CC = m_mailEnvio.Split(',');
                //}
                //else
                //{
                //    export.Address = m_mailEnvio;
                //}

                if (emailDestinosStr.Contains(","))
                {
                    string[] emailDestinoArr = emailDestinosStr.Split(',');
                    export.Address = emailDestinoArr[0];
                    export.CC = emailDestinoArr;
                }
                else
                {
                    export.Address = emailDestinosStr;
                }
