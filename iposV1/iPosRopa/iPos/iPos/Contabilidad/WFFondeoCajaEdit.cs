...
namespace iPos
{
    public partial class WFFondeoCajaEdit : IposForm
    {
        ...
        public WFFondeoCajaEdit(bool bSeleccionarCajero, long autorizoId)
            : this()
        {
        ...
        public WFFondeoCajaEdit(long doctoId, string modo)
            : this()
        {
            predoctoId = doctoId;
            m_modo = modo;
        ...
        private void WFFondeoCajaEdit_Load(object sender, EventArgs e)
        {
        ...
        public void ImprimirTickets(long doctoId)
        {
            ...
            int iCantidadTickets = CurrentUserID.CurrentParameters.ICANTTICKETFONDOCAJA > 0 ? CurrentUserID.CurrentParameters.ICANTTICKETFONDOCAJA : 1;
        ...
        public void ImprimirTicket(long doctoId)
        {
            ...
            this.fONDEO_CAJA_IMPRESIONTableAdapter.Fill(this.dSContabilidad3.FONDEO_CAJA_IMPRESION, doctoId);

            if (this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows.Count < 1)
            {
                return;
            }

            long lTipoDoctoId = long.Parse(this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["TIPODOCTOID"].ToString());

            ticket.AddHeaderLine("" + this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["TIPODOCTOCLAVE"].ToString());
            ticket.AddHeaderLine("Sucursal        : " + this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["SUCURSAL"].ToString());
            ticket.AddHeaderLine("Fecha/Hora      : " + ((DateTime)this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["FECHAHORA"]).ToString("dd/MM/yyyy HH:mm"));
            ticket.AddHeaderLine("Fecha Corte     : " + ((DateTime)this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["FECHA"]).ToString("dd/MM/yyyy"));
            ticket.AddHeaderLine("Usuario Corte   : " + this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["USUARIONOMBRE"].ToString());



            ticket.AddHeaderLine(new string('-', Ticket.maxChar));
            ticket.AddHeaderLine("");
            if (this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["NOTAS"].ToString().Trim().Length > 0)
            {
                ticket.AddHeaderLine("Nota             ");
                ticket.AddHeaderLine(this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["NOTAS"].ToString());
                ticket.AddHeaderLine("");
            }
            ticket.AddHeaderLine(formatTotalLine("Monto: ", decimal.Parse(this.dSContabilidad3.FONDEO_CAJA_IMPRESION.Rows[0]["TOTAL"].ToString()).ToString("N2")));
            ticket.AddHeaderLine(new string('-', Ticket.maxChar));
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                doctoBE.ITIPODOCTOID = DBValues._DOCTO_TIPO_FONDEO_CAJA;
                doctoBE.IPERSONAID = DBValues._CLIENTEMOSTRADOR;


                doctoBE.IREFERENCIAS = TBNotas.Text;
                doctoBE.ITOTAL = importe;
                if (!doctoD.FONDEO_INSERT(ref doctoBE, null))
                {
                    fTrans.Rollback();
                    fConn.Close();
                    MessageBox.Show("Error al insertar el fondeo, asegurese de que el corte del vendedor este abierto");
                    return;
                }
                ...
                corteBE.IFECHACORTE = DTPFecha.Value;
                corteBE = corteD.seleccionarCORTEXDIAyCAJERO(corteBE, fTrans);
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            ...
            else if (m_Docto.IESTATUSDOCTOID == DBValues._DOCTO_ESTATUS_COMPLETO)
            {
                ...
                if (m_Docto.ICORTEID == CurrentUserID.CurrentUser.ICORTEID)
                {
                    ...
                    if (!usuariosD.UsuarioTienePermisos((int)CurrentUserID.CurrentUser.IID, (int)DBValues._DERECHO_FONDEOCANCELAR, null))
                    {
                        MessageBox.Show("No tiene derechos para cancelar fondeos en el corte actual");
                        return;
                ...
                else if (m_Docto.IVENDEDORID == CurrentUserID.CurrentUser.IID)
                {
                    ...
                    if (!usuariosD.UsuarioTienePermisos((int)CurrentUserID.CurrentUser.IID, (int)DBValues._DERECHO_FONDEOCANCELAR, null))
                    {
                        ...
                        MessageBox.Show("No tiene derechos para cancelar fondeos en un corte no activo");
                        return;
                    }
                }
                else
                {
                    ...
                    if (!usuariosD.UsuarioTienePermisos((int)CurrentUserID.CurrentUser.IID, (int)DBValues._DERECHO_FONDEOCANCELAR, null))
                    {
                        ...
                        MessageBox.Show("No tiene derechos para cancelar fondeos de otro cajero");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    if (doctoD.FONDEO_CANCEL(m_Docto, fTrans))
                    {
                        fTrans.Commit();
                        fConn.Close();
                        MessageBox.Show("El registro se ha cancelado");
