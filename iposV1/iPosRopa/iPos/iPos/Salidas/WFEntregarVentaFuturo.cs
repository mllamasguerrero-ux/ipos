...
namespace iPos
{
    public partial class WFEntregarVentaFuturo : IposForm
    {
        private long m_TipoDoctoID;



        public WFEntregarVentaFuturo()
        {
            InitializeComponent();
            m_TipoDoctoID = 821;
        }

        public WFEntregarVentaFuturo(long TipoDoctoID) : this()
        {
            m_TipoDoctoID = TipoDoctoID;
        }
        private void WFEntregarVentaFuturo_Load(object sender, EventArgs e)
        {
            ...
            this.vENTASFUTUROPENDTableAdapter.Fill(this.dSSalidasAux.VENTASFUTUROPEND, (int)m_TipoDoctoID);
        ...
        private void iMP_FILESDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (this.iMP_FILESDataGridView[e.ColumnIndex, e.RowIndex].Value.ToString() == "Surtir")
                {
                    WFEntregarVentaFuturoDet fic;

                    fic = new WFEntregarVentaFuturoDet((long)iMP_FILESDataGridView["GR_ID", e.RowIndex].Value, m_TipoDoctoID);

                    fic.d_totalVentaFuturo = (decimal)iMP_FILESDataGridView["GR_TOTAL", e.RowIndex].Value;
                    fic.d_aplicadoVentaFuturo = (decimal)iMP_FILESDataGridView["APLICADO", e.RowIndex].Value;
                    fic.d_porAplicarVentaFuturo = (decimal)iMP_FILESDataGridView["PORAPLICAR", e.RowIndex].Value;
                ...
                else if (this.iMP_FILESDataGridView[e.ColumnIndex, e.RowIndex].Value.ToString() == "Finalizar")
                {

                    if (MessageBox.Show("Quiere finalizar la venta a futuro?", "Finalizar la venta a futuro", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                        return;

                    if (FinalizarDocto((long)iMP_FILESDataGridView["GR_ID", e.RowIndex].Value, null))
                    {
                        this.vENTASFUTUROPENDTableAdapter.Fill(this.dSSalidasAux.VENTASFUTUROPEND, (int)m_TipoDoctoID);
                    }
