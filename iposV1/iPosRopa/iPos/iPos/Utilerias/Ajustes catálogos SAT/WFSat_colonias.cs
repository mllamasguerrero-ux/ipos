...
{
    public partial class WFSat_colonias : IposForm
    {
        public DataRow m_rtnDataRow;
        public bool m_bRetornarValor;

        public WFSat_colonias()
        {
            InitializeComponent();
        ...
        public WFSat_colonias(string palabrasBusqueda)
            : this()
        {
            m_bRetornarValor = true;
        }

        private void WFSat_colonias_Load(object sender, EventArgs e)
        {
        ...
        private void MostrarRegistro(string opc)
        {
            try
            {
                ...
                WFSat_coloniaEdit fPi = new WFSat_coloniaEdit();
        ...
        private void LlenarGrid()
        {
            ...
            if(TSTPalabrasClave.Text.Length < 3)
            {
                MessageBox.Show("Por favor ingrese al menos 3 caracteres en el texto de busqueda");
                return;
            }
            ...
            try
            {
                strABuscar = "%" + TSTPalabrasClave.Text.ToUpper() + "%";
                numeroRegistros = this.TableTableAdapter.Fill(this.dSCatalogos.SAT_COLONIA_VIEW, strABuscar);
                if (this.dSCatalogos.SAT_COLONIA_VIEW.Rows.Count > 0)
