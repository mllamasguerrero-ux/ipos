...
namespace iPos
{
    public partial class WFValidarFirmaDocumento : IposForm
    {

        public CDOCTOBE m_selectedDocto = null;
        public string m_tipoBusqueda = "Ventas";

        public WFValidarFirmaDocumento()
        {
            InitializeComponent();
        }


        public WFValidarFirmaDocumento(string tipoBusqueda):this()
        {
            m_tipoBusqueda = tipoBusqueda;
        }

        private void WFValidarFirmaDocumento_Load(object sender, EventArgs e)
        {
        ...
        private void BuscarFolio()
        {
            ...
            try
            {
                ...
                personaBE = personaD.SeleccionarPersonaPorId(doctoBE.IPERSONAID, null);

                pnlFirma.Visible = true;

                if (personaBE.IFIRMAIMAGEN != null && personaBE.IFIRMAIMAGEN != "")
                {

                    lblNoFirmasRegistradas.Visible = false;
                    FIRMAIMAGENPictureBox.Visible = true;

                    string imagePath = CurrentUserID.CurrentParameters.IRUTAFIRMAIMAGENES + "\\" + personaBE.IFIRMAIMAGEN;
                    try
                    {
