create or alter procedure REP_DOCTO_CONSALDO (
    FECHAREF D_FECHA,
    TIPODOCTOID D_FK,
    VENCIDOS D_BOOLCN,
    PALMACENID D_FK)
...
BEGIN
   ...
   FOR SELECT
      ...
      a_t.nombre nombrealmacentraspas ,
      D.ID
   ...
   WHERE (TIPODOCTOID = :TIPODOCTOID  OR (:TIPODOCTOID = -21 AND TIPODOCTOID IN (21,31,23,81))
       or  (:TIPODOCTOID = -701 AND TIPODOCTOID IN (21,31,23,81,701))
          OR (:TIPODOCTOID = -31 AND TIPODOCTOID IN (31,81)) )
     and  (:vencidos = 'N' or dateadd (coalesce(p.dias, 0) day to d.fecha) <= :fecharef)
