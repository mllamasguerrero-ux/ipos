create or alter procedure PRODUCTOS_IMPORTAR_MOVIL (
    CLAVE D_CLAVE,
    NOMBRE D_NOMBRE,
    ...
    CLASIFICA D_PCLASIF,
    PROMOMOVIL D_BOOLCN,
    MARGENMOVIL D_PRECIO,
    MPRECIO4MOVIL D_PRECIO,
    CPRECIO4MOVIL D_PRECIO,
    TPRECIO4MOVIL D_PRECIO,
    APRECIO4MOVIL D_PRECIO)
returns (
...
BEGIN
  ...
  into :LINEAID;
   ...
   if(:tasaiva is null and coalesce(:tasaivaid, -2) >= 0) then
   begin
      select first 1 tasa from tasaiva where id = :tasaivaid into :tasaiva;
   end

   TASAIMPUESTO = COALESCE(:TASAIEPS,0)  +  COALESCE(:TASAIVA,0) +  ((COALESCE(:TASAIEPS,0) * COALESCE(:TASAIVA,0))/100) ;
