create or alter procedure FACTURAELECTRONICA_XML_GROUP (
    DOCTOID D_FK)
returns (
    ID D_FK,
...
BEGIN
  ...
  FOR SELECT
    ...
    WHERE
        (:TIPODOCTOID NOT IN (701) or  COALESCE(tipolinea,'') NOT IN('CONCEPTO_PARTE', 'CONCEPTO_ISRRETENIDO','CONCEPTO_IVARETENIDO', 'CONCEPTO_IVA', 'CONCEPTO_IEPS',  'TOTAL_TRASLADOSRETENCIONES', 'COMPROBANTE_ISRRETENIDO', 'COMPROBANTE_IVARETENIDO', 'COMPROBANTE_IVA','COMPROBANTE_IEPS' ))
        AND  (:TIPODOCTOID NOT IN (21,22) or  COALESCE(tipolinea,'') NOT IN(  'CONCEPTO_ISRRETENIDO','CONCEPTO_IVARETENIDO', 'CONCEPTO_IVA', 'CONCEPTO_IEPS',  'TOTAL_TRASLADOSRETENCIONES', 'COMPROBANTE_ISRRETENIDO', 'COMPROBANTE_IVARETENIDO', 'COMPROBANTE_IVA','COMPROBANTE_IEPS' ))
    ...
    BEGIN
      ...
      FOR SELECT
                                    ID,-- MIN(ID),
                                    TIPOLINEA ,
                                    ORDENLINEA, --MIN(ORDENLINEA) ,
                                    COALESCE(IMPORTE,0), --SUM(COALESCE(IMPORTE,0)) ,
                                    IMPUESTO,
                                    Tasa ,
                                    TasaCuota ,
                                    TipoFactor ,
                                    CASE WHEN COALESCE(TASA,0) > 0 THEN COALESCE(IMPORTE,0.00) / (COALESCE(TASA,0.00) / 100.00) ELSE  COALESCE(BaseImp,0) END  BASEIMP,
                                    --CASE WHEN COALESCE(TASA,0) > 0 THEN SUM(COALESCE(IMPORTE,0.00)) / (COALESCE(TASA,0.00) / 100.00) ELSE  SUM(COALESCE(BaseImp,0)) END  BASEIMP,
                                    PADRELINEA
          FROM
          ...
          --GROUP BY TIPOLINEA, IMPUESTO, TASA, TASACUOTA, TIPOFACTOR, PADRELINEA
              ...
              BEGIN
                 ...
                 if(coalesce(:IMPORTE,0) <> 0 or
                    (:TIPODOCTOID IN (21,22)  and coalesce(:IMPUESTO,'') = '002')
                   ) THEN
                 BEGIN  
                    SUSPEND;
                 END
          ...
          GROUP BY TIPOLINEA, IMPUESTO, TASA, TASACUOTA, TIPOFACTOR, PADRELINEA
              ...
              BEGIN
                ID = :ID + 1;
                 ...
                 IF(COALESCE(:TIPOLINEA,'') = 'CONCEPTO_ISRRETENIDO' or COALESCE(:TIPOLINEA,'') = 'CONCEPTO_IVARETENIDO') THEN
                 BEGIN
                 ...
                 if(coalesce(:IMPORTE,0) <> 0 or
                    (:TIPODOCTOID IN (701)  and coalesce(:IMPUESTO,'') = '002' and coalesce(:BaseImp,0) <> 0)
                   ) THEN
                 BEGIN  
                    SUSPEND;
                 END
    ...
    IF(:TIPODOCTOID IN (701,21,22)) THEN
    BEGIN
          ...
          GROUP BY TIPOLINEA, IMPUESTO, TASA, TASACUOTA, TIPOFACTOR
              ...
              BEGIN
                ...
                TIPOLINEA =  case WHEN TIPOLINEA = 'CONCEPTO_ISRRETENIDO' then 'COMPROBANTE_ISRRETENIDO'
                 ...
                 if(coalesce(:IMPORTE,0) <> 0 or
                    (:TIPODOCTOID IN (701,21,22)  and coalesce(:IMPUESTO,'') = '002')
                   ) THEN
                 BEGIN  
                    SUSPEND;
                 END
