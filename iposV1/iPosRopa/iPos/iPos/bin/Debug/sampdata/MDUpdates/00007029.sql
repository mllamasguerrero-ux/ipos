create or alter procedure CONSOLIDADO_PORCONSOLIDAR (
    ...
    OMITIRPAGOTARJETA D_BOOLCN_NULL,
...
declare variable DOCTOMAXIMOPORCID D_FK;
declare variable MAXMONTOPORPORCVTA D_PRECIO;
declare variable MAXIMOMONTOPORPORC D_PRECIO;
BEGIN
   NUMERO = 0;
   DOCTOMAXIMOID = 0;
    ...
    DOCTOMAXIMOPORCID = 0;
    MAXMONTOPORPORCVTA = 0;
    MAXIMOMONTOPORPORC = 0;


    SELECT VERSIONCFDI FROM PARAMETRO INTO :VERSIONCFDI;

    IF(COALESCE(:MAXIMOPORCENTAJE,0.00) > 0.00 ) THEN
    BEGIN
        MAXIMOMONTO = 0.00;
    END
    ...
    IF(COALESCE(:MAXIMOMONTO, 0) > 0  AND (COALESCE(:VERSIONCFDI,'') = '3.3' or COALESCE(:VERSIONCFDI,'') = '4.0')) THEN
    BEGIN
       TOTALACUMULADO = 0;
      ...
      FOR SELECT DOCTO.ID , DOCTO.total
            FROM         DOCTO LEFT OUTER JOIN
                         ...
                         PERSONA USUARIO ON USUARIO.ID = DOCTO.VENDEDORID  LEFT JOIN
                         PERSONA CLIENTE ON CLIENTE.ID = DOCTO.PERSONAID LEFT JOIN
                         (SELECT distinct TFR.FILTRO_INTEGER FROM TEMP_FILTRO_REPO TFR WHERE TFR.NOMBRE_REPORTE = 'FacturacionXDia' AND TFR.GRUPO_REPORTE = :GRUPOFILTROFOLIOS) FOLIOS
                             ON FOLIOS.FILTRO_INTEGER = DOCTO.FOLIO
            WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
                ...
                AND  (COALESCE(:GRUPOUSUARIOID,0) = 0 or (COALESCE(:GRUPOUSUARIOID,0) = USUARIO.GRUPOUSUARIOID) )
                AND (COALESCE(:OMITIRCLIENTESRFC,'N') = 'N' OR COALESCE(CLIENTE.RFC,'') = '' OR COALESCE(CLIENTE.RFC,'') = 'XAXX010101000')
                AND (COALESCE(:GRUPOFILTROFOLIOS,'') = '' or (COALESCE(FOLIOS.FILTRO_INTEGER, -1) > 0) )
              ORDER BY DOCTO.ID
             ...
             BEGIN
                    ...
                    FROM    DOCTO DOCTODEV
                        ...
                        LEFT JOIN
                            PARAMETRO ON 1 = 1
                        LEFT JOIN
                         PERSONA USUARIO ON USUARIO.ID = DOCTODEV.VENDEDORID
                    WHERE
    ...
    END
     ...
     -- SI HAY UN MAXIMO DE PORCENTAJE A CONSIDERAR, ENTONCES NECESITAMOS VER HASTA CUAL DOCTO CONSIDERAREMOS
    IF(COALESCE(:MAXIMOPORCENTAJE, 0) > 0  AND (COALESCE(:VERSIONCFDI,'') = '3.3' or COALESCE(:VERSIONCFDI,'') = '4.0')) THEN
    BEGIN
       TOTALACUMULADO = 0;
       SUMADESCBUFFERCONVALE = 0;
       SUMADESCCONVALE = 0;
       ...
       SELECT
            SUM(cast(coalesce(TOTAL, 0.00) * COALESCE(:MAXIMOPORCENTAJE, 0.00)/100.00 as d_precio))   AS SUMATOTAL
        FROM
            CONSOLIDADO_PORCONSOLIDAR(:FECHAINICIAL, :FECHAFINAL, :OMITIRVENTASAFRANQUICIAS, :INCLUIRGASTOS, 0, :OMITIRVALES, :SUMACOMPLETAVENTAS, :GRUPOUSUARIOID, :OMITIRCLIENTESRFC, 0.00,'S', :GRUPOFILTROFOLIOS)
        WHERE COALESCE(APLICADO,'S') = 'S'
        INTO :MAXIMOMONTOPORPORC;
      ...
      FOR SELECT DOCTO.ID , DOCTO.total
            ...
            WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
                ...
                AND (COALESCE(PARAMETRO.pagoservconsolidado,'S') = 'S' or COALESCE(DOCTO.subtipodoctoid,0) NOT IN (24,22) )
                AND  (COALESCE(:GRUPOUSUARIOID,0) = 0 or (COALESCE(:GRUPOUSUARIOID,0) = USUARIO.GRUPOUSUARIOID) )
                AND (COALESCE(:OMITIRCLIENTESRFC,'N') = 'N' OR COALESCE(CLIENTE.RFC,'') = '' OR COALESCE(CLIENTE.RFC,'') = 'XAXX010101000'  OR ( COALESCE(DOCTO.PAGOCONTARJETA,'N') <> 'N' AND COALESCE(:MAXIMOPORCENTAJE, 0.00) > 0.00 ))
                AND ( COALESCE(DOCTO.PAGOCONTARJETA,'N') = 'N')
                AND (COALESCE(:GRUPOFILTROFOLIOS,'') = '' or (COALESCE(FOLIOS.FILTRO_INTEGER, -1) > 0) )
              ORDER BY DOCTO.TOTAL , DOCTO.ID
             INTO :DOCTOBUFFERID, :TOTALVENTABUFFER
             DO
             BEGIN
                    SUMADESCCONVALE = 0;
                    ...
                    WHERE
                        ...
                        DOCTOMAXIMOPORCID = :DOCTOBUFFERID;
                        MAXMONTOPORPORCVTA = :TOTALVENTABUFFER;

                        IF(COALESCE(:TOTALACUMULADO,0) > COALESCE(:MAXIMOMONTOPORPORC, 0)) THEN
                        BEGIN
                              BREAK;
                        END
   ...
   FROM            DOCTO LEFT OUTER JOIN
                         TIPODOCTO ON TIPODOCTO.ID = DOCTO.TIPODOCTOID LEFT JOIN
                         PARAMETRO ON 1 = 1 LEFT JOIN
                         PERSONA CLIENTE ON CLIENTE.ID = DOCTO.PERSONAID
    WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
         AND DOCTO.esfacturaelectronica = 'N'
         AND DOCTO.ESTATUSDOCTOID = 1
         AND DOCTO.TIPODOCTOID IN (21)
        ...
        FROM    DOCTO INNER join
                ...
                PARAMETRO ON 1 = 1
                AND (COALESCE(:VERSIONCFDI,'') <> '3.3' AND COALESCE(:VERSIONCFDI,'') <> '4.0') LEFT JOIN
                PERSONA CLIENTE ON CLIENTE.ID = DOCTO.PERSONAID
        ...
        FROM    DOCTO DOCTODEV LEFT join
                ...
                LEFT JOIN
                PARAMETRO ON 1 = 1  LEFT JOIN
                PERSONA CLIENTE ON CLIENTE.ID = DOCTODEV.PERSONAID
        ...
        WHERE
           (DOCTODEV.FECHA >= :FECHAINICIAL) AND
            ...
            DOCTODEV.ESTATUSDOCTOID = 1-- AND DOCTODEV.tipodoctoid = 22  
            AND (COALESCE(PARAMETRO.pagoservconsolidado,'S') = 'S' or COALESCE(DOCTODEV.subtipodoctoid,0) NOT IN (24,22) )
            AND (COALESCE(:VERSIONCFDI,'') <> '3.3' AND COALESCE(:VERSIONCFDI,'') <> '4.0')
            AND (COALESCE(:OMITIRCLIENTESRFC,'N') = 'N' OR COALESCE(CLIENTE.RFC,'') = '' OR COALESCE(CLIENTE.RFC,'') = 'XAXX010101000')
        ...
        FROM    DOCTO DOCTODEV LEFT join
                ...
                LEFT JOIN
                TIPODOCTO ON TIPODOCTO.ID = DOCTODEV.TIPODOCTOID
                LEFT JOIN PARAMETRO ON 1 = 1  LEFT JOIN
                PERSONA CLIENTE ON CLIENTE.ID = DOCTODEV.PERSONAID
                         ...
                         DOCTO.TIPODOCTOID ,
                          ...
                          'VENTA' NOMBRETIPOAPLICACION ,
                                ...
                                CASE WHEN
                                   ...
                                   (COALESCE(DOCTO.PAGOCONTARJETA,'N') <> 'N' OR

                                        (
                                          (COALESCE(:DOCTOMAXIMOID, 0) = 0 or COALESCE(DOCTO.id, 0) <= COALESCE(:DOCTOMAXIMOID, 0) )   AND
                                            (COALESCE(:DOCTOMAXIMOPORCID, 0) = 0 or
                                                (
                                                    COALESCE(DOCTO.TOTAL, 0) < COALESCE(:MAXMONTOPORPORCVTA, 0) OR
                                                    (
                                                        COALESCE(DOCTO.TOTAL, 0) = COALESCE(:MAXMONTOPORPORCVTA, 0) AND
                                                        COALESCE(:DOCTOMAXIMOPORCID, 0) >= COALESCE(DOCTO.ID, 0)
                                                    )
                                                )
                                            )
                                     )
   ...
   FROM            DOCTO LEFT OUTER JOIN
    ...
    WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
         ...
         AND  (COALESCE(:GRUPOUSUARIOID,0) = 0 or (COALESCE(:GRUPOUSUARIOID,0) = USUARIO.GRUPOUSUARIOID) )     
         AND (COALESCE(:OMITIRCLIENTESRFC,'N') = 'N' OR COALESCE(CLIENTE.RFC,'') = '' OR COALESCE(CLIENTE.RFC,'') = 'XAXX010101000' OR ( COALESCE(DOCTO.PAGOCONTARJETA,'N') <> 'N' AND COALESCE(:MAXIMOPORCENTAJE, 0.00) > 0.00 ))
         AND (COALESCE(:OMITIRPAGOTARJETA,'N') = 'N' OR COALESCE(DOCTO.PAGOCONTARJETA,'N') = 'N')
         AND (COALESCE(:GRUPOFILTROFOLIOS,'') = '' or (COALESCE(FOLIOS.FILTRO_INTEGER, -1) > 0) )
        ...
        UNION
         ...
         SELECT
            ...
            'EGRESOS DEVOLUCIONES' NOMBRETIPOAPLICACION ,
            CASE WHEN
              (COALESCE(DOCTODEV.PAGOCONTARJETA,'N')  <> 'N' OR
                 (
                    COALESCE(:OMITIRVALES,'N') = 'N' AND
                        (COALESCE(:DOCTOMAXIMOID, 0) = 0 or COALESCE(DOCTODEV.doctorefid, 0) <= COALESCE(:DOCTOMAXIMOID, 0) )  AND
                        (COALESCE(:DOCTOMAXIMOPORCID, 0) = 0 or
                           (
                                COALESCE(DOCTOVENTA.TOTAL, 0) < COALESCE(:MAXMONTOPORPORCVTA, 0) OR
                                (
                                    COALESCE(DOCTOVENTA.TOTAL, 0) = COALESCE(:MAXMONTOPORPORCVTA, 0) AND
                                    COALESCE(:DOCTOMAXIMOPORCID, 0) >= COALESCE(DOCTODEV.doctorefid, 0)
                                )
                           )
                        )
                   )
        ...
        FROM    DOCTO DOCTODEV LEFT join
                ...
                LEFT JOIN
                PARAMETRO ON 1 = 1     LEFT JOIN
                PERSONA USUARIO ON USUARIO.ID = DOCTODEV.VENDEDORID LEFT JOIN
                PERSONA CLIENTE ON CLIENTE.ID = DOCTODEV.PERSONAID  LEFT JOIN
                DOCTO DOCTOVENTA ON DOCTOVENTA.ID = DOCTODEV.DOCTOREFID  LEFT JOIN
                (SELECT distinct TFR.FILTRO_INTEGER FROM TEMP_FILTRO_REPO TFR WHERE TFR.NOMBRE_REPORTE = 'FacturacionXDia' AND TFR.GRUPO_REPORTE = :GRUPOFILTROFOLIOS) FOLIOS
                     ON FOLIOS.FILTRO_INTEGER = DOCTOVENTA.FOLIO


        WHERE
           (DOCTOVENTA.FECHA >= :FECHAINICIAL) AND
            (DOCTOVENTA.FECHA <= :FECHAFINAL) AND
            (DOCTODEV.ESFACTURAELECTRONICA = 'N') AND
            (DOCTODEV.ESTATUSDOCTOID = 1) AND
            ( DOCTODEV.TIPODOCTOID IN (22)) AND
            (DOCTODEV.FACTCONSID IS NULL) AND
            ...
            DOCTODEV.ESTATUSDOCTOID = 1-- AND DOCTODEV.tipodoctoid = 22  
            AND (COALESCE(PARAMETRO.pagoservconsolidado,'S') = 'S' or COALESCE(DOCTODEV.subtipodoctoid,0) NOT IN (24,22) )
            AND (COALESCE(:VERSIONCFDI,'') = '3.3' or COALESCE(:VERSIONCFDI,'') = '4.0')
            AND COALESCE(:OMITIRVALES , 'N') = 'N'
            AND COALESCE(:SUMACOMPLETAVENTAS , 'N') = 'N' 
            AND  (COALESCE(:GRUPOUSUARIOID,0) = 0 or (COALESCE(:GRUPOUSUARIOID,0) = USUARIO.GRUPOUSUARIOID) )    
            AND (COALESCE(:OMITIRCLIENTESRFC,'N') = 'N' OR COALESCE(CLIENTE.RFC,'') = '' OR COALESCE(CLIENTE.RFC,'') = 'XAXX010101000' OR ( COALESCE(DOCTODEV.PAGOCONTARJETA,'N') <> 'N' AND COALESCE(:MAXIMOPORCENTAJE, 0.00) > 0.00 ))
            AND (COALESCE(:OMITIRPAGOTARJETA,'N') = 'N' OR COALESCE(DOCTODEV.PAGOCONTARJETA,'N') = 'N')
            AND (COALESCE(:GRUPOFILTROFOLIOS,'') = '' or (COALESCE(FOLIOS.FILTRO_INTEGER, -1) > 0) )
