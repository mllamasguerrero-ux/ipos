...
BEGIN
   ...
   BEGIN
      FOR
            select P.ID, p.fecha,
            case when ds.tipodoctoid = 201 then 'Anticipo de cliente'
            when fs.nombre is null or p.formapagosatid = 99 or p.formapagoid in (20,21) then f.nombre
            else fs.nombre end  as formapago,
            coalesce((p.importe *
          (CASE WHEN ((p.TIPOPAGOID = 1 AND p.FORMAPAGOID IN (1,2,3,4,5,14,15,16,17,18,20,21)) or (p.TIPOPAGOID = 2 AND p.FORMAPAGOID IN (7,19)))  THEN 1
           ...
           p.BANCOMERPARAMID,
           P.COMISION,
           coalesce(P.NOTAS,'') || ' ' || coalesce(pago.notas,''),
           P.APLICADO,
           P.FECHAAPLICADO ,
           PAGO.ID PAGOID,
           case when ds.tipodoctoid = 201 then ds.totalanticipo else PAGO.IMPORTE end MONTOPAGOCOMPUESTO   ,
           PAGO.REFINTERNO
          ...
          left join docto de on de.id = p.doctoid
          left join docto ds on ds.id = p.doctosalidaid
          where (p.doctoid =:DOCTOID or p.doctosalidaid = :doctoid)  and f.abona = 'S'
          order by p.id
      INTO
            :ID, :FECHA, :FORMADEPAGO, :MONTO,  :REFERENCIABANCARIA  ,:BANCO , :FORMAPAGOID, :TIPOABONODESCRIPCION  , :TIPOABONOID  , :REVERTIDO  , :FOLIOREF, :FECHAELABORACION, :FECHARECEPCION  , :NOMBREVENDEDOR , :FORMAPAGOSATID  , :RECIBOELECTRONICOID , :ESCONPINPADBANCOMER ,:BANCOMERPARAMID  , :COMISION , :NOTAS, :APLICADO, :FECHAAPLICADO , :PAGOID, :MONTOPAGOCOMPUESTO, :REFINTERNO
   ...
   BEGIN
      FOR
            select P.ID,p.fecha,
            case when de.tipodoctoid = 202 then 'Anticipo de proveedor'
            when fs.nombre is null or p.formapagosatid = 99  or p.formapagoid in (20,21) then f.nombre
            else fs.nombre end  as formapago,
            coalesce((p.importe *
           ...
           P.FECHAAPLICADO  ,
           PAGO.ID PAGOID,
           case when de.tipodoctoid = 202 then de.totalanticipo  else PAGO.IMPORTE end MONTOPAGOCOMPUESTO   ,
           PAGO.REFINTERNO
          from doctopago p
          inner join formapago f on p.formapagoid = f.id  
          left join bancos b on b.id = p.banco     
          ...
          order by p.id
      INTO
            :ID, :FECHA, :FORMADEPAGO, :MONTO, :BANCO, :REFERENCIABANCARIA, :FORMAPAGOID, :TIPOABONODESCRIPCION   , :TIPOABONOID , :REVERTIDO , :FOLIOREF , :FECHAELABORACION, :FECHARECEPCION , :FORMAPAGOSATID , :RECIBOELECTRONICOID   , :ESCONPINPADBANCOMER ,:BANCOMERPARAMID , :COMISION, :NOTAS, :APLICADO, :FECHAAPLICADO , :PAGOID, :MONTOPAGOCOMPUESTO  , :REFINTERNO
      DO
      BEGIN
