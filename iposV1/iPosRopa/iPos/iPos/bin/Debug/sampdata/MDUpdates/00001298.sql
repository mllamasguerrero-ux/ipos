...
BEGIN
   ...
   iva = (coalesce(tasaiva,0)/100) * costo * movto.cantidad,
    total = costo * movto.cantidad  * ( 1 + (coalesce(tasaiva,0)/100) )
   where movto.doctoid = :doctoid;
   ...
   BEGIN
      UPDATE DOCTO SET TIPODOCTOID = 21, CARGO = TOTAL WHERE ID = :DOCTOID;
