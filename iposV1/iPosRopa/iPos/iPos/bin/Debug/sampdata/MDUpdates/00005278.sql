...
BEGIN
   ...
   BEGIN
        ...
        IF(COALESCE(:PAGOACREDITO,'N') = 'S' or COALESCE(:PRE_FORMAPAGOSAT,'') = '99' ) THEN
        BEGIN      
            FORMAPAGO = '99';
            PAGOENUNASOLAEXIBICION = 'N';
   ...
   BEGIN
            ...
            SELECT
                SERIESAT, FOLIOSAT, DATEADD(-5 MINUTE TO TIMBRADOFECHAFACTURA) FECHA, TOTAL, 'MXN' ,'', '',
                'PUE'  ,
                'X',  :EMPRESA_FISCALCODIGOPOSTAL, subtotal,  0.0
            ...
            INTO
                :SERIE, :FOLIO, :FECHA, :TOTAL, :MONEDA, :TIPOCAMBIO, :CONDICIONESPAGO,
                :METODOPAGO ,
                :MOTIVODESCUENTO, :LUGAREXPEDICION, :SUBTOTAL, :DESCUENTO;
                ...
                ROUND( (COALESCE(MOVTO.SUBTOTAL,0) +
                 ...
                 CASE when COALESCE(:PERSONADESGLOSAIEPS,'N') = 'S' AND
                  COALESCE(:EMPRESADESGLOSAIEPS,'N') = 'S' THEN MOVTO.ieps ELSE 0.0 END,
                   ...
                   MOVTO.isrretenido, MOVTO.TASAISRRETENIDO , PRODUCTO.ESKIT, MOVTO.MOVTOREFID
                 FROM MOVTO
                 ...
                 :PORCENTAJEIEPSMOVTO, :PORCENTAJEIVAMOVTO , :MONTOIVAMOVTO , :MONTOIEPSMOVTO , :MONTOIVARETENIDOMOVTO, :PORCENTAJEIVARETENIDOMOVTO,
                  :MONTOISRRETENIDOMOVTO, :PORCENTAJEISRRETENIDOMOVTO, :ESKIT, :MOVTOREFID
                 DO
                 BEGIN
                 ...
                 PADRELINEA = 0;
                  ...
                  BEGIN
                    ...
                    FOR SELECT DOCTO.ID, DOCTO.folio,   DOCTO.iva,  DOCTO.total,  DOCTO.ieps  ,
                            COALESCE(DOCTO.SUBTOTAL,0) +    CASE when COALESCE(PERSONA.desgloseieps,'N') = 'S' AND
                                            COALESCE(:EMPRESADESGLOSAIEPS,'N') = 'S'  THEN 0.0 ELSE COALESCE(DOCTO.IEPS,0.0) END +
                                            CASE WHEN COALESCE(:MOSTRARDESCUENTOENFACTURA,'N') = 'S' THEN COALESCE(DOCTO.DESCUENTO,0) ELSE 0.0 END SUBTOTAL,
                        ...
                        BEGIN
                             SELECT SUM(COALESCE(MOVTO.SUBTOTAL,0) +
                                        CASE when COALESCE(:PERSONADESGLOSAIEPSCONS,'N') = 'S' AND
                                        COALESCE(:EMPRESADESGLOSAIEPS,'N') = 'S'  THEN 0.0 ELSE COALESCE(MOVTO.IEPS,0.0) END +
                                        CASE WHEN COALESCE(:MOSTRARDESCUENTOENFACTURA,'N') = 'S' THEN COALESCE(MOVTO.DESCUENTO,0) ELSE 0.0 END
                            ...
                            Cantidad = 1.0;
                            Unidad = '';
                            Descripcion = 'Venta ' || cast(coalesce(:doctofoliocons,'') as varchar(20));
                            ...
                            FOR SELECT
                                    ...
                                    BaseImp
                               FROM
                                    FACTURAELECTRONICA_XML_CORE(:DOCTOIDCONS)
                               ...
                               DO BEGIN
                                ...
                                FOR SELECT
                                    ...
                                    BaseImp * -1
                                FROM
                                    FACTURAELECTRONICA_XML_CORE(:DOCTODEVOCONSID)
                                    WHERE TIPOLINEA IN ('CONCEPTO_ISRRETENIDO','CONCEPTO_IVARETENIDO', 'CONCEPTO_IVA', 'CONCEPTO_IEPS')
