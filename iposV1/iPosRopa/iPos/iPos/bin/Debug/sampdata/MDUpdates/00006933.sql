...
declare variable SALDOVENTA D_PRECIO;
BEGIN
  ...
  CAJEROID = 1331;
   ...
   SELECT DOCTOREFID, TOTAL, FECHA, TIPODOCTOID,DOCTOREFID FROM DOCTO WHERE ID = :DOCTOID
   INTO :DOCTOREFID, :DOCTOTOTAL, :FECHA, :TIPODOCTOID, :DOCTOREFID;
   ...
   BEGIN
     ...
     REFERENCIABANCARIATARJETA = '';
      ...
      FROM DOCTOPAGO_INSERT (
        ...
        IF(COALESCE(:TIPODOCTOID,0) = 22 AND COALESCE(:DOCTOREFID,0) <> 0 ) THEN
        BEGIN
             SELECT SALDO FROM DOCTO WHERE ID = :DOCTOREFID INTO :SALDOVENTA;
             IF(COALESCE(:MONTOCREDITO,0) <= COALESCE(:SALDOVENTA,0)) THEN
             BEGIN
                
                FORMAPAGOID = 6; -- Tarjeta
   ...
   IF(COALESCE(:DOCTOTOTAL,0) - COALESCE(:MONTOTARJETA, 0) - COALESCE(:MONTOCREDITO, 0) > 0.01) THEN
   BEGIN
