create or alter procedure INVKIT_ENSAMBLETEMP (
    DOCTOID D_FK,
    VENDEDORID D_FK)
returns (
...
BEGIN
  ...
  ERRORCODE = 0;
   ...
   SELECT first 1 ID FROM DOCTO WHERE TIPODOCTOID = 502 AND DOCTOREFID = :DOCTOID AND
    FECHA >= :FECHAINVENTARIO INTO :DOCTOIDDESENSAMBLADO;

    
   -- Solo admitir TipoDocto = 50, captura de inventario fisico.
   IF (COALESCE(:DOCTOIDDESENSAMBLADO, 0) = 0) THEN
   BEGIN
      ERRORCODE = 1078;
      SUSPEND;
      EXIT;
   ...
   WHERE DOCTOID = :DOCTOIDDESENSAMBLADO
   INTO
       :PRODUCTOID, :LOTE, :FECHAVENCE, :CANTIDAD, :PRECIO, :COSTO,
       :CANTIDADDEFACTURA,
       :CANTIDADDEREMISION,
   ...
   BEGIN
               ...
               SELECT  ROUND(CANTMAXENSAMBLAR,0)
