...
BEGIN
  ...
  --Checar producto descontinuado en venta
   ...
   BEGIN
      SELECT PRECIO1, TASAIVA, COSTOPROMEDIO, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION, ENPROCESODESALIDA
      FROM PRODUCTO
      WHERE ID = :PRODUCTOID
      INTO :PRECIOLISTA, :TASAIVA, :COSTOPROMEDIO, :COSTOREPOSICION, :PRECIOMAXIMOPUBLICO, :NEGATIVOS, :INVENTARIABLE, :UNIDADPRODUCTO, :COMISIONXUNIDAD, :ENPROCESODESALIDA;
   ...
   ELSE
   BEGIN
      SELECT PRECIO1, coalesce(TASAIVA,0), COSTOREPOSICION, PRECIOMAXIMOPUBLICO, COSTOPROMEDIO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION , ENPROCESODESALIDA
      FROM PRODUCTO
      WHERE ID = :PRODUCTOID
      INTO :PRECIOLISTA, :TASAIVA, :COSTOREPOSICION, :PRECIOMAXIMOPUBLICO, :COSTOPROMEDIO, :NEGATIVOS, :INVENTARIABLE, :UNIDADPRODUCTO, :COMISIONXUNIDAD, :ENPROCESODESALIDA;
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF(:TIPODOCTOID IN (21,25,12) AND
      ...
      IF(:CANTIDAD < 0 AND :TIPODOCTOID IN (11,12,21,22,25)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            IF(:TIPODOCTOID IN (21,25,12) AND
                 COALESCE(:EXISTENCIAACTUAL,0) - COALESCE(:ENPROCESODESALIDA,0) < (:CANTIDADACTUAL + :CANTIDAD) )THEN
            BEGIN
                    ERRORCODE = 1081;
                    SUSPEND;
                    EXIT;
      ...
      /* no puede haber cantidades negativas en compras , ventas, devoluciones de compras y devoluciones de ventas*/
      IF((:CANTIDAD + :CANTIDADACTUAL) < 0 AND :TIPODOCTOID IN (11,12,21,22,25)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
      ...
      BEGIN
         ...
         BEGIN
            IF ((:PRECIO IS NOT NULL) AND
            (:PRECIO <> :PRECIONUEVO) AND
            (:LLEGOPRECIOMANUAL = 'S' or :INGRESOPRECIOMANUAL = 'S' )) THEN
            BEGIN
               SELECT ERRORCODE
               FROM VALIDAR_PRECIO(:PRODUCTOID, :PRECIO, :PRECIONUEVO, :COSTO, :VENDEDORID, :TIPODOCTOID)
               INTO :ERRORCODE;
