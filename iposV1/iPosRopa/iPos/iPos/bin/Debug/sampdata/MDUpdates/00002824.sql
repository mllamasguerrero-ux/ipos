...
select
        ...
        ANIO,
        MES,
        SUM(CASE WHEN TIPODOCTOID IN (21,31,81) THEN COALESCE(CANTIDAD,0) ELSE COALESCE(CANTIDAD,0) * -1 END) CANTIDAD,
        SUM(CASE WHEN TIPODOCTOID IN (21,31,81) THEN COALESCE(TOTAL,0) ELSE COALESCE(TOTAL,0) * -1 END) TOTAL,
        SUM(CASE WHEN TIPODOCTOID IN (21,31,81) THEN COALESCE(UTILIDAD,0) ELSE COALESCE(UTILIDAD,0) * -1 END) UTILIDAD
      from ttl_prod_tipo_mes
      where
          ANIO = :ANIO1   AND PRODUCTOID = :PRODUCTOID_P AND
          ( TIPODOCTOID IN (21,22,23) OR   (:INCLUIRTRASL = 'S' AND TIPODOCTOID IN (31,32,33,81,83) AND SUBTIPODOCTOID NOT IN (7,8))    )
