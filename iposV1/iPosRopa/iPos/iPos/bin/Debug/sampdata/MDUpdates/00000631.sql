...
BEGIN
   ...
   END
    ...
    WHERE P.CORTEID = :CORTEID AND P.FORMAPAGOID IN (1,2,3,4,5,14) AND  P.TIPOPAGOID = 1
    INTO :INGRESOPORIVA;
   ...
   INTO :TICKETSDEVOLUCION;
    ...
    INNER JOIN docto D ON p.doctoid = d.id
    WHERE P.CORTEID = :CORTEID AND P.FORMAPAGOID IN (1,2,3,4,5,14) AND  P.TIPOPAGOID = 2 AND D.tipodoctoid = 23
    INTO :DEVOLUCION;
   ...
   -- calculo del egreso
    select coalesce(sum(p.importe),0) from doctopago P
    WHERE P.CORTEID = :CORTEID AND P.FORMAPAGOID IN (1,2,3,5,14) AND  P.TIPOPAGOID = 2
    INTO :EGRESO;
