...
BEGIN
   ...
   if(not(:TIPODOCTOIDENTRADA  in (11,12,13,15,60,61) or :TIPODOCTOIDSALIDA in (11,12,13,15,60,61))) then
   begin
     exit;
   end
    ...
    END
     ...
     BEGIN
       ...
       begin
        ...
        if (:TIPODOCTOIDENTRADA IN (12,13) AND NEW.TIPOPAGOID = 1) THEN
        BEGIN
           UPDATE PERSONA SET SALDO = coalesce(SALDO,0) + (NEW.IMPORTE * :SENTIDO) WHERE ID = :PERSONAIDENTRADA;
        END
        ELSE if (:TIPODOCTOIDSALIDA in (11,15,60)  AND NEW.TIPOPAGOID = 2) then
        BEGIN
