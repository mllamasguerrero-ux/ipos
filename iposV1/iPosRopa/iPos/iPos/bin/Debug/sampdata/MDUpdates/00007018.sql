...
returns (
    ...
    PESOBRUTOVEHICULAR D_PESOVEHICULAR)
as
...
BEGIN
   ...
   SELECT
        ...
        DOCTO.USARDATOSENTREGA , DOCTO.DOM_ENTREGAID
    FROM DOCTO
    ...
    INTO      
       ...
       :USARDATOSENTREGA, :DOM_ENTREGAID ;
  ...
  END
   ...
   BEGIN
    ...
    INTO
        ...
        :CODIGOPOSTAL  ;
   END
   ELSE IF(COALESCE(:USARDATOSENTREGA,'N') <> 'N' AND COALESCE(:DOM_ENTREGAID, 0) <> 0 ) THEN
   BEGIN
    ...
    SELECT
        ...
        '' TIPOESTACION ,
        CAST(cast(COALESCE(DOMICILIO.DISTANCIA, PERSONA.entrega_distancia) as NUMERIC(18,3)) AS d_stdtext_medium) DISTANCIARECORRIDA ,
    ...
    FROM DOCTO
        LEFT JOIN PERSONA ON DOCTO.PERSONAID = PERSONA.id
        LEFT JOIN DOMICILIO ON DOMICILIO.id = DOCTO.DOM_ENTREGAID
        LEFT JOIN GUIA ON DOCTO.guiaid = GUIA.id 
        LEFT join SAT_COLONIA on SAT_COLONIA.id = DOMICILIO.sat_coloniaid
        LEFT join SAT_LOCALIDAD on SAT_LOCALIDAD.id = DOMICILIO.sat_localidadid
        LEFT join SAT_MUNICIPIO on SAT_MUNICIPIO.id = DOMICILIO.sat_municipioid
        LEFT join ESTADO on ESTADO.CLAVE = DOMICILIO.estado
        WHERE DOCTO.ID = :DOCTOID
    INTO
        :TIPOUBICACION ,
        :IDUBICACION   ,
   ...
   BEGIN
    ...
    SELECT
        ...
        '' TIPOESTACION ,
        CAST(cast(PERSONA.entrega_distancia as NUMERIC(18,2)) AS d_stdtext_medium) DISTANCIARECORRIDA ,
        PERSONA.domicilio CALLE,
        PERSONA.NUMEROEXTERIOR NUMEROEXTERIOR ,
        PERSONA.NUMEROINTERIOR NUMEROINTERIOR ,
        SAT_COLONIA.colonia COLONIA  ,
