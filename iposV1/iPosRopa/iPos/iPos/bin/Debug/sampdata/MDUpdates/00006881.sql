...
BEGIN
   ...
   SELECT FIRST 1 ID, ALMACENID FROM PERSONA WHERE VENDEDORID = :VENDEDORID INTO :USUARIOID,:ALMACENVENDEDORID;
   ...
   ALMACENDOCTOID = COALESCE( :ALMACENVENDEDORID, 1);
   IF(COALESCE(:PLAZOXPRODUCTO,'N') = 'S' AND COALESCE(:MANEJAALMACEN,'N') = 'S' AND
      (COALESCE(:DOCTOACTUALID,0) > 0)) THEN
   begin
            ...
            IF(COALESCE(:PLAZOPRODUCTOID,1) <> COALESCE(:PLAZODOCTOID,1)) THEN
            BEGIN
                DOCTOACTUALID = NULL;
                ...
                SELECT FIRST 1 ID FROM DOCTO WHERE TIPODOCTOID = 331 AND VENDEDORID = :VENDEDORID AND
                                                   ...
                                                   COALESCE(ESTATUSDOCTOID,0) = 0 AND COALESCE(PLAZOID,1) = COALESCE(:PLAZOPRODUCTOID,1)
                                                   INTO :DOCTOACTUALID;

            END
            
            PLAZODOCTOID = :PLAZOPRODUCTOID;
            ALMACENDOCTOID = COALESCE(:ALMACENPRODUCTOID,:ALMACENDOCTOID);
   END
   ELSE
   BEGIN 
