...
MOVTO.CANTIDAD, CASE WHEN PRODUCTO.descripcion_comodin = 'S' AND
                         ...
                         CASE WHEN MOVTO.TASAIMPUESTO IS NULL or COALESCE(PARAMETRO.precionetoengrids, 'N') = 'N'  THEN
                         ...
                         (case when coalesce(movto.estadorebaja,0) <> 0 then movto.totalconrebaja
                                                                        else case when COALESCE(PARAMETRO.precionetoengrids, 'N') = 'N' then MOVTO.SUBTOTAL else MOVTO.TOTAL end end   ) /
