...
BEGIN
   ...
   BEGIN
      FOR
            ...
            coalesce((p.importe *
          (CASE WHEN ((TIPOPAGOID = 1 AND p.FORMAPAGOID IN (1,2,3,4,5,6,14,15,16)) or (TIPOPAGOID = 2 AND p.FORMAPAGOID IN (7)))  THEN 1
   ...
   BEGIN
      FOR
            ...
            case when fs.nombre is null or p.formapagosatid = 99  then f.nombre else fs.nombre end  as formapago,
            coalesce((p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND p.FORMAPAGOID IN (1,2,3,4,5,7,14,15,16)) or (TIPOPAGOID = 1 AND p.FORMAPAGOID IN (6)))  THEN 1
                when ((TIPOPAGOID = 1 AND p.FORMAPAGOID IN (7)) or (TIPOPAGOID = 2 AND p.FORMAPAGOID IN (6)))  THEN -1
                ELSE 0 END)
