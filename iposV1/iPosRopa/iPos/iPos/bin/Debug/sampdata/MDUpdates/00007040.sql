...
returns (
    ...
    MESES D_STDTEXT_64,
    ANIO D_STDTEXT_64,
    SALDOVENTAPRINCIPAL D_PRECIO,
    MAXFORMAPAGOSATCLAVE D_CLAVE_NULL)
as
...
declare variable USARDATOSENTREGA D_BOOLCN_I;
declare variable DOM_ENTREGAID D_FK;
BEGIN
   ...
   SELECT PERSONAID, TIPODOCTOID, PAGOACREDITO,  TIMBRADOFORMAPAGOSAT , SUBTIPODOCTOID , DOCTOREFID, FECHAINI, FECHAFIN, USARDATOSENTREGA, DOM_ENTREGAID
    FROM docto WHERE ID = :DOCTOID
    INTO :PERSONAID, :TIPODOCTOID, :PAGOACREDITO, :PRE_FORMAPAGOSAT, :SUBTIPODOCTOID, :DOCTOREFID, :FECHAINI, :FECHAFIN, :USARDATOSENTREGA, :DOM_ENTREGAID;
   ...
   BEGIN
        ...
        SELECT SALDO FROM DOCTO
        WHERE ((:TIPODOCTOID = 21 AND ID = :DOCTOID) or (:TIPODOCTOID = 22 AND ID = :DOCTOREFID))
        INTO :SALDOVENTAPRINCIPAL;
        ...
        SELECT COUNT(*) FROM doctopago
         ...
         SELECT FIRST 1 FORMAPAGOSATCLAVE
         FROM
         (SELECT SUM(COALESCE(IMPORTE,0)) SUMAIMPORTE,
                COALESCE(DOCTOPAGO.formapagosatid,0) FORMAPAGOSATID,
                FORMAPAGOSAT.clave FORMAPAGOSATCLAVE
         FROM doctopago
         LEFT JOIN FORMAPAGOSAT ON DOCTOPAGO.formapagosatid = FORMAPAGOSAT.ID
         WHERE ((:TIPODOCTOID = 21 AND DOCTOID = :DOCTOID) or (:TIPODOCTOID = 22 AND DOCTOID = :DOCTOREFID)  )
                    AND COALESCE(DOCTOPAGO.formapagosatid,0) <> 0 AND
                    COALESCE(DOCTOPAGO.revertido, 'N') = 'N'  AND DOCTOPAGO.formapagoid IN (1,2,3,5,15)
          GROUP BY COALESCE(DOCTOPAGO.formapagosatid,0) , FORMAPAGOSAT.clave
          ORDER BY  SUM(COALESCE(IMPORTE,0)) desc  ) T
          INTO :MAXFORMAPAGOSATCLAVE;
        ...
        BEGIN      
            ...
            BEGIN
                IF( COALESCE(:SALDOVENTAPRINCIPAL ,0 ) > 1 or COALESCE(:MAXFORMAPAGOSATCLAVE,'') = '' ) THEN
                BEGIN
                    FORMAPAGO = '15';
                END
                ELSE
                BEGIN
                    FORMAPAGO = COALESCE(:MAXFORMAPAGOSATCLAVE,'');
                END
            END
            ELSE
            BEGIN   
   ...
   TIPOLINEA = 'EMISOR';
     SELECT PARAMETRO.FISCALCALLE, PARAMETRO.FISCALNUMEROEXTERIOR,
                ...
                PARAMETRO.FISCALMUNICIPIO, ESTADO.acronimo AS FISCALESTADO, 'MEX',
                PARAMETRO.FISCALCODIGOPOSTAL,PARAMETRO.RFC, PARAMETRO.FISCALNOMBRE, SAT_REGIMENFISCAL.sat_CLAVE , PARAMETRO.FISCALNOMBRE  , :EMPRESA_FISCALCODIGOPOSTAL
                FROM PARAMETRO
                    LEFT JOIN SAT_REGIMENFISCAL ON PARAMETRO.sat_regimenfiscalid = SAT_REGIMENFISCAL.ID
                    LEFT JOIN ESTADO ON TRIM(ESTADO.NOMBRE) = TRIM(PARAMETRO.FISCALESTADO)
                INTO :CALLE, :NOEXTERIOR, :NOINTERIOR, :COLONIA, :LOCALIDAD, :REFERENCIA, :MUNICIPIO,
                    :ESTADO, :PAIS, :CODIGOPOSTAL, :RFC, :RAZONSOCIAL, :REGIMENFISCAL, :NOMBRE, :DOMICILIOFISCAL;
      SUSPEND;
   ...
   BEGIN  
     SELECT PARAMETRO.CALLE, PARAMETRO.NUMEROEXTERIOR,
                ...
                PARAMETRO.LOCALIDAD, ESTADO.acronimo AS FISCALESTADO, 'MEX',
   ...
   END
     ...
     BEGIN
        ...
        SELECT  SRF.sat_clave , p.codigopostal
        FROM PERSONA P
         LEFT JOIN sat_regimenfiscal SRF on P.sat_regimenfiscalid = SRF.ID
         WHERE P.ID = :PERSONAID
         INTO  :REGIMENFISCAL, :DOMICILIOFISCAL;

        IF(COALESCE(:PREGUNTARDATOSENTREGA,'N') = 'N' or coalesce(:USARDATOSENTREGA,'N') <> 'S'  or COALESCE(:DOM_ENTREGAID, 0) = 0 ) THEN
        BEGIN
            SELECT PERSONA.DOMICILIO, PERSONA.NUMEROEXTERIOR,
                ...
                PERSONA.CIUDAD, ESTADO.acronimo ESTADO, 'MEX',
                PERSONA.CODIGOPOSTAL,PERSONA.RFC, PERSONA.nombres || ( CASE when coalesce(PERSONA.apellidos,'') = '' THEN '' ELSE  ' ' || coalesce(PERSONA.apellidos,'') END), '',
                PERSONA.nombres || ( CASE when coalesce(PERSONA.apellidos,'') = '' THEN '' ELSE  ' ' || coalesce(PERSONA.apellidos,'') END) ,''
                FROM PERSONA           
                    LEFT JOIN ESTADO ON TRIM(ESTADO.NOMBRE) = TRIM(PERSONA.ESTADO)
         ...
         ELSE
         BEGIN
            SELECT DOMICILIO.CALLE, DOMICILIO.NUMEROEXTERIOR,
                DOMICILIO.NUMEROINTERIOR,
                DOMICILIO.COLONIA,
                COALESCE(DOMICILIO.referenciadom, '.') REFERENCIA,
                DOMICILIO.MUNICIPIO, DOMICILIO.ESTADO, 'MEX',
                DOMICILIO.CODIGOPOSTAL,
                PERSONA.RFC,
                PERSONA.nombres || ( CASE when coalesce(PERSONA.apellidos,'') = '' THEN '' ELSE  ' ' || coalesce(PERSONA.apellidos,'') END), '',
                PERSONA.nombres || ( CASE when coalesce(PERSONA.apellidos,'') = '' THEN '' ELSE  ' ' || coalesce(PERSONA.apellidos,'') END) ,''
                FROM DOMICILIO
                INNER JOIN PERSONA ON PERSONA.ID = DOMICILIO.PERSONAID
                WHERE DOMICILIO.ID = :DOM_ENTREGAID
     ...
     BEGIN
        ...
        IF(COALESCE(:VERSIONCFDI,'4.0') = '4.0'  ) THEN
        BEGIN    
            USOCFDI = 'S01';
        END
        ELSE 
