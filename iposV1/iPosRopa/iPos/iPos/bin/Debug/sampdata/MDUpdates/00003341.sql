create or alter procedure CONSOLIDADO_DETALLE (
    FACTCONSID D_FK
    )
returns (
    ID D_FK,
    ...
    WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
        ...
        UNION
         ...
         SELECT
            ...
            0.0 AS SALDO,
            SUM(COALESCE(DOCTOPAGO.importe,0) * (CASE WHEN DOCTOPAGO.TIPOPAGOID = 1 THEN -1 WHEN DOCTOPAGO.TIPOPAGOID = 2 THEN 1 ELSE 0 END)) ABONO,
        ...
        WHERE
           (DOCTO.FECHA >= :FECHAINICIAL) AND
            ...
            --((:OMITIRVENTASAFRANQUICIAS = 'N' OR DOCTO.SUBTIPODOCTOID IS NULL OR  DOCTO.SUBTIPODOCTOID NOT IN (7, 8))) AND
        ...
        FROM    DOCTO DOCTODEV LEFT join
                DOCTOPAGO ON DOCTOPAGO.DOCTOSALIDAID = DOCTODEV.id AND DOCTOPAGO.formapagoid IN (1,2,3,5,15)
                LEFT JOIN
                TIPODOCTO ON TIPODOCTO.ID = DOCTODEV.TIPODOCTOID
