      ...
      FOR
       ...
       SELECT ID, CLAVE, LOCALIDAD, PRODUCTOID, PRODUCTONOMBRE, DESCRIPCION, DESCRIPCION1, PRODUCTOCLAVE, PZACAJA
        ...
        SELECT
           ...
           WHERE  ((ANAQUEL.CLAVE >= :ANAQUELINICIAL AND ANAQUEL.CLAVE <= :ANAQUELFINAL) OR :TODOSLOSPRODUCTOS = 1)
       ...
       BEGIN
        NUMERO = :NUMERO + 1;
           ...
           IF(COALESCE(:ANAQUELRES,'') <> 'Libre' OR COALESCE(:CANTIDADTEORICA,0 ) > 0) THEN
           BEGIN 
               SUSPEND;
           END
