...
MOVTO.CANTIDAD, CASE WHEN PRODUCTO.descripcion_comodin = 'S' AND
                         ...
                         CASE WHEN MOVTO.eskit = 'S' AND COALESCE(MOVTO.CANTIDAD,0.0) > 0.0 then CAST(MOVTO.TOTAL/MOVTO.cantidad  AS D_PRECIO)
                             WHEN MOVTO.TASAIMPUESTO IS NULL or COALESCE(PARAMETRO.precionetoengrids, 'N') = 'N'  THEN
                                (case when coalesce(movto.estadorebaja,0) not in (0,3) AND coalesce(parametro.versiontopeid,1) <> 2 then movto.precioconrebaja
                         ...
                         (case when coalesce(movto.estadorebaja,0) not in (0,3) AND coalesce(parametro.versiontopeid,1) <> 2  then movto.totalconrebaja
                         ...
                         (case when coalesce(movto.estadorebaja,0) not in (0,3) AND coalesce(parametro.versiontopeid,1) <> 2   then coalesce(movto.total,0) - coalesce(movto.totalconrebaja,0) + coalesce(movto.descuento,0)
                        ...
                        cast( ROUND((  cast((ROUND(   
                         ...
                         (case when coalesce(movto.estadorebaja,0) not in (0,3)  AND coalesce(parametro.versiontopeid,1) <> 2 and coalesce(movto.preciolista,0) <> 0 then (coalesce(movto.PRECIOLISTA,1) - coalesce(MOVTO.PRECIOCONREBAJA,1))/coalesce(movto.PRECIOLISTA,1) else MOVTO.DESCUENTOPORCENTAJE end)   as DESCUENTOPORCENTAJE ,
