...
BEGIN
   ...
   BEGIN
          ...
          select coalesce(sum(p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (7)) or (TIPOPAGOID = 1 AND FORMAPAGOID IN (6)))  THEN 1
          ...
          select coalesce(sum(p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (1,2,3,4,5,14)) )  THEN 1
                ELSE 0 END)
          ),0) from doctopago p inner join formapago f on p.formapagoid = f.id
   ...
   BEGIN
        select coalesce(sum(p.importe *
          (CASE WHEN ((TIPOPAGOID = 1 AND FORMAPAGOID IN (6)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (7)))  THEN 1
                when ((TIPOPAGOID = 1 AND FORMAPAGOID IN (7)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (6))) THEN -1
                ELSE 0 END)
          ),0) from doctopago p inner join formapago f on p.formapagoid = f.id
        ...
        select coalesce(sum(p.importe *
          (CASE WHEN ((TIPOPAGOID = 1 AND FORMAPAGOID IN (1,2,3,4,5,14)) )  THEN 1
                ELSE 0 END)
          ),0) from doctopago p inner join formapago f on p.formapagoid = f.id
