...
when PM.LISTAPRECIOXPZACAJA = 5 then  P.PRECIO5
      ...
      ((100.00 + COALESCE(PM.COMISIONPORTARJETA,0.00))/100.00) * (P.precio3 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000))) AS PRECIOMEDIOMAYOREOCONTARJETA,
CASE WHEN COALESCE(P.PRECIO6,0) > 0 THEN PRECIO6
           ELSE ((100.00 + COALESCE(PM.COMISIONPORTARJETA,0.00))/100.00) * (P.precio4 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000)))
