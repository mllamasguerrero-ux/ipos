        ...
        (SUM(COALESCE(M.CANTIDAD,0) * COALESCE(K.cantidadparte,0))) - sum(COALESCE(INV.cantidad,0) ) FALTANTE
         ...
         FROM MOVTO M
           ...
           WHERE M.DOCTOID = :DOCTOID
              AND (M.CANTIDAD >0) AND COALESCE(PKIT.eskit,'N') = 'S'  AND COALESCE(PPARTE.inventariable,'N') = 'S' AND COALESCE(PPARTE.negativos,'N') = 'N' AND COALESCE(PKIT.INVENTARIABLE,'N') = 'S'
