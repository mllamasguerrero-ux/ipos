       ...
       CORE.IMPORTEENDOLAR,
        ...
        case
            ...
            when  ( coalesce(producto.pzacaja,0) = 0 or  coalesce(producto.pzacaja,0) = 1 ) then
                ...
                CAST(cast(trunc(coalesce(     (CASE WHEN (docto.tipodoctoid in  (41)) THEN core.cantidadsurtida ELSE core.cantidad END   )   ,0)/coalesce(producto.pzacaja,1)) as INTEGER) as varchar(20)) || ' CAJ ' ||  CAST(cast(mod(coalesce(    (CASE WHEN (docto.tipodoctoid in  (41)) THEN core.cantidadsurtida ELSE core.cantidad END )   ,0),coalesce(producto.pzacaja,1)) as integer) AS VARCHAR(10)) || ' PZA'
        end 
         ...
         (SELECT * FROM RECIBOS_CORE WHERE DOCTOID = :IDDOCTO) CORE
          LEFT JOIN DOCTO ON ID = CORE.DOCTOID
            left join MOVTO ON MOVTO.DOCTOID = CORE.DOCTOID AND MOVTO.id = CORE.MOVTOID
              ...
              AND (COALESCE(:CREADOPOR_USERID ,0 ) = 0 or COALESCE(:CREADOPOR_USERID ,0 ) = MOVTO.creadopor_userid )
        order by
