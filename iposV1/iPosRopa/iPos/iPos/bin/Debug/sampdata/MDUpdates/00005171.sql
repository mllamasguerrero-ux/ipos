create or alter procedure KIT_REFERENCIA_N2 (
    PRODUCTOID D_FK,
    CANTIDAD D_CANTIDAD,
    TOTAL D_PRECIO,
    SUBTOTAL D_PRECIO,
...
returns (
    ...
    EXTKITSUBTOTAL D_DECIMAL_EXACT,
...
BEGIN
   ...
   SELECT ESKIT FROM PRODUCTO WHERE ID = :PRODUCTOID INTO :ESKIT;
   ...
   SELECT PARAMETRO.desgloseieps FROM PARAMETRO INTO :DESGLOSEIEPS;
      ...
      SELECT TASAIEPSEXT , TASAIVAEXT FROM PRODUCTO WHERE ID = :PRODUCTOID INTO :TASAIEPSEXT , :TASAIVAEXT;


        TOTALPRECALCULADO = round((:SUBTOTAL  * cast( (1.0000 + ( (CASE WHEN COALESCE(:desgloseieps,'N') = 'S' THEN COALESCE(:TASAIEPSEXT,0) ELSE 0.0000 END) /100.00)) * (1.00 + (COALESCE(:TASAIVAEXT,0)/100.00)) as numeric(18,8))),2) ;
