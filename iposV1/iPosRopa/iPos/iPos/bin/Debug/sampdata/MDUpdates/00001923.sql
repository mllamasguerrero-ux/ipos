...
MOVTO.CANTIDAD, CASE WHEN PRODUCTO.descripcion_comodin = 'S' AND
                        ...
                        cast( ROUND((  cast((ROUND( ROUND((MOVTO.PRECIO * ((100 - PARAMETRO.DESCUENTOVALE)/100.00)),2) * MOVTO.CANTIDAD , 2)) as d_importe) * (COALESCE (MOVTO.tasaieps,0)/100.00)) ,2 )  as d_importe)
                         ...
                         movto.tasaimpuesto   ,
                         CASE WHEN COALESCE(PARAMETRO.esvendedormovil,'N') <> 'S' then  'NINGUNO'
                         ELSE CASE WHEN COALESCE(PRODUCTO.MARGENMOVIL,0) - COALESCE(PRODUCTO.mprecio4movil,0) <= 2 THEN  'ROJO'
                                   WHEN COALESCE(PRODUCTO.MARGENMOVIL,0) - COALESCE(PRODUCTO.mprecio4movil,0) > 2 and
                                        COALESCE(PRODUCTO.MARGENMOVIL,0) - COALESCE(PRODUCTO.mprecio4movil,0) <= 3 THEN 'AMARILLO'
                                        ELSE '' END END  ALERTA
