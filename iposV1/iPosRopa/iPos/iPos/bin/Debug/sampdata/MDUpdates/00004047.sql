...
BEGIN
   ...
   LISTAPRECIOXUEMPAQUE, LISTAPRECIOXPZACAJA , LISTAPRECIOINIMAYO, APLICARMAYOREOPORLINEA, PRECIOSORDENADOS
   FROM parametro
   ...
   IF (:LISTAPRECIOCLIENTE = '2' AND (:PRECIO > :PRECIO2 OR COALESCE(:FORZARPRECIOLISTA,'N') = 'S') ) THEN
      PRECIO = :PRECIO2;
   ELSE IF (:LISTAPRECIOCLIENTE = '3' AND (:PRECIO > :PRECIO3  OR COALESCE(:FORZARPRECIOLISTA,'N') = 'S') ) THEN
      PRECIO = :PRECIO3;
   ELSE IF (:LISTAPRECIOCLIENTE = '4' AND (:PRECIO > :PRECIO4  OR COALESCE(:FORZARPRECIOLISTA,'N') = 'S') ) THEN
      PRECIO = :PRECIO4;
   ELSE IF (:LISTAPRECIOCLIENTE = '5' AND (:PRECIO > :PRECIO5  OR COALESCE(:FORZARPRECIOLISTA,'N') = 'S') ) THEN
      PRECIO = :PRECIO5;
   ...
   END
   ELSE IF(COALESCE(:PAGOCONTARJETA,'N') = 'S') THEN
   BEGIN              
   ...
   IF(coalesce(:APLICARMAYOREOPORLINEA,'N') = 'S' AND COALESCE(:PAGOCONTARJETA,'N') = 'N' AND COALESCE(:PAGOACREDITO,'N') = 'N') THEN
   BEGIN
       HABPROMOCIONBODEGAZO = 'S';
       
     insert into traza(valor) values('ttb' || :HABPROMOCIONBODEGAZO);
   END
