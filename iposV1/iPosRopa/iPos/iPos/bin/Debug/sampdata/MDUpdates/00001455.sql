...
BEGIN
      ...
      SELECT COALESCE(SUM(COALESCE(case when d.tipodoctoid = 201 then D.SALDO * -1 ELSE D.SALDO end ,0)) ,0)
      FROM DOCTO D
      WHERE D.tipodoctoid IN (22,11,15,201)
      AND COALESCE(case when d.tipodoctoid = 201 then D.SALDO * -1 ELSE D.SALDO end ,0) > 0
      ...
      FROM DOCTO D
      WHERE (D.tipodoctoid IN (21,24,12,202)  /*or (D.tipodoctoid IN (25) and d.mercanciaentregada = 'S') */ )
