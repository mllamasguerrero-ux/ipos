  ...
  FOR SELECT
    ...
    BEGIN
      ...
      FOR SELECT
                                    ...
                                    TasaCuota ,
                                    TipoFactor ,
                                    CASE WHEN COALESCE(TASA,0) > 0 THEN CAST(ROUND(SUM(COALESCE(IMPORTE,0.00)) / (COALESCE(TASA,0.00) / 100.00),2) AS D_PRECIO) ELSE  SUM(COALESCE(BaseImp,0.00)) END  BASEIMP,
                                    PADRELINEA
          FROM
      ...
      FOR SELECT
                                    ...
                                    TasaCuota ,
                                    TipoFactor ,
                                    SUM(COALESCE(BaseImp,0.00))  BASEIMP
          FROM
              ...
              (
                SELECT
                                    MIN(ID) ID,
                                    TIPOLINEA ,
                                    MIN(ORDENLINEA) ORDENLINEA,
                                    SUM(COALESCE(IMPORTE,0)) IMPORTE ,
                                    IMPUESTO,
                                    Tasa ,
                                    TasaCuota ,
                                    TipoFactor ,
