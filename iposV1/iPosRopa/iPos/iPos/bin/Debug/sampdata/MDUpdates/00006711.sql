...
BEGIN
  ...
  ERRORCODE = 0;
   ...
   SELECT TIPODOCTOID, ESTATUSDOCTOID, KITDESGLOSADO , ACTIVO
   FROM DOCTO
   WHERE ID = :DOCTOID
   INTO :TIPODOCTOID, :ESTATUSDOCTOID, :KITDESGLOSADO, :ACTIVO;
   ...
   IF ( COALESCE(:ACTIVO,'S') = 'S' AND COALESCE(:KITDESGLOSADO,'N') = 'S') THEN
   BEGIN

        SELECT ERRORCODE FROM INVKIT_DESENSAMBLETEMP (:DOCTOID ,:VENDEDORID ) INTO :ERRORCODE;  
   ...
   BEGIN
        ...
        IF(COALESCE(:ACTIVO,'S') = 'S' ) THEN
        BEGIN
        
            -- Procesar sobrantes.
            SELECT ERRORCODE 
            FROM INVFIS_DIF_GENERAR(:DOCTOID, 51, :VENDEDORID)
            INTO :ERRORCODE;
            ...
            -- Procesar faltantes.
            SELECT ERRORCODE 
            FROM INVFIS_DIF_GENERAR(:DOCTOID, 52, :VENDEDORID)
            INTO :ERRORCODE;
   ...
   BEGIN 
        ...
        BEGIN               
            -- Procesar faltantes.
            SELECT ERRORCODE 
            FROM INVFIS_DIF_GENERAR_XLOC(:DOCTOID, 52, :VENDEDORID)
            INTO :ERRORCODE;
            ...
            -- Procesar sobrantes.
            SELECT ERRORCODE 
            FROM INVFIS_DIF_GENERAR_XLOC(:DOCTOID, 51, :VENDEDORID)
            INTO :ERRORCODE;
   ...
   BEGIN
        ...
        SELECT ERRORCODE FROM INVKIT_ENSAMBLETEMP (:DOCTOID ,:VENDEDORID ) INTO :ERRORCODE;
        IF(COALESCE(:ERRORCODE,0) <> 0) THEN
        BEGIN
            SUSPEND;
            EXIT;
