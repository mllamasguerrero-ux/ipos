...
declare variable NOMBANCOORDEXT D_STDTEXT_64;
...
BEGIN
    ...
    FOR SELECT 
        ...
        BEGIN
         ...
         END  
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'INFORMACIONADUANERA') THEN
         BEGIN   
            SELECT ID FROM CFDI_CONC
            WHERE CFDIID = :CFDIID AND CONSECUTIVO = :PADRELINEA
            INTO :CFDI_CONC_ID;
         ...
         END
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'CONCEPTO_IVA') THEN
         BEGIN  
            SELECT ID FROM CFDI_CONC
            WHERE CFDIID = :CFDIID AND CONSECUTIVO = :PADRELINEA
            INTO :CFDI_CONC_ID;
         ...
         END
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'COMPROBANTE_IVA') THEN
         BEGIN
         ...
         END
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'COMPROBANTE_IEPS') THEN
         BEGIN
         ...
         END
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'COMPROBANTE_IVARETENIDO') THEN
         BEGIN
         ...
         END  
         ELSE IF(COALESCE(:TIPOLINEA,'') = 'COMPROBANTE_ISRRETENIDO') THEN
         BEGIN
