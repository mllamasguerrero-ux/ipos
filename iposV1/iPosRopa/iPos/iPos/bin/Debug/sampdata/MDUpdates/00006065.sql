create or alter procedure CORTE_MONTOBILLETES_UPDATE (
    CORTEID D_FK,
    TIPODECAMBIO D_TIPOCAMBIO,
    P1000 D_CANTIDAD,
    P500 D_CANTIDAD,
    P200 D_CANTIDAD,
    P100 D_CANTIDAD,
    P50 D_CANTIDAD,
    P20 D_CANTIDAD,
    D100 D_CANTIDAD,
    D50 D_CANTIDAD,
    D20 D_CANTIDAD,
    D10 D_CANTIDAD,
    D5 D_CANTIDAD,
    D2 D_CANTIDAD,
    D1 D_CANTIDAD,
    MORRALLAPESOS D_IMPORTE,
    MORRALLADOLARES D_IMPORTE,
    MORRALLADEDOLARENPESOS D_IMPORTE,
    SALDOFINAL D_IMPORTE,
    CHEQUES D_IMPORTE,
    VALES D_IMPORTE,
    TARJETA D_IMPORTE,
    CREDITO D_IMPORTE,
    MONEDERO D_IMPORTE,
    TRANSFERENCIA D_IMPORTE,
    INDEFINIDO D_IMPORTE,
    TIPOMONTOBILLETESID D_FK,
    DOCTOID D_FK,
    DEPOSITO D_IMPORTE,
    DEPTERCERO D_IMPORTE,
    FONDODINAMICOFINAL D_IMPORTE)
returns (
    MONTO_BILLETES_ID D_FK,
    ERRORCODE D_ERRORCODE)
as
declare variable ACTIVO D_BOOLCN;
BEGIN


    if(COALESCE(:TIPOMONTOBILLETESID,1)  = 1) then
    BEGIN
    
        SELECT ACTIVO
        FROM CORTE
        WHERE ID = :CORTEID
        INTO :ACTIVO;

        IF (:CORTEID IS NULL) THEN
        BEGIN
            ERRORCODE = 1023;
            SUSPEND;
            EXIT;
        END

        /*IF (:ACTIVO <> 'S') THEN
        BEGIN
            ERRORCODE = 1024;
            SUSPEND;
            EXIT;
        END */


        SELECT FIRST 1 COALESCE(ID, 0)
        FROM montobilletes
        WHERE CORTEID = :CORTEID  AND coalesce(TIPOMONTOBILLETESID,1) = COALESCE(:TIPOMONTOBILLETESID,1)
        INTO :MONTO_BILLETES_ID;
    END
    ELSE if(COALESCE(:TIPOMONTOBILLETESID,1)  = 2 AND COALESCE(:DOCTOID,0) <> 0) then
    BEGIN
           
        SELECT FIRST 1 COALESCE(ID, 0)
        FROM montobilletes
        WHERE COALESCE(DOCTOID,0) = :DOCTOID   AND coalesce(TIPOMONTOBILLETESID,1) = COALESCE(:TIPOMONTOBILLETESID,1)
        INTO :MONTO_BILLETES_ID;


    END




   IF ((:MONTO_BILLETES_ID IS NULL) OR (:MONTO_BILLETES_ID = 0)) THEN
   BEGIN
       INSERT INTO MONTOBILLETES
       (
    CORTEID ,
    TIPODECAMBIO,
    P1000 ,
    P500 ,
    P200 ,
    P100 ,
    P50  ,
    P20  ,
    D100 ,
    D50  ,
    D20  ,
    D10  ,
    D5   ,
    D2   ,
    D1   ,
    MORRALLAPESOS ,
    MORRALLADOLARES ,
    MORRALLADEDOLARENPESOS ,
    SALDOFINAL ,
    CHEQUES,
    VALES   ,
    TARJETA ,
    CREDITO ,
    MONEDERO,
    TRANSFERENCIA,
    INDEFINIDO,
    TIPOMONTOBILLETESID,
    DOCTOID,
    DEPOSITO,
    DEPTERCERO)

    VALUES (
    :CORTEID ,
    :TIPODECAMBIO,
    :P1000 ,
    :P500 ,
    :P200 ,
    :P100 ,
    :P50  ,
    :P20  ,
    :D100 ,
    :D50  ,
    :D20  ,
    :D10  ,
    :D5   ,
    :D2   ,
    :D1   ,
    :MORRALLAPESOS ,
    :MORRALLADOLARES ,
    :MORRALLADEDOLARENPESOS ,
    :SALDOFINAL ,
    :CHEQUES,
    :VALES   ,
    :TARJETA,
    :CREDITO,
    :MONEDERO,
    :TRANSFERENCIA,
    :INDEFINIDO,
    :TIPOMONTOBILLETESID,
    :DOCTOID,
    :DEPOSITO,
    :DEPTERCERO)   RETURNING ID INTO :MONTO_BILLETES_ID ;
   END
   ELSE
   BEGIN

       UPDATE MONTOBILLETES
        SET CORTEID = :CORTEID,
            TIPODECAMBIO = :TIPODECAMBIO,
            P1000 = :P1000,
            P500 = :P500 ,
            P200 = :P200 ,
            P100 = :P100,
            P50  = :P50,
            P20  = :P20,
            D100 = :D100,
            D50  = :D50,
            D20  = :D20,
            D10  = :D10,
            D5   = :D5,
            D2   = :D2,
            D1   = :D1,
            MORRALLAPESOS = :MORRALLAPESOS,
            MORRALLADOLARES = :MORRALLADOLARES ,
            MORRALLADEDOLARENPESOS = :MORRALLADEDOLARENPESOS ,
            SALDOFINAL = :SALDOFINAL ,
            CHEQUES = :CHEQUES,
            VALES = :VALES  ,
            TARJETA = :TARJETA,
            CREDITO = :CREDITO,
            MONEDERO = :MONEDERO ,
            TRANSFERENCIA = :TRANSFERENCIA,
            INDEFINIDO = :INDEFINIDO,
            TIPOMONTOBILLETESID = :TIPOMONTOBILLETESID,
            DOCTOID = :DOCTOID ,
            DEPOSITO = :DEPOSITO,
            DEPTERCERO = :DEPTERCERO
            WHERE ID = :MONTO_BILLETES_ID;

   END


   UPDATE CORTE
   SET
      SALDOREAL  = :SALDOFINAL ,
      DIFERENCIA = coalesce(SALDOFINAL,0) - coalesce(:SALDOFINAL,0) - coalesce(SALDOREALCREDITO,0),
      FONDODINAMICOFINAL = :FONDODINAMICOFINAL
   WHERE ID = :CORTEID;


   ERRORCODE = 0;
   SUSPEND;

   WHEN ANY DO
   BEGIN
      ERRORCODE = 1025;
      SUSPEND;
   END 
END