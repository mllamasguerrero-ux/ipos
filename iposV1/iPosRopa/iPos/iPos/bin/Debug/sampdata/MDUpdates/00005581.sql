            ...
            SELECT
                ...
                COALESCE(SUBTOTAL,0) +
                  ...
                  CASE WHEN COALESCE(:MOSTRARDESCUENTOENFACTURA,'N') = 'S' THEN COALESCE(DOCTO.DESCUENTO,0) ELSE 0.0 END
                   SUBTOTAL ,
                   --COALESCE(DOCTO.DESCUENTO,0) DESCUENTO,
                CASE WHEN COALESCE(:MOSTRARDESCUENTOENFACTURA,'N') = 'S' THEN COALESCE(DESCUENTO,0.0) ELSE 0 END DESCUENTO,
                DOCTO.OMITIRVALES
                FROM DOCTO
                WHERE DOCTO.ID = :DOCTOID
     ...
     SUSPEND;
      ...
      FOR SELECT MOVTO.ID, MOVTO.CANTIDAD, SAT_UNIDADMEDIDA.sat_nombre,
                ...
                ROUND( (COALESCE(MOVTO.SUBTOTAL,0) +
                        ...
                        CASE WHEN COALESCE(:MOSTRARDESCUENTOENFACTURA,'N') = 'S' THEN COALESCE(MOVTO.DESCUENTO,0) ELSE 0.0 END)  / CASE WHEN COALESCE(MOVTO.cantidad, 1.0) > 0 THEN  COALESCE(MOVTO.cantidad, 1.0) ELSE 1 END
                     , 2) VALORUNITARIO ,
