...
declare variable TIPODOCTOREFID D_FK;
...
BEGIN
   ...
   -- Validar estatus.
   SELECT TIPODOCTOID, ESTATUSDOCTOID, REFERENCIAS, ESFACTURAELECTRONICA, ALMACENID
   FROM DOCTO
   WHERE ID = :DOCTOID
   INTO :TIPODOCTOID, :ESTATUSDOCTOID, :REFERENCIAS, :ESFACTURAELECTRONICA, :ALMACENID;
   ...
   --insert into traza(valor) values('Es franquicia ' || COALESCE(:ESFRANQUICIA,'N') );

   IF(COALESCE(:ESFRANQUICIA,'N') = 'S' or (COALESCE(:ESFACTURAELECTRONICA,'N') = 'S') ) THEN
   BEGIN
   ...
   SELECT DOCTO.DOCTOREFID  , DOCTOREF.TIPODOCTOID
   FROM DOCTO
   LEFT JOIN DOCTO DOCTOREF ON DOCTOREF.ID = DOCTO.DOCTOREFID
   WHERE DOCTO.ID = :DOCTOID
   INTO :DOCTOVENTAID, :TIPODOCTOREFID;




   IF(COALESCE(:DOCTOVENTAID,0) = 0 or COALESCE(:TIPODOCTOREFID,21) <> 21 ) THEN
   BEGIN

        --CUANDO EL TRASLADO TENGA ALGUNA REFERENCIA PRINCILA , PASARLA AL CAMPO REF ID2
        IF(COALESCE(:TIPODOCTOREFID,21) <> 21  AND COALESCE(:DOCTOVENTAID,0) <>  0) THEN
        BEGIN
          UPDATE DOCTO SET  DOCTOREFID2 = DOCTOREFID WHERE DOCTO.ID = :DOCTOID;
        END
        ...
        FOLIOSAT,SERIESAT,TIMBRADOFECHA, TIMBRADOUUID,TIMBRADOCERTSAT,ESAPARTADO,DOCTOREFID, IEPS, IMPUESTO, SUBTIPODOCTOID, TIMBRADOFORMAPAGOSAT, ESTADOSURTIDOID)
        SELECT
        ...
        FOLIOSAT,SERIESAT,TIMBRADOFECHA, TIMBRADOUUID,TIMBRADOCERTSAT , ESAPARTADO,ID, 0.00, 0.00 ,8  ,99 , ESTADOSURTIDOID
