...
BEGIN
   ...
   SUM(TOTAL ) - SUM(COALESCE(DESCUENTOCONVALE,0)) AS SUMATOTAL,
   ...
   SUM(DESCUENTO ) + SUM(COALESCE(DESCUENTOCONVALE,0)) AS SUMADESCUENTO ,
   ...
   SUM(COALESCE(DESCUENTOCONVALE,0)) SUMADESCUENTOCONVALE
   FROM
  CONSOLIDADO_PORCONSOLIDAR(:FECHAINICIAL, :FECHAFINAL, :OMITIRVENTASAFRANQUICIAS, :INCLUIRGASTOS, :MAXIMOMONTO, :OMITIRVALES)
