...
BEGIN
   ...
   BEGIN
      FOR
            ...
            coalesce((p.importe *
          (CASE WHEN ((TIPOPAGOID = 1 AND p.FORMAPAGOID IN (1,2,3,4,5,6,14,15,16,17,18,20,21)) or (TIPOPAGOID = 2 AND p.FORMAPAGOID IN (7,19)))  THEN 1
          ...
          order by p.id
      INTO
            :ID, :FECHA, :FORMADEPAGO, :MONTO,  :REFERENCIABANCARIA  ,:BANCO , :FORMAPAGOID, :TIPOABONODESCRIPCION  , :TIPOABONOID  , :REVERTIDO  , :FOLIOREF, :FECHAELABORACION, :FECHARECEPCION  , :NOMBREVENDEDOR , :FORMAPAGOSATID  , :RECIBOELECTRONICOID , :ESCONPINPADBANCOMER ,:BANCOMERPARAMID  , :COMISION , :NOTAS, :APLICADO, :FECHAAPLICADO
   ...
   BEGIN
      FOR
            ...
            case when fs.nombre is null or p.formapagosatid = 99  or p.formapagoid in (20,21) then f.nombre else fs.nombre end  as formapago,
            coalesce((p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND p.FORMAPAGOID IN (1,2,3,4,5,7,14,15,16,17,19,20,21)) or (TIPOPAGOID = 1 AND p.FORMAPAGOID IN (6,18)))  THEN 1
                when ((TIPOPAGOID = 1 AND p.FORMAPAGOID IN (7,19)) or (TIPOPAGOID = 2 AND p.FORMAPAGOID IN (6,18)))  THEN -1
                ELSE 0 END)
          ...
          order by p.id
      INTO
            :ID, :FECHA, :FORMADEPAGO, :MONTO, :BANCO, :REFERENCIABANCARIA, :FORMAPAGOID, :TIPOABONODESCRIPCION   , :TIPOABONOID , :REVERTIDO , :FOLIOREF , :FECHAELABORACION, :FECHARECEPCION , :FORMAPAGOSATID , :RECIBOELECTRONICOID   , :ESCONPINPADBANCOMER ,:BANCOMERPARAMID , :COMISION, :NOTAS, :APLICADO, :FECHAAPLICADO
      DO
      BEGIN
