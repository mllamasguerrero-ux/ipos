...
BEGIN
   ...
   BEGIN
     UPDATE MOVTO SET PRECIOCONREBAJA = PRECIO, TOTALCONREBAJA = TOTAL, ESTADOREBAJA = 0
      WHERE DOCTOID = :DOCTOPROMOVERREBAJAID AND COALESCE(ESTADOREBAJA,0) = 0;


      SELECT SUM(COALESCE(TOTALCONREBAJA,0)) FROM MOVTO WHERE ID = :DOCTOPROMOVERREBAJAID INTO  :SUMATOTALCONREBAJA;

      UPDATE DOCTO SET TOTALCONREBAJA = :SUMATOTALCONREBAJA  WHERE ID = :DOCTOPROMOVERREBAJAID;
