...
BEGIN
   ...
   if(coalesce(:TIPODOCTOIDENTRADA,0)  in (11,12,13,16,18) or coalesce(:TIPODOCTOIDSALIDA,0) in (11,12,13,16,18)) then
   begin
     exit;
   end
   ...
   begin
       ...
       BEGIN
          ...
          (CASE WHEN ((TIPOPAGOID = 1 AND FORMAPAGOID IN (1,2,3,4,5,6,9,14,15,16,17,18,20,21)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (7,19/*,8*/)))  THEN 1
                when ((TIPOPAGOID = 1 AND FORMAPAGOID IN (7,19/*,8*/)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (6,8,18/*9*/))) THEN -1
                ELSE 0 END)
          ),0) ABONO,
       ...
       BEGIN
          select coalesce(sum(p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (1,2,3,4,5,7/*,8*/,14,15,16,17,19,20,21)) or (TIPOPAGOID = 1 AND FORMAPAGOID IN (6,9,18)))  THEN 1
                when ((TIPOPAGOID = 1 AND FORMAPAGOID IN (7,19/*,8*/)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (6,8/*9*/,18)))  THEN -1
                ELSE 0 END)
          ),0)  ABONO,
          coalesce(sum(p.importe *
