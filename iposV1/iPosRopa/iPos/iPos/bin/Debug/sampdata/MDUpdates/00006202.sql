...
into :COUNTEXISTE;
    ...
    BEGIN
        ...
        select 'Z'  tipo,
        ('V' || LPAD(coalesce(cast(docto.folio as varchar(20)),''),5,'0')
        || case when coalesce(sucursal.esmatriz,'N') <> 'S' or
                     (coalesce(sucursal.surtidor,'') <> ''  and coalesce(sucursaldest.surtidor,'') <> coalesce(sucursal.clave,'')) then
                      '_' || sucursal.clave
                  else
                      ''
            end
        ...
        left join sucursal sucursaldest on sucursaldest.id = docto.sucursaltid
