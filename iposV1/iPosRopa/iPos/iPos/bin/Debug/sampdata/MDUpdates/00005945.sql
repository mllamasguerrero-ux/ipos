...
begin
  ...
  IF( NOT (NEW.ESTATUSDOCTOID = 1 AND NEW.TIPODOCTOID IN (11,12) AND COALESCE(NEW.SUBTIPODOCTOID,0) IN (0,16,23,7))) THEN
  BEGIN
    EXIT;
  END
  ...
  BEGIN
    ...
    IF(NEW.ESTATUSDOCTOID = 1 AND NEW.TIPODOCTOID = 11 AND COALESCE(NEW.SUBTIPODOCTOID,0) IN (0,16,23,7)) THEN
    BEGIN
    ...
    IF(NEW.ESTATUSDOCTOID = 1 AND NEW.TIPODOCTOID = 12 AND COALESCE(NEW.SUBTIPODOCTOID,0) IN (0,16,23,7)) THEN
    BEGIN
        UPDATE OR INSERT INTO REPLEXPOCTRL
        (DESTINOSUCURSALCLAVE,  FUENTEID,  FECHAHORASYNC, TIPOID,  ESTATUS)
