...
BEGIN
     ...
     insert into traza(valor) values('ttA' || :HABPROMOCIONBODEGAZO);
   ...
   BEGIN
        ...
        BEGIN
               PRECIOBUFFER =  (:COSTO * :CANTIDAD * (1.00 + (COALESCE(:PORCPROMO,0)/100.00)))/:CANTIDAD;

               
                insert into traza(valor) values('ttc' );

                IF( :PRECIOBUFFER < :PRECIOACTUAL) THEN
                BEGIN
                    
                insert into traza(valor) values('ttd' );
