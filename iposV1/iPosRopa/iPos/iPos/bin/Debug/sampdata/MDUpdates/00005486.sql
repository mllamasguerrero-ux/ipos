...
BEGIN
   ...
   SELECT ESTATUSDOCTOID, TIPODOCTOID, SUBTIPODOCTOID  , FACTCONSID , DEVCONSID  , TIMBRADOUUID
   FROM DOCTO
   WHERE ID = :DOCTOCANCELARID
   INTO :ESTATUSDOCTOID, :TIPODOCTOCANCELARID, :SUBTIPODOCTOID, :DOCTOFACTCONSID, :DOCTODEVCONSID, :TIMBRADOUUID;
   ...
   -- Si no esta vigente: Salir.
   IF (COALESCE(:DOCTOFACTCONSID,0) <> 0 or (COALESCE(:DOCTODEVCONSID,0) <> 0 and COALESCE(:TIMBRADOUUID,'') <> '')  ) THEN
   BEGIN
      ERRORCODE = 5009;
      SUSPEND;
      EXIT;
   ...
   FOR SELECT
      ...
      TIPODIFERENCIAINVENTARIOID  , CANTIDADDEFACTURA, CANTIDADDEREMISION, CANTIDADDEINDEFINIDO , DESCRIPCION1, DESCRIPCION2, DESCRIPCION3  , MOVTOREFID, LOTEIMPORTADO
   FROM MOVTO
   ...
   BEGIN
      ...
      --asegurate de que se pasen las cantidades de facturado y remisionado
         UPDATE MOVTO  SET  CANTIDADDEFACTURA = :CANTIDADDEFACTURA  ,
           ...
           UPDATE MOVTO SET ESTADOREBAJA = 1 WHERE ID = :MOVTOREFID and ESTADOREBAJA = 2;
