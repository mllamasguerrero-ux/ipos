...
declare variable MAYOREOXPRODUCTO D_BOOLCN;
declare variable LISTAPRECMEDIOMAYID D_FK;
declare variable LISTAPRECMAYOREOID D_FK;
declare variable CANTMEDIOMAY D_CANTIDAD;
declare variable CANTMAYOREO D_CANTIDAD;
declare variable LISTAPRECIOMAYLINEA D_FK;
declare variable LISTAPRECMEDMAYLINEA D_FK;     
declare variable PRECIOMAYLINEA D_PRECIO;
declare variable PRECIOMEDMAYLINEA D_PRECIO;
BEGIN
   ...
   SELECT CAST(COALESCE(LISTAPRECIOID, 1) AS CHAR(1)),SUPERLISTAPRECIOID, MAYOREOXPRODUCTO
   FROM PERSONA
   WHERE ID = :PERSONAID
   INTO :LISTAPRECIOCLIENTE,:SUPERLISTAPRECIOID, :MAYOREOXPRODUCTO;
   ...
   LISTAPRECIOXUEMPAQUE, LISTAPRECIOXPZACAJA , LISTAPRECIOINIMAYO, APLICARMAYOREOPORLINEA, PRECIOSORDENADOS , HABDESCLINEAPERSONA, PAGOTARJMENORPRECIOLISTAALL
   , COALESCE(LISTAPRECIOMAYLINEA,4), COALESCE(LISTAPRECMEDMAYLINEA ,3)
   FROM parametro
   ...
   --AJUSTE PARA AGREGAR CONFIGURACION POR PRODUCTO Y CLIENTE DE MAYOREO Y MEDIO MAYOREO
   IF(COALESCE(:MAYOREOXPRODUCTO,'N') = 'S') THEN
   BEGIN
        select  LISTAPRECMEDIOMAYID ,LISTAPRECMAYOREOID ,CANTMEDIOMAY , CANTMAYOREO
        from PRODUCTO
        where ID = :PRODUCTOID
        into :LISTAPRECMEDIOMAYID ,:LISTAPRECMAYOREOID ,:CANTMEDIOMAY , :CANTMAYOREO ;


        IF( COALESCE(:CANTMEDIOMAY,0) > 0 AND COALESCE(:LISTAPRECMEDIOMAYID, 0) > 0 AND COALESCE(:CANTIDAD, 0) >= COALESCE(:CANTMEDIOMAY,0) AND
            (COALESCE(:CANTMAYOREO,0) = 0 OR COALESCE(:CANTIDAD, 0) < COALESCE(:CANTMAYOREO,0) )
            ) THEN
        BEGIN
             LISTAPRECIOCLIENTE = CAST(:LISTAPRECMEDIOMAYID AS VARCHAR(1));
        END
        ELSE IF( COALESCE(:CANTMAYOREO,0) > 0 AND COALESCE(:CANTIDAD, 0) >= COALESCE(:CANTMAYOREO,0) AND COALESCE(:LISTAPRECMAYOREOID, 0) > 0 ) THEN
        BEGIN
             LISTAPRECIOCLIENTE = CAST(:LISTAPRECMAYOREOID AS VARCHAR(1));
        END   
        ELSE IF( ( COALESCE(:CANTMAYOREO,0) > 0 or COALESCE(:CANTMEDIOMAY,0) > 0 ) OR
                  ( COALESCE(:LISTAPRECMEDIOMAYID,0) > 0 or COALESCE(:LISTAPRECMAYOREOID,0) > 0 )
                ) THEN
        BEGIN
             LISTAPRECIOCLIENTE = '1';
        END
   ...
   BEGIN
        ...
        BEGIN
            ...
            PRECIOXLINEAPERSONA = :PRECIO1 * ((100.00 - COALESCE(:DESCXLINEAPERSONA,0.00)) / 100.00) ;
   ...
   BEGIN              
        ...
        IF(:PRECIO > :PRECIOXLISTACLIENTE AND ( COALESCE(:PAGOTARJMENORPRECIOLISTAALL,'N') = 'S') ) THEN
        BEGIN
            PRECIO = :PRECIOXLISTACLIENTE;
        END
   ...
   BEGIN
        ...
        PRECIOMAYLINEA = CASE WHEN :LISTAPRECIOMAYLINEA = 1 THEN :PRECIO1
                              WHEN :LISTAPRECIOMAYLINEA = 2 THEN :PRECIO2
                              WHEN :LISTAPRECIOMAYLINEA = 3 THEN :PRECIO3
                              WHEN :LISTAPRECIOMAYLINEA = 4 THEN :PRECIO4
                              WHEN :LISTAPRECIOMAYLINEA = 5 THEN :PRECIO5
                              WHEN :LISTAPRECIOMAYLINEA = 6 THEN :PRECIO6
                              ELSE :PRECIO4
                           END;
                           
        PRECIOMEDMAYLINEA = CASE WHEN :LISTAPRECMEDMAYLINEA = 1 THEN :PRECIO1
                              WHEN :LISTAPRECMEDMAYLINEA = 2 THEN :PRECIO2
                              WHEN :LISTAPRECMEDMAYLINEA = 3 THEN :PRECIO3
                              WHEN :LISTAPRECMEDMAYLINEA = 4 THEN :PRECIO4
                              WHEN :LISTAPRECMEDMAYLINEA = 5 THEN :PRECIO5
                              WHEN :LISTAPRECMEDMAYLINEA = 6 THEN :PRECIO6
                              ELSE :PRECIO3
                           END;


        SELECT PRECIOEXPORTACIONLINEA, TIPOMAYOREOLINEAIDAPLICADO, ERRORCODE FROM GET_MAYOREOLINEA_PRECIO(:PRODUCTOID ,:PERSONAID, :CANTIDAD ,:PRECIO1 ,:PRECIO,:PAGOCONTARJETA,:PRECIOMEDMAYLINEA,:PRECIOMAYLINEA, :TIPOMAYOREOID, :PRECIO6)
        INTO :PRECIO , :TIPOMAYOREOLINEAIDAPLICADO, :ERRORCODE;
   END
...
--INSERT INTO TRAZA(VALOR) VALUES ( 'CE 2 ' || CAST(:PRECIO AS VARCHAR(12)));
   ...
   END
    ...
    FROM GET_PROMO_PRECIO(:PRODUCTOID ,:PERSONAID, :CANTIDAD ,:PRECIO1 ,:PRECIO, :HABPROMOCIONBODEGAZO, :PAGOCONTARJETA)
    INTO :PRECIO, :ESPROMOCION, :PROMOCIONID ,:PROMOCIONDESGLOSE, :MONEDEROABONO, :ERRORCODE;

    If(coalesce(:ESPROMOCION,'N') = 'S') THEN
    BEGIN    
        --no se aplica entonces el mayoreo
        TIPOMAYOREOLINEAIDAPLICADO = 1;
    END
