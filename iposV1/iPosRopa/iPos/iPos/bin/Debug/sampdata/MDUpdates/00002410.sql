CREATE OR ALTER TRIGGER DOCTO_BI0_UTILIDAD FOR DOCTO
...
begin
  /* Trigger text */
   ...
   begin
      ...
      IF( new.vendedorutilidadid is null) then
      begin
        ...
        begin
            ...
            select sum( ( (case when coalesce(movto.estadorebaja,0) = 1 then coalesce(movto.precioconrebaja,0) else coalesce(movto.precio,0) end) - (CASE WHEN :tipoutilidadid = 1 THEN COALESCE(MOVTO.costoreposicion,0)
                                                           WHEN :tipoutilidadid = 3 THEN COALESCE(MOVTO.costoultimo,0)
                                                           ELSE COALESCE(MOVTO.costopromedio,0) END ) ) * COALESCE(MOVTO.cantidad, 0) )
