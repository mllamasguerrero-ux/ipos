create or alter procedure MOVTO_INSERT (
    ...
    REFERENCIAS varchar(255) character set NONE,
...
/* tipo cambio*/
  ...
  --Checar producto descontinuado en venta
   IF (:TIPODOCTOID in (21,25,321)) THEN
   BEGIN
   ...
   END
    ...
    select coalesce(SUBTIPODOCTOID,0), PAGOCONTARJETA, TIPOMAYOREOID from docto where id = :doctoid into :SUBTIPODOCTOID, :PAGOCONTARJETA, :TIPOMAYOREOID;
   ...
   BEGIN
      SELECT PRECIO1, TASAIVA, COSTOPROMEDIO, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION, SUM(COALESCE(PRODUCTOALMACEN.ENPROCESODESALIDA,0)) ENPROCESODESALIDA , TASAIEPS, TASAIMPUESTO
      FROM PRODUCTO
      left join PRODUCTOALMACEN ON PRODUCTO.ID = PRODUCTOALMACEN.PRODUCTOID AND PRODUCTOALMACEN.ALMACENID = :ALMACENID
      WHERE PRODUCTO.ID = :PRODUCTOID
      group by PRECIO1, TASAIVA, COSTOPROMEDIO, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION,  TASAIEPS, TASAIMPUESTO
   ...
   ELSE
   BEGIN
      SELECT PRECIO1, coalesce(TASAIVA,0), COSTOREPOSICION, PRECIOMAXIMOPUBLICO, COSTOPROMEDIO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION , SUM(COALESCE(PRODUCTOALMACEN.ENPROCESODESALIDA,0)) ENPROCESODESALIDA , TASAIEPS, TASAIMPUESTO
      FROM PRODUCTO   
      left join PRODUCTOALMACEN ON PRODUCTO.ID = PRODUCTOALMACEN.PRODUCTOID  AND PRODUCTOALMACEN.ALMACENID = :ALMACENID
      WHERE PRODUCTO.ID = :PRODUCTOID
      group by PRECIO1, TASAIVA, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, COSTOPROMEDIO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION  , TASAIEPS, TASAIMPUESTO
   ...
   BEGIN
        ...
        ELSE if (:TIPODOCTOID IN (21,25,23, 321, 323, 31, 331)) then
        BEGIN
   ...
   BEGIN
    ...
    select clave, ESKIT from producto where id = :productoid into :PRODUCTOCLAVE, :ESKIT;
      ...
      BEGIN
         ...
         IF ((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S') and (:TIPODOCTOID not in (321)))  THEN
         BEGIN
            ...
            BEGIN
               ...
               IF((:TIPODOCTOID IN (/*21,*/25,12,31) or (:TIPODOCTOID = 21 AND :SUBTIPODOCTOID <> 8 ) ) AND
         ...
         FROM GET_PRODUCTO_PRECIO_DOCTO(:PRODUCTOID, :PERSONAID, :CANTIDAD,
                 :TIPODOCTOID, :SUCURSALID, :SUCURSALTID, :COSTO, :PAGOCONTARJETA, :TIPOMAYOREOID)
         INTO :PRECIONUEVO, :ESPROMOCION, :PROMOCIONID, :PROMOCIONDESGLOSE, :MONEDEROABONO ,:ERRORCODE;

          insert into traza(valor) values('pct ' || coalesce(:PAGOCONTARJETA,'N'));
          insert into traza(valor) values('tm ' || CAST(coalesce(:TIPOMAYOREOID,1) AS VARCHAR(12)));
          insert into traza(valor) values('gppd ' || CAST(:precionuevo AS VARCHAR(12)));
         ...
         -- Para tipo 21=Venta
         IF (:TIPODOCTOID  in (21,25,22, 321) AND :SUBTIPODOCTOID NOT IN (13)/* y no es una nc x rebaja*/) THEN
         BEGIN
            ...
            BEGIN
                ...
                IF(:SUBTIPODOCTOID <> 15) THEN  /*Si no es una venta movil*/
                BEGIN
      ...
      IF(:CANTIDAD < 0 AND :TIPODOCTOID IN (11,12,21,22,25,16,17, 321)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
      ...
      IF(:TIPODOCTOID IN (11,12,13,14,15,16,17,18)) THEN
      BEGIN
   ...
   BEGIN
      ...
      -- Validar existencia bajo cero.
      IF (((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S')) AND (:TIPODOCTOID not in (50,81,53, 321))) THEN
      BEGIN
         -- Solo para documentos con tipo que sea SALIDA del inventario:
         -- Ventas, devoluciones, etc.
         IF (:TIPODOCTOID IN (SELECT ID FROM TIPODOCTO WHERE SENTIDOINVENTARIO = -1) ) THEN
      ...
      /* no puede haber cantidades negativas en compras , ventas, devoluciones de compras y devoluciones de ventas*/
      IF((:CANTIDAD + :CANTIDADACTUAL) < 0 AND :TIPODOCTOID IN (11,12,21,22,25,16,17, 321)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
