...
SELECT P.ID,
    ...
    || upper(COALESCE(TRIM(P.DESCRIPCION2), '')) producto,
     ...
     P.PRECIO4,
     P.PRECIO5,
     (P.precio2 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000))) AS PRECIO2_MAS_IMPUESTO ,
     (P.precio3 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000))) AS PRECIO3_MAS_IMPUESTO ,
     (P.precio4 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000))) AS PRECIO4_MAS_IMPUESTO ,
     (P.precio5 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.0000))) AS PRECIO5_MAS_IMPUESTO ,
    (
      (
