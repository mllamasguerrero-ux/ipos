CREATE OR ALTER PROCEDURE KIT_APLIDESENSAMBLE_REF (
    KITDOCTOID D_FK)
RETURNS (
...
BEGIN
   ...
   SELECT TIPODOCTOID, ESTATUSDOCTOID  , ALMACENID, SUCURSALID, CAJAID, ALMACENTID , SUBTIPODOCTOID
   FROM DOCTO
   WHERE ID = :KITDOCTOID
   INTO :TIPODOCTOID, :ESTATUSDOCTOID ,:ALMACENID, :SUCURSALID, :CAJAID, :ALMACENTID, :SUBTIPODOCTOID;
   ...
   IF (:TIPODOCTOID NOT IN (502)) THEN
   BEGIN
      ERRORCODE = 4003;
      SUSPEND;
      EXIT;
   ...
   FOR SELECT
      ...
      M.LOTE,
      M.FECHAVENCE,
      (COALESCE(MK.cantidad, 0) / COALESCE(MP.cantidad,0)) * COALESCE(M.CANTIDAD,0),
      0,
      0,
      M.COSTO,
   ...
   FROM MOVTO M
     LEFT JOIN DOCTO D
       ON D.ID = M.DOCTOID
          LEFT JOIN MOVTO MP
           ON MP.ID = M.MOVTOREFID
          LEFT JOIN MOVTO MK
           ON MK.DOCTOID = D.doctokitrefid AND MK.MOVTOrefid = M.MOVTOREFID
   WHERE M.DOCTOID = :KITDOCTOID  AND COALESCE(MP.CANTIDAD,0) > 0
      AND (M.CANTIDAD >0)
      ORDER BY M.partida
   INTO
   ...
   BEGIN
      ...
      SUCURSALTID = 0;
            ...
            FROM MOVTO_INSERT(:DOCTOIDPARTE, 0, :ALMACENTID, :SUCURSALID, :TIPODOCTOIDPARTE, 0, 0, :PERSONAID, :VENDEDORID, 1, 0,
               :PRODUCTOID, :LOTE, :FECHAVENCE, :CANTIDAD, :CANTIDAD, :CANTIDAD, 0, 0, 0, 0,
