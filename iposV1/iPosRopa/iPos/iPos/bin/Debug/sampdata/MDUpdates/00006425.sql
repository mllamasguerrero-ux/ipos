...
BEGIN
    ...
    SELECT ESKIT, TASAIVAID, TASAIVA, TASAIEPS, TASAIMPUESTO, KITIMPFIJO
        FROM PRODUCTO
        WHERE ID = :PRODUCTOKITID
        INTO :ESKIT, :TASAIVAID, :PORCENTAJEIVA, :PORCENTAJEIEPS, :PORCENTAJEIMPUESTO, :KITIMPFIJO;
                         ...
                         CAST(
                          COALESCE(KITDEFINICION.COSTOPARTE,0) * COALESCE(KITDEFINICION.CANTIDADPARTE,0)
                            + COALESCE(KITDEFINICION.COSTOPARTE,0) * COALESCE(KITDEFINICION.CANTIDADPARTE,0) * (CASE WHEN PARAMETRO.DESGLOSEIEPS = 'S' THEN COALESCE(PRODUCTO.TASAIEPS,0) / 100.00 ELSE
                             ...
                             +  cast(
                                    COALESCE(KITDEFINICION.COSTOPARTE,0) * COALESCE(KITDEFINICION.CANTIDADPARTE,0) *
                                    (CASE WHEN PARAMETRO.DESGLOSEIEPS = 'S' THEN 1 + (COALESCE(PRODUCTO.TASAIEPS,0) / 100.00)
                                        ELSE 1 END)
                                 as d_decimal_exact) * (COALESCE(PRODUCTO.TASAIVA,0) / 100.00)
