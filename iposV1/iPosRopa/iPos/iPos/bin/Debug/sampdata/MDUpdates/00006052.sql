...
BEGIN
   ...
   ERRORCODE = 0;
        ...
        SELECT P.KITIMPFIJO FROM MOVTO M
            INNER JOIN PRODUCTO P ON P.ID = M.PRODUCTOID
            WHERE M.ID = :MOVTOID
            INTO :KITIMPFIJO;

        IF(COALESCE(:eskit, 'N') = 'S' AND COALESCE(:KITIMPFIJO,'N') <> 'S') THEN
        BEGIN
            select COUNT(*) NUMREG , MAX(ERRORCODE), SUM(COALESCE(DETALLEKITIVA,0.0)) IVA, SUM(COALESCE(DETALLEKITIEPS,0.0)) ieps FROM KIT_OBTENREFERENCIA (:DOCTOID, :MOVTOID,'S')
