...
FROM            DOCTO INNER JOIN
                          ...
                          (
                            SELECT MOVTO.ID,
                                    ...
                                    cast(coalesce(movto.cantidad,0.00) * (case when movto.precio < coalesce(producto.preciotope,0) and coalesce(producto.preciotope,0) > 0 then  round(producto.preciotope,2) else movto.precio end * ((100.00 + MOVTO.TASAIVA) / 100.00) * ((100.00 + MOVTO.TASAIEPS) / 100.00))  as d_precio)   AS TOTALCONTOPE,
                                     cast(coalesce(movto.cantidad,0.00) * (case when movto.precio < coalesce(producto.preciotope,0) and coalesce(producto.preciotope,0) > 0 then  round(producto.preciotope,2) else movto.precio end * ((100.00 + MOVTO.TASAIVA) / 100.00) * ((100.00 + MOVTO.TASAIEPS) / 100.00))  as d_precio)   AS IMPORTECONTOPECONIMP
