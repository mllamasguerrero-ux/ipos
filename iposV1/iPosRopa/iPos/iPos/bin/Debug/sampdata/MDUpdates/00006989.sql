...
returns (
    ...
    NOTA1 D_OBSERVACION,
    NOTA2 D_DESCRIPCION,
    MOVILCONTADO D_BOOLCN_NULL,
    STATUSSALDO varchar(10))
as
...
BEGIN
   ...
   FOR SELECT        DOCTO.ID, DOCTO.FOLIO, DOCTO.SERIE, DOCTO.FOLIOSAT, DOCTO.SERIESAT, DOCTO.TOTAL, CLIENTE.NOMBRE AS NOMBRECLIENTE, VENDEDOR.NOMBRE AS NOMBREVENDEDOR, VENDEDOR.USERNAME, 
                         ...
                         DOCTO.OBSERVACION NOTA1, DOCTO.DESCRIPCION NOTA2, DOCTO.MOVILCONTADO,
                         CLIENTE_VIEW.STATUSSALDO

        FROM            DOCTO LEFT OUTER JOIN
                         PERSONA CLIENTE ON CLIENTE.ID = DOCTO.PERSONAID LEFT OUTER JOIN   
                         CLIENTE_VIEW ON CLIENTE_VIEW.ID = DOCTO.PERSONAID LEFT OUTER JOIN
        ...
        WHERE        (DOCTO.TIPODOCTOID = 331 or (DOCTO.TIPODOCTOID = 21 AND DOCTO.COTI_ENRUTADA = 'S')) AND
            (DOCTO.ESTATUSDOCTOID = 0) AND
            (
                (DOCTO.ESTADOSURTIDOID = :ESTADOSURTIDOID) OR
                (:ESTADOSURTIDOID = 4 AND coalesce(DOCTO.estadosurtidoid,0) = 0 AND
                  ((COALESCE(CLIENTE.bloqueado,'N') = 'S') OR
                   (COALESCE(CLIENTE.LIMITECREDITO,0) > 0 AND COALESCE(CLIENTE.LIMITECREDITO,0) < COALESCE(DOCTO.TOTAL,0) + (COALESCE(CLIENTE.saldo,0) * -1))
                   )
                )
            )  AND
