CREATE TABLE SAT_PAIS (
    ID                    D_PK NOT NULL /* D_PK = BIGINT NOT NULL */,
    ACTIVO                D_BOOLCS /* D_BOOLCS = CHAR(1) DEFAULT 'S' NOT NULL CHECK (VALUE IN ('S', 'N')) */,
    CREADO                D_TIMESTAMP /* D_TIMESTAMP = TIMESTAMP default current_timestamp NOT NULL */,
    CREADOPOR_USERID      D_FK DEFAULT 0 /* D_FK = BIGINT */,
    MODIFICADO            D_TIMESTAMP /* D_TIMESTAMP = TIMESTAMP default current_timestamp NOT NULL */,
    MODIFICADOPOR_USERID  D_FK DEFAULT 0 /* D_FK = BIGINT */,
    SAT_CLAVE             D_STDTEXT_SHORT /* D_STDTEXT_SHORT = VARCHAR(32) */,
    SAT_DESCRIPCION       D_STDTEXT_MEDIUM /* D_STDTEXT_MEDIUM = VARCHAR(128) */,
    SAT_FORMATOCP         D_STDTEXT_MEDIUM /* D_STDTEXT_MEDIUM = VARCHAR(128) */,
    SAT_FORMATORIT        D_STDTEXT_LARGE /* D_STDTEXT_LARGE = VARCHAR(255) */,
    SAT_VALIDACIONRIT     D_STDTEXT_SHORT /* D_STDTEXT_SHORT = VARCHAR(32) */,
    SAT_AGRUPACIONES      D_STDTEXT_SHORT /* D_STDTEXT_SHORT = VARCHAR(32) */
);




