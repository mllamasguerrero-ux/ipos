...
BEGIN
   ...
   if(not(coalesce(:TIPODOCTOIDENTRADA,0)  in (11,12,13,15,60,61,16,18,202,204,1011,1012,1013,1015) or coalesce(:TIPODOCTOIDSALIDA,0) in (11,12,13,15,60,61,16,18,202,204,1011,1012,1013,1015))) then
   begin
     exit;
   end
    ...
    --si es de compras de sucursales no debe afectar el saldo de la persona
    IF( :TIPODOCTOIDENTRADA  in (1011,1012,1013,1015) ) THEN
    BEGIN
        AFECTASALDOPERSONA = 'N';
    END
   ...
   begin
      /* if (:TIPODOCTOIDENTRADA IN (12) AND (NEW.formapagoid in (6,7) or NEW.tipopagoid = 1 ) ) THEN
       ...
       BEGIN
          ...
          where p.doctoid = new.doctoid and f.abona = 'S' into :abono,:abonotarjeta;
          ...
          UPDATE DOCTO SET ABONO = :ABONO , SALDO = CARGO - :ABONO, ABONOTARJETA = :ABONOTARJETA WHERE ID = NEW.DOCTOID;
       END
       /*END
       ...
       BEGIN
          ...
          coalesce(sum(p.importe *
          (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (2)) )   THEN 1
                ELSE 0 END)
          ),0) ABONOTARJETA
          from doctopago p inner join formapago f on p.formapagoid = f.id
          where p.doctosalidaid = new.doctosalidaid and f.abona = 'S' into :abono, :abonotarjeta;
          ...
          UPDATE DOCTO SET ABONO = :ABONO , SALDO = CARGO - :ABONO, ABONOTARJETA = :ABONOTARJETA WHERE ID = NEW.DOCTOSALIDAID;
       END
       --END
