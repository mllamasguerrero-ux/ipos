...
(CASE WHEN dp.formapagoid IN (7,19) then 'SALDO DESAPLICADO'
      WHEN dp.formapagoid IN (6,18) then 'SALDO APLICADO'
...
from movto m  
inner join doctopago dp on m.doctoid = dp.doctoid and dp.formapagoid in (1,2,3,5,6,7,14,15,16,18,19,20,21)  and dp.tipopagoid = 1
...
and ( dp.formapagoid in (1,2,3,5,14,20,21) or coalesce(d.id,0) <> coalesce(ds.doctorefid,-1))
...
null foliodevolucion
from movto m
inner join doctopago dp on m.doctoid = dp.doctoid and dp.formapagoid in (1,2,3,5,6,7,14,18,19,20,21)  and dp.tipopagoid = 1
...
from movto m  
inner join doctopago dp on m.doctoid = dp.doctosalidaid and dp.formapagoid in (1,2,3,5,6,14,18,20,21) and (dp.tipopagoid = 2 or (dp.tipopagoid = 1 and dp.formapagoid in (6,18)) )
inner join docto d on dp.doctoid = d.id
...
and dp.formapagoid in (1,2,3,5,14, 20, 21)
...
from movto m  
inner join doctopago dp on m.doctoid = dp.doctoid and dp.formapagoid in (1,2,3,5,7,14,19,20,21) and (dp.tipopagoid = 1 or (dp.tipopagoid = 2 and dp.formapagoid in (7,19)))
