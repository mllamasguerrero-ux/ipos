...
BEGIN
   ...
   CONTIENEMATERIALPELIGROSO = 'N';
    ...
    SELECT   
        DOCTO.PERSONAID, 'No' TRANSPINTERNAC, '' ENTRADASALIDAMERC , '' PAISORIGENDESTINO ,'' VIAENTRADASALIDA , cast( cast(PERSONA.distancia as numeric(18,2)) as d_stdtext_medium) TOTALDISTREC ,
        DOCTO.FOLIO
    FROM DOCTO
  ...
  SELECT PARAMETRO.SUCURSALID, PERSONA.ID
  FROM PARAMETRO
  ...
  SELECT
    ...
    FROM DOCTO
    LEFT JOIN PERSONA ON :PERSONAID = PERSONA.id
  ...
  SELECT
   ...
   SELECT
    ...
    CAST((COUNT(DISTINCT MOVTO.PRODUCTOID)) AS D_STDTEXT_MEDIUM ) NUMTOTALMERCANCIAS,
    '' CARGOPORTASACION
   FROM MOVTO
   ...
   FOR SELECT
        ...
        --case when coalesce(producto.espeligroso,'N') = 'S' then '1170' else '1170' end  CVEMATERIALPELIGROSO  ,
        --case when coalesce(producto.espeligroso,'N') = 'S' then sat_tipoembalaje.clave else '' end PRODUCTOEMBALAJE  ,

        
        case when coalesce(producto.espeligroso,'N') = 'S' then 'Si' else 'No' end MATERIALPELIGROSO,
        case when coalesce(producto.espeligroso,'N') = 'S' then '1170' else null end   CVEMATERIALPELIGROSO  ,
        case when coalesce(producto.espeligroso,'N') = 'S' then sat_tipoembalaje.clave else '' end PRODUCTOEMBALAJE  ,
