...
case when docto.estatusdoctoid = 2 then cast('Cancelacion' as varchar(20)) else cast('' as varchar(20)) end  as HEADERCANCELACION ,
    ...
    cast( CASE WHEN DOCTO.TIPODOCTOID = 21 THEN ( CASE WHEN DOCTO.PERSONAID <> 1 THEN 'CLIENTE' ELSE 'VENTA DE MOSTRADOR' END)
          WHEN DOCTO.tipodoctoid = 25 THEN 'CLIENTE APARTADO'
          WHEN DOCTO.TIPODOCTOID IN (11,12) THEN 'PROVEEDOR'
          ELSE ''
     ...
     s.nombres personanombres,
       ...
       case when docto.tipodoctoid in (11,41) then sumarizado.subtotal else docto.subtotal end subtotal,
       case when docto.tipodoctoid in (11,41) then sumarizado.iva else docto.iva end  iva,
       case when docto.tipodoctoid in (11,41) then coalesce(sumarizado.subtotal,0) + coalesce(sumarizado.iva,0) else docto.total end  total,
      ...
      s.entregacalle,
      s.entreganumeroexterior,
      s.entreganumerointerior,
      s.entregacolonia,
      s.entregamunicipio,
      s.entregaestado,
      s.entregacodigopostal
   ...
   FROM DOCTO  LEFT JOIN SUCURSAL ON SUCURSAL.ID = DOCTO.SUCURSALID
        ...
        LEFT JOIN SUCURSAL SUCURSALTRASLADO ON SUCURSALTRASLADO.ID = DOCTO.SUCURSALTID
         ...
         left join (select movto.doctoid, sum(coalesce(movto.subtotal,0)) subtotal, sum(coalesce(movto.iva,0)) iva, sum(coalesce(movto.total,0)) total, sum(coalesce(movto.cantidad,0)) numeropiezas, sum(coalesce(movto.descuentovale,0)) descuentovale from movto group by movto.doctoid) sumarizado on docto.id = sumarizado.doctoid
