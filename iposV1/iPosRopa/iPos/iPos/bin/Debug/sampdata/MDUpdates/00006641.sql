...
returns (
    ...
    PRODUCTODESCRIPCION varchar(800),
    CANTIDAD D_CANTIDAD,
    PRECIO D_PRECIO,
    IMPORTE D_IMPORTE,
    DESCUENTO D_IMPORTE,
...
BEGIN
  ...
  SELECT COALESCE(PARAMETRO.versioncfdi, '') FROM PARAMETRO INTO :VERSIONCFDI;
   ...
   FOR SELECT
      ...
      coalesce(c.texto2,''),
                ...
                case
                    ...
                    when  ( P.unidad = 'PZA' and ( coalesce(P.pzacaja,0) = 0 or  coalesce(P.pzacaja,0) = 1) ) then
                        '0 Cajas ' ||  cast(cast((CASE WHEN (D.tipodoctoid in  (41)) THEN CORE.cantidadsurtida ELSE CORE.cantidad END ) as integer) as varchar(20)) || ' piezas'
    ...
    CORE.KITCANTIDAD ,
        ...
        case when coalesce(PKITREF.pzacaja,0) = 0 or  coalesce(PKITREF.pzacaja,0) = 1  then
            '0 C ' || cast(cast(CORE.KITCANTIDAD as integer) as varchar(10)) || ' P'
        else
 ...
 END
 ELSE if (COALESCE(:TIPODOCTOID,1) = 701 AND (COALESCE(:VERSIONCFDI,'') <> '3.3' AND COALESCE(:VERSIONCFDI,'') <> '4.0')) then
 BEGIN
 ...
 END  
 ELSE if (COALESCE(:TIPODOCTOID,1) = 701 AND (COALESCE(:VERSIONCFDI,'') = '3.3' or COALESCE(:VERSIONCFDI,'') = '4.0')) then
 BEGIN
    ...
    FOR SELECT
      ...
      :DOCTOID,
      'FACTURACONSOLIDADA',
      'Venta ' || CAST(D.FOLIO AS VARCHAR(20)) ,
      1,
      D.SUBTOTAL,
      D.SUBTOTAL,
      0,
   ...
   LEFT JOIN TIPODOCTO  TD ON  TD.id = d.tipodoctoid
   LEFT JOIN parametro on 1 = 1
   WHERE D.factconsid = :DOCTOID AND D.tipodoctoid = 21 
     AND COALESCE(D.factconsaplicado,'S') = 'S'
   into
      :FECHA ,
      :FECHAHORA ,
      :TURNOCLAVE,
 ...
 END
 ELSE IF (COALESCE(:TIPODOCTOID,1) = 205    AND (COALESCE(:VERSIONCFDI,'') <> '3.3' AND COALESCE(:VERSIONCFDI,'') <> '4.0') ) THEN
 BEGIN
   ...
   :FECHA ,
      ...
      :SAT_DESCRIPCIONUNIDAD;
       ...
       SUSPEND;
 END
 ELSE IF (COALESCE(:TIPODOCTOID,1) = 205 AND (COALESCE(:VERSIONCFDI,'') = '3.3' or COALESCE(:VERSIONCFDI,'') = '4.0')) THEN
 BEGIN
