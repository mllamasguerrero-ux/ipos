...
case when docto.estatusdoctoid = 2 then cast('Cancelacion' as varchar(20)) else cast('' as varchar(20)) end  as HEADERCANCELACION ,
    ...
    cast( CASE WHEN DOCTO.TIPODOCTOID in (21,201) THEN ( CASE WHEN DOCTO.PERSONAID <> 1 THEN 'CLIENTE' ELSE 'VENTA DE MOSTRADOR' END)
     ...
     cast( CASE WHEN DOCTO.TIPODOCTOID in (11,12,21,22) AND P.manejaalmacen = 'S'  THEN  'DEL ALMACEN: ' ||  ALMACEN.NOMBRE
      ...
      case when coalesce(docto.sucursaltid,0) > 0 then suct.entregacalle
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.calle else s.entregacalle end ) end entregacalle,
      case when  coalesce(docto.sucursaltid,0) > 0 then suct.entreganumeroexterior
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.numeroexterior else s.entreganumeroexterior end ) end entreganumeroexterior,
      case when coalesce(docto.sucursaltid,0) > 0 then suct.entreganumerointerior
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.numerointerior else s.entreganumerointerior end ) end entreganumerointerior,
      case when coalesce(docto.sucursaltid,0) > 0 then suct.entregacolonia
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.colonia else s.entregacolonia end ) end entregacolonia,
      case when  coalesce(docto.sucursaltid,0) > 0 then suct.entregamunicipio
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.municipio else s.entregamunicipio end ) end entregamunicipio,
      case when  coalesce(docto.sucursaltid,0) > 0 then suct.entregaestado
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.estado else s.entregaestado end ) end entregaestado,
      case when  coalesce(docto.sucursaltid,0) > 0 then suct.entregacodigopostal
            else (case when coalesce(docto.dom_entregaid,0 ) > 0 then  dom.codigopostal else s.entregacodigopostal end ) end entregacodigopostal ,
   ...
   FROM DOCTO  LEFT JOIN SUCURSAL ON SUCURSAL.ID = DOCTO.SUCURSALID
        ...
        left join domicilio dom on dom.id = docto.dom_entregaid
