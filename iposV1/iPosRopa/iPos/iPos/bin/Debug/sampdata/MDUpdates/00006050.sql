...
declare variable ESSUBKIT D_BOOLCN;
declare variable EXTTOTALSUBKIT D_DECIMAL_EXACT;
declare variable PRORRATEO_ORG numeric(18,4);
...
BEGIN
   ...
   WHERE M.DOCTOID = :DOCTOIDPARALLENAR  AND COALESCE(PRODUCTO.ESKIT, 'N') = 'S'  AND (COALESCE(PRODUCTO.KITIMPFIJO,'N') <> 'S' OR COALESCE(:IGNORAKITIMFIJOS,'N') <> 'S')
      AND (M.CANTIDAD > 0)
    INTO :CUENTAKITS;
   ...
   BEGIN
    ...
    BEGIN
       ...
       IF (:TIPODOCTOID IN (21) AND COALESCE(:SUBTIPODOCTOID,0) = 26 ) THEN
       BEGIN
         SELECT DOCTO.doctoasustituirid FROM DOCTO WHERE ID = :DOCTOIDPARALLENAR INTO :DOCTOKITPARENTREFID;  
            IF(COALESCE(:MOVTOPARENTREFID,0) = 0 ) THEN
            BEGIN
                SELECT FIRST 1 MOVTO.ID FROM MOVTO WHERE MOVTO.DOCTOID = :DOCTOKITPARENTREFID AND MOVTO.PRODUCTOID = :PRODUCTOID AND MOVTO.PRECIO = :PRECIO INTO :MOVTOPARENTREFID;
            END
       END
     ...
     BEGIN
        ...
        INTO  :SUMAIEPS,  :SUMAIVA, :MAXIEPS, :MAXIVA;
         ...
         END
            ...
            FOR SELECT
                        ...
                        CASE WHEN COALESCE(PRODUCTO.ESKIT, 'N') = 'S'   AND (COALESCE(PRODUCTO.KITIMPFIJO,'N') <> 'S' OR COALESCE(:IGNORAKITIMFIJOS,'N') <> 'S') THEN 'S' ELSE 'N' END ESKIT
        ...
        BEGIN
            ...
            IF(COALESCE(:ESSUBKIT,'N') = 'S') THEN
            BEGIN

            
                PRORRATEO_ORG = :PRORRATEO;
                --TASAIEPSPARTE_ORG = :TASAIEPSPARTE;
                --TASAIVAPARTE_ORG = :TASAIVAPARTE;
                --TASACOSTOTOTALPARTE_ORG = :TASACOSTOTOTALPARTE;
               ...
               FOR SELECT
                    ...
                    BEGIN
                             TASAIVAPARTE = COALESCE(:DETALLEKITIVA,0.0) / COALESCE(:SUBTOTAL, 1.0);
                             TASAIEPSPARTE = COALESCE(:DETALLEKITIEPS,0) / COALESCE(:SUBTOTAL, 1.0);
                             TASACOSTOTOTALPARTE = COALESCE(:EXTKITSUBTOTAL,0) / COALESCE(:SUBTOTAL, 1.0);



                        PRORRATEO = COALESCE(:PRORRATEO, 0.0) * coalesce(:PRORRATEO_ORG ,0);
                        --TASAIEPSPARTE = COALESCE(:TASAIEPSPARTE, 0.0) * CAST(:TASAIEPSPARTE_ORG/ 100.00 AS D_DECIMAL_EXACT);
                        --TASAIVAPARTE = COALESCE(:TASAIVAPARTE, 0.0) * CAST(:TASAIVAPARTE_ORG/ 100.00 AS D_DECIMAL_EXACT);
                        --TASACOSTOTOTALPARTE = COALESCE(:TASACOSTOTOTALPARTE, 0.0) * CAST(:TASACOSTOTOTALPARTE_ORG/ 100.00 AS D_DECIMAL_EXACT);
