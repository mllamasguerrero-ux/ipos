create or alter procedure CONSOLIDADO_REPORTE (
    FACTCONSID D_FK)
returns (
    ID D_FK,
    FOLIO D_DOCTOFOLIO,
    ...
    FACTCONSAPLICADO D_BOOLCS_NULL)
as
...
BEGIN
   ...
   SELECT FECHAINI, FECHAFIN FROM DOCTO WHERE ID = :FACTCONSID INTO :FECHAINICIAL, :FECHAFINAL;
   ...
   FROM            DOCTO LEFT OUTER JOIN
    ...
    WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
         ...
         AND (DOCTO.FACTCONSID = :FACTCONSID)
        ...
        WHERE
           (DOCTO.FECHA >= :FECHAINICIAL) AND
            ...
            (DOCTO.FACTCONSID = :FACTCONSID) AND
            ...
            AND DOCTOPAGO.factconsid = :FACTCONSID
        ...
        UNION
         ...
         SELECT
            ...
            DOCTODEV.FOLIO,
            DOCTODEV.SERIE,
            DOCTODEV.IMPORTE , --SUM(COALESCE(DOCTOPAGO.importe,0) * (CASE WHEN DOCTOPAGO.TIPOPAGOID = 1 THEN 1 WHEN DOCTOPAGO.TIPOPAGOID = 2 THEN -1 ELSE 0 END)) IMPORTE ,
            DOCTODEV.DESCUENTO , --0 DESCUENTO,
            DOCTODEV.SUBTOTAL , --SUM(COALESCE(DOCTOPAGO.importe,0) * (CASE WHEN DOCTOPAGO.TIPOPAGOID = 1 THEN 1 WHEN DOCTOPAGO.TIPOPAGOID = 2 THEN -1 ELSE 0 END)) SUBTOTAL ,
            DOCTODEV.IVA ,--0.0 IVA ,
            DOCTODEV.IEPS ,--0.0 IEPS ,
            DOCTODEV.IMPUESTO ,--0.0 IMPUESTO ,
            DOCTODEV.TOTAL ,--SUM(COALESCE(DOCTOPAGO.importe,0) * (CASE WHEN DOCTOPAGO.TIPOPAGOID = 1 THEN 1 WHEN DOCTOPAGO.TIPOPAGOID = 2 THEN -1 ELSE 0 END)) AS TOTAL,
            0.0 AS SALDO,
            0.0 ABONO,
            DOCTODEV.CARGO , --SUM(COALESCE(DOCTOPAGO.importe,0) * (CASE WHEN DOCTOPAGO.TIPOPAGOID = 1 THEN 1 WHEN DOCTOPAGO.TIPOPAGOID = 2 THEN -1 ELSE 0 END)) CARGO,
            TIPODOCTO.NOMBRE AS TIPODOCTONOMBRE,
            DOCTODEV.FECHA,
            DOCTODEV.FECHAHORA ,-- COALESCE(MIN(DOCTOPAGO.FECHAHORA),CURRENT_TIMESTAMP) FECHAHORA ,
            DOCTODEV.TIPODOCTOID,
            3 AS TIPOAPLICACION   ,
            0 DOCTOPAGOID  ,
        ...
        WHERE
           --(DOCTODEV.FECHA >= :FECHAINICIAL) AND
            ...
            DOCTODEV.FACTCONSID = :FACTCONSID AND
            --(DOCTODEV.FACTCONSID IS NULL) AND
            --(DOCTODEV.DEVCONSID IS NULL) AND
            --((:OMITIRVENTASAFRANQUICIAS = 'N' OR DOCTODEV.SUBTIPODOCTOID IS NULL OR  DOCTODEV.SUBTIPODOCTOID NOT IN (7, 8))) AND
            DOCTODEV.ESTATUSDOCTOID = 1-- AND DOCTODEV.tipodoctoid = 22
