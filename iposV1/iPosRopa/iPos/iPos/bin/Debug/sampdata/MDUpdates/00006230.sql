...
BEGIN
   SELECT
    ...
    COALESCE(COSTOTOTALENTRADAS, 0.0000),
   COALESCE(CANTTOTALENTRADAS, 0.0000)
   FROM PRODUCTO
   WHERE ID = :PRODUCTOID
   INTO :EXISTENCIA, :COSTOTOTALPREVIO, :CANTIDADPREVIA;

       
    -- SI ESTABA EL INVENTARIO EN 0 Y ES COMPRA, REINICIA LOS ACUMULADOS
     IF( COALESCE(:TIPODOCTOID,0) = 11 AND  COALESCE(:EXISTENCIA,0) = COALESCE(:CANTIDADENTRADA,0) ) THEN
     BEGIN
