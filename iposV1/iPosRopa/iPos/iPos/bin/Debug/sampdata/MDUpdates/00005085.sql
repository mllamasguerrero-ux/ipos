CREATE OR ALTER VIEW CORTE_BASE_DOCTO_Y_PAGO(
    CORTEID,
    TIPODOCTOID,
    TIPOABONOID,
    MISMOCORTE,
...
select
    ...
    sum(base.intercambiomercancia) intercambiomercancia   ,
    sum(case when base.formapagoid = 20 then base.importe else 0 end) deposito,
    sum(case when base.formapagoid = 21 then base.importe else 0 end) depositotercero,

    base.doctoid, --aditional
    base.pagoid    --aditional
    ...
    (
            ...
            case when  p.formapagoid in (18,19) then
                    case when  p.formapagoid in (18)  then
                        case when p.tipoabonoid = 3 then p.importe * -1 else p.importe end
                    else
                        case when p.tipoabonoid = 3 then p.importe  else p.importe * -1 end
                    end
            else 0.00
            end intercambiomercancia ,
            case when p.tipopagoid = 2 then
                ds.id
            else
                de.id
            end doctoid , --aditional
            p.id pagoid  --aditional
            ...
            where p.formapagoid in (1,2,3,4,5,14,15,16, 18,19,20,21)
    ...
    (
        ...
        from
            ...
            left join docto de on p.doctoid = de.id
            left join docto ds on p.doctosalidaid = ds.id
            where p.formapagoid in (1,2,3,5,14,15,16,20,21) and p.tipoabonoid in ( 2,3)
        ...
        group by base.corteid,base.tipodoctoid, base.tipoabonoid, base.mismocorte ,abonos_mc.abonomismocorte , base.subtipodoctoid , base.abonootrocorte, base.ABONOREFERENCIAOTROCORTE
            , base.doctoid, base.pagoid     --aditional
;