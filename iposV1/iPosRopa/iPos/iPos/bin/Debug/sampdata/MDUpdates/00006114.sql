...
BEGIN
    ...
    --si se pago todo con vale , pon todo sin vale en 0
   IF( COALESCE(:DOCTOTOTAL,0.00) + COALESCE(:IMPORTEVALE, 0) <= 0) THEN
   BEGIN
          
        UPDATE MOVTO
        SET SINVALE_TOTAL = 0.00 ,
            SINVALE_IVA  =  0.00 ,
            SINVALE_IEPS  =  0.00 ,
            SINVALE_SUBTOTAL  =  0.00,
            SINVALE_IVA_RET =  0.00,
            SINVALE_ISR_RET =  0.00
          WHERE DOCTOID = :DOCTOID;
   END
   ELSE
   BEGIN
