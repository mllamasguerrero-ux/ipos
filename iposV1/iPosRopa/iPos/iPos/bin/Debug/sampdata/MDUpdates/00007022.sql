...
declare variable MAX_ESTADOREBAJA D_FK;
BEGIN
 ...
 FOR SELECT
   ...
   DELETE FROM MOVTO WHERE DOCTOID = :DOCTOID AND COALESCE(:CANTIDAD,0) = 0;
      ...
      SELECT COUNT(*) ,max(coalesce(ESTADOREBAJA,0)) MAX_ESTADOREBAJA
      FROM MOVTO
      WHERE DOCTOID = :DOCTOID AND COALESCE(ESTADOREBAJA,0) in ( 1,3)
      INTO :CUENTAMOVTOPEND, :MAX_ESTADOREBAJA;

      IF(COALESCE(:CUENTAMOVTOPEND,0) = 0) THEN
      BEGIN
            UPDATE DOCTO SET ESTADOREBAJA = 2
            WHERE ID = :DOCTOID  AND COALESCE(ESTADOREBAJA,0) IN (1,3);
      END
      ELSE
      BEGIN
            UPDATE DOCTO SET ESTADOREBAJA = coalesce(:MAX_ESTADOREBAJA,1)
             WHERE ID = :DOCTOID AND COALESCE(ESTADOREBAJA,0) NOT IN (1,3);
      END
   ...
   UPDATE DOCTO SET  PERSONAIDSURTIDO = :USUARIOID, PERSONAIDAPROBACIONCXC = :USUARIOID, ESTADOSURTIDOID = 1 WHERE ID = :DOCTOID;
