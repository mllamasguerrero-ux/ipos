...
select P.ID,
    ...
    (
      ...
      CASE WHEN COALESCE(P.eskit, 'N') = 'S' AND COALESCE(P.TASAIEPSEXT,0) > 0  AND COALESCE(P.TASAIVAEXT,0) > 0    THEN
            round((  CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilidadlistado/100.00))) AS NUMERIC(18,4))  * cast( (1.0000 + ( (CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPSEXT ELSE 0.0000 END) /100.00)) * (1.00 + (P.TASAIVAEXT/100.00)) as numeric(18,4))),2)
      else
            round((  CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilidadlistado/100.00))) AS NUMERIC(18,4))  * cast( (1.0000 + ( (CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0.0000 END) /100.00)) * (1.00 + (P.TASAIVA/100.00)) as numeric(18,4))),2)
       END
      AS COSTOREPOCONUTILIMP ,

      round(CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilidadlistado/100.00))) AS NUMERIC(18,4)),2)
      AS COSTOREPOCONUTIL        ,

      CASE WHEN COALESCE(P.eskit, 'N') = 'S' AND COALESCE(P.TASAIEPSEXT,0) > 0  AND COALESCE(P.TASAIVAEXT,0) > 0    THEN
            round((  CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilmacrolistado/100.00))) AS NUMERIC(18,4))  * cast( (1.0000 + ( (CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPSEXT ELSE 0.0000 END) /100.00)) * (1.00 + (P.TASAIVAEXT/100.00)) as numeric(18,4))),2)
      else
            round((  CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilmacrolistado/100.00))) AS NUMERIC(18,4))  * cast( (1.0000 + ( (CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0.0000 END) /100.00)) * (1.00 + (P.TASAIVA/100.00)) as numeric(18,4))),2)
       END
      AS COSTOREPOCONUTILMACROIMP ,

      round(CAST((coalesce(P.costoreposicion,0) * (1 + (PM.porcutilmacrolistado/100.00))) AS NUMERIC(18,4)),2)
