   ...
   BEGIN
    ...
    FOR  SELECT
        ...
        BEGIN
            ...
            SET
            PRECIO    = 
                                      /*PRECIO MAS BAJO POR VALE*/case when ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) < MOVTO.PRECIO  THEN   ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) ELSE MOVTO.PRECIO END
                                    ,
            ...
            DESCUENTOVALE =  ROUND(  ((:DESCUENTOVALE)/100) *  ((ROUND( 
                                      /*PRECIO MAS BAJO POR VALE*/case when ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) < MOVTO.PRECIO  THEN   ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) ELSE MOVTO.PRECIO END
                                     * MOVTO.CANTIDAD , 2))     +       ((ROUND( 
                                      /*PRECIO MAS BAJO POR VALE*/case when ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) < MOVTO.PRECIO  THEN   ROUND((MOVTO.PRECIOLISTA * ((100 - :DESCUENTOVALE)/100.00)),2) ELSE MOVTO.PRECIO END
                                     * MOVTO.CANTIDAD , 2)) * (COALESCE (MOVTO.tasaiva,0)/100)))  )
