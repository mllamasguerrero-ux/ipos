create or alter procedure KIT_ENSAMBLARALVUELOPORCAMBIO (
...
declare variable APLICARENSAMBLENORMAL D_BOOLCS;
declare variable DOCTOIDFORKITNORMAL D_FK;
declare variable EXISTENCIAPORALMACEN D_CANTIDAD;
...
BEGIN
   ...
   APLICARENSAMBLENORMAL = 'S';

   SELECT TIPODOCTOID, ESTATUSDOCTOID  , ALMACENID, SUCURSALID, CAJAID, PERSONAID  ,VENDEDORID, ORIGENFISCALID , TIPODOCTO.sentidoinventario , SUBTIPODOCTOID
   FROM DOCTO LEFT JOIN TIPODOCTO ON DOCTO.TIPODOCTOID = TIPODOCTO.ID
   WHERE DOCTO.ID = :DOCTOIDPARALLENAR
   INTO :TIPODOCTOID, :ESTATUSDOCTOID ,:ALMACENID, :SUCURSALID, :CAJAID, :PERSONAID, :VENDEDORID, :ORIGENFISCALID, :SENTIDOINVENTARIO, :SUBTIPODOCTOID;
   ...
   IF (COALESCE(:SENTIDOINVENTARIO ,0) <> -1  AND :TIPODOCTOID NOT IN (501,502,503,504)) THEN--(:TIPODOCTOID NOT IN (12,13,24)) THEN
   BEGIN
      ERRORCODE = 1071;
      SUSPEND;
      EXIT;
   ...
   END
    ...
    IF(COALESCE(:SUBTIPODOCTOID,0) = 28) THEN
    BEGIN
      ERRORCODE = 0;
      SUSPEND;
      EXIT;
   ...
   INTO
      ...
      :DOCTOREFID , :MOVTOPARENTREFID, :LOTEIMPORTADO
   DO
   BEGIN
            ...
            --INSERT INTO TRAZA(VALOR) VALUES('KEC1-' || cast(coalesce(:DOCTOREFID,0) as varchar(10)));

            IF(COALESCE(:DOCTOKITPARENTREFID,0) > 0 AND COALESCE(:MOVTOPARENTREFID,0) > 0) THEN
            BEGIN

                --INSERT INTO TRAZA(VALOR) VALUES('KEC2-' || cast(coalesce(:DOCTOKITPARENTREFID,0) as varchar(10)) || '-' || cast(coalesce(:MOVTOPARENTREFID,0) as varchar(10)));
                ...
                (select MK.PRODUCTOID ,SUM(COALESCE(MK.CANTIDAD, 0) / COALESCE(MP.CANTIDAD, 1)) cantidadmk FROM MOVTO MK
                    ...
                    LEFT JOIN
                    (select K.PRODUCTOPARTEID , sum(COALESCE(K.cantidadparte,0)) cantidadkit  FROM  KITSUNNIVEL_VIEW K where k.productokitid = :PRODUCTOID GROUP by K.PRODUCTOPARTEID) KIT
               ...
               BEGIN
                    --INSERT INTO TRAZA(VALOR) VALUES('KEC3-' || cast(coalesce(:DOCTOKITPARENTREFID,0) as varchar(10)) || '-' || cast(coalesce(:MOVTOPARENTREFID,0) as varchar(10)));
                  ...
                  --INSERT INTO TRAZA(VALOR) VALUES('KEC4-' || cast(coalesce(:CUENTADIFERENCIA, 0) as varchar(10)));
               ...
               BEGIN
                     ...
                     BEGIN
                        ...
                        BEGIN       
                                  exception KIT_GENERALERROR 'Error ensamble de kits cambio (doctoinsert) ' || cast(:ERRORCODE as varchar(10));
                                  SUSPEND;
                                  EXIT;
                        END

                        UPDATE DOCTO SET SUBTIPODOCTOID = 12, DOCTOKITREFID = :DOCTOKITPARENTREFID WHERE ID = :DOCTOIDFORKIT;
                     ...
                     SELECT DOCTOID, MOVTOID, ERRORCODE
                        ...
                        :REFERENCIA, '', :COSTO, :SUCURSALTID, :ALMACENID, 'N', NULL, CURRENT_DATE, CURRENT_DATE, 0.00,:DOCTOIDPARALLENAR,NULL,NULL,NULL,NULL, :DESCRIPCION1, :DESCRIPCION2, :DESCRIPCION3, NULL, :LOTEIMPORTADO, 'N','N')
                        INTO :DOCTOIDFORKIT, :MOVTOID, :ERRORCODE;

                    IF ((:ERRORCODE IS NOT NULL) AND (:ERRORCODE > 0)) THEN
                    BEGIN       
                        exception KIT_GENERALERROR 'Error ensamble de kits cambio (movtoinsert) ' || cast(:ERRORCODE as varchar(10));
                        ERRORCODE = :ERRORCODE ;
                        SUSPEND;
                        EXIT;
                    END
            ...
            IF(:APLICARENSAMBLENORMAL = 'S') THEN
            BEGIN


               SELECT SUM(COALESCE(I.CANTIDAD,0)) CANTIDADPARAKIT
               FROM INVENTARIO I
               WHERE  I.PRODUCTOID = :PRODUCTOID  AND I.ALMACENID = :ALMACENID
               INTO :EXISTENCIAPORALMACEN;
                  IF(COALESCE(:EXISTENCIAPORALMACEN ,0 ) <  COALESCE(:CANTIDAD,0) ) THEN
                  BEGIN

                    IF(COALESCE(:DOCTOIDFORKITNORMAL,0) = 0) THEN
                     BEGIN
                        ...
                        IF ((:ERRORCODE IS NOT NULL) AND (:ERRORCODE > 0)) THEN
                        BEGIN           
                                   exception KIT_GENERALERROR 'Error ensamble de kits cambio (doctoinsert) 2 ' || cast(:ERRORCODE as varchar(10));
                                  SUSPEND;
                                  EXIT;
                        END
                    END
                     ...
                     SELECT DOCTOID, MOVTOID, ERRORCODE
                        FROM MOVTO_INSERT(:DOCTOIDFORKITNORMAL, 0, :ALMACENID, :SUCURSALID, :TIPODOCTOIDFORKIT, 0, 0, :PERSONAID, :VENDEDORID, 1, 0,
                        :PRODUCTOID, :LOTE, :FECHAVENCE, COALESCE(:CANTIDAD,0) - COALESCE(:EXISTENCIAPORALMACEN ,0 ), 0, 0, 0, 0, 0, 0,
                        :REFERENCIA, '', :COSTO, :SUCURSALTID, :ALMACENID, 'N', NULL, CURRENT_DATE, CURRENT_DATE, 0.00,:DOCTOIDPARALLENAR,NULL,NULL,NULL,NULL, :DESCRIPCION1, :DESCRIPCION2, :DESCRIPCION3, NULL, :LOTEIMPORTADO, 'N','N')
                        INTO :DOCTOIDFORKITNORMAL, :MOVTOID, :ERRORCODE;

                    IF ((:ERRORCODE IS NOT NULL) AND (:ERRORCODE > 0)) THEN
                    BEGIN          
                        exception KIT_GENERALERROR 'Error ensamble de kits cambio (movtoinsert) 2' || cast(:ERRORCODE as varchar(10));
                        ERRORCODE = :ERRORCODE ;
                        SUSPEND;
                        EXIT;
                    END
   ...
   BEGIN
        SELECT ERRORCODE
        FROM KIT_APLIENSAMBLE_REF(:DOCTOIDFORKIT)
        INTO :ERRORCODE;
   END

   IF ((:ERRORCODE IS NOT NULL) AND (:ERRORCODE > 0)) THEN
   BEGIN    
      exception KIT_GENERALERROR 'Error ensamble de kits cambio (kitensabmble) ' || cast(:ERRORCODE as varchar(10));
      SUSPEND;
      EXIT;
   END
   ...
   BEGIN
        SELECT ERRORCODE
        FROM KIT_APLICAR(:DOCTOIDFORKITNORMAL)
        INTO :ERRORCODE;
   END

   IF ((:ERRORCODE IS NOT NULL) AND (:ERRORCODE > 0)) THEN
   BEGIN           
      exception KIT_GENERALERROR 'Error ensamble de kits cambio (kitaplicar) ' || cast(:ERRORCODE as varchar(10));
      SUSPEND;
      EXIT;
   END
