...
BEGIN
  ...
  FOR SELECT TASA
   ...
   WHERE (TIPODOCTOID = :TIPODOCTOID  OR (:TIPODOCTOID = -21 AND TIPODOCTOID IN (21,31,23,81))
          OR (:TIPODOCTOID = -31 AND TIPODOCTOID IN (31,81))  OR (:TIPODOCTOID = -11 AND d.TIPODOCTOID IN (11, 41, 12)) )
   ...
   BEGIN
     ...
     BEGIN
        ...
        SELECT SUM(CASE WHEN COALESCE(ESKIT,'N') = 'N' THEN COALESCE(SUBTOTAL,0) ELSE 0 END)
        FROM MOVTO
        WHERE (DOCTOID = :DOCTOID or DOCTOID = :DOCTOREFKITID)
        AND TASAIVA = 0
        INTO :SUBTOTALIVA0;
        ...
        FROM MOVTO
        WHERE (DOCTOID = :DOCTOID or DOCTOID = :DOCTOREFKITID)
        AND TASAIVA NOT IN (0,16)
        INTO :SUBTOTALOTROIVA;
        ...
        select SUM(COALESCE(MOVTO.CANTIDAD,0) * COALESCE(MOVTO.costopromedio,0)) SUMACOSTOPROMEDIO
        FROM movto
        WHERE DOCTOID = :doctoid
        INTO :SUMACOSTOPROMEDIO;
