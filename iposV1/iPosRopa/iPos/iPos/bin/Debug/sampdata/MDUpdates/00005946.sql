...
declare variable DOCTOIDKARDEX D_FK;
declare variable DOCTOLOTEIMPORTADOID D_FK;
declare variable TIPODOCTOIDKARDEX D_FK;
...
BEGIN
  ...
  into :TIPODOCTOID, :FECHA, :FECHAHORA, :ALMACENID, :ESFACTURAELECTRONICA, :DOCTOORIGENFISCAL, :SUBTIPODOCTOID;
    ...
    IF(:TIPODOCTOID in ( 81, 82, 83) AND :SUBTIPODOCTOID = 7) THEN
    BEGIN 
        ERRORCODE = 0;
        SUSPEND;
        EXIT;
    ...
    --LOTE IMPORTACION MANEJO ESPECIAL DE ALGUNOS TIPOS DE DOCTO
    SELECT manejarloteimportacion FROM PARAMETRO  INTO :MANEJARLOTEIMPORTACION; 
    SELECT TIPODISTLOTEIMPORTADOID FROM tipodocto WHERE ID = :tipodoctoid INTO :TIPODISTLOTEIMPORTADOID;
    SELECT MANEJALOTEIMPORTADO FROM PRODUCTO WHERE ID = :PRODUCTOID INTO :MANEJALOTEIMPORTADO;
    IF(COALESCE(:MANEJARLOTEIMPORTACION,'N') = 'S' AND COALESCE(:MANEJALOTEIMPORTADO,'N') = 'S') THEN
    BEGIN
            IF(COALESCE(:TIPODISTLOTEIMPORTADOID,0) NOT IN (0)) THEN
            BEGIN   
                ERRORCODE = 0;
                SUSPEND;
                EXIT;
  ...
  END
   ...
   BEGIN
     ...
     --SELECT COALESCE(SUM( CANTIDAD),0) FROM INVENTARIO WHERE PRODUCTOID = :PRODUCTOID AND ORIGENFISCALID = 3 INTO :EXISTENCIADEFACTURA;
      SELECT ULTIMOORIGENFISCALVENTA FROM PRODUCTO WHERE ID = :PRODUCTOID INTO :ULTIMOORIGENFISCAL;
       ...
       -- PARCHE VERIFICAR CUANDO POR ALGUNA RAZON HAY CANTIDADES NEGATIVAS EN UN ORIGEN FISCAL Y EN EL OTRO NO
      IF(COALESCE(:EXISTENCIADEFACTURA,0) < 0 AND COALESCE(:existenciaderemision, 0) > 0 ) THEN
      BEGIN
        EXISTENCIADEREMISION = :EXISTENCIADEREMISION + :existenciadefactura;
        EXISTENCIADEFACTURA = 0;
      END 
      ELSE IF(COALESCE(:EXISTENCIADEFACTURA,0) > 0 AND COALESCE(:existenciaderemision, 0) < 0 ) THEN
      BEGIN
        EXISTENCIADEFACTURA = :EXISTENCIADEFACTURA + :EXISTENCIADEREMISION;
   ...
   END
    ...
    DOCTOIDKARDEX = :DOCTOID;
    DOCTOLOTEIMPORTADOID = NULL;
    TIPODOCTOIDKARDEX = :TIPODOCTOID;

    IF(COALESCE(:MANEJARLOTEIMPORTACION,'N') = 'S' AND :TIPODOCTOID IN (901,902)) THEN
    BEGIN
             SELECT DOCTO.loteimportadodoctoid, DORIG.tipodoctoid
              FROM DOCTO LEFT JOIN DOCTO DORIG ON DORIG.ID = DOCTO.LOTEIMPORTADODOCTOID
               WHERE DOCTO.ID = :DOCTOID INTO :DOCTOIDKARDEX, :TIPODOCTOIDKARDEX;
             DOCTOLOTEIMPORTADOID = :DOCTOID;
    END
   ...
   BEGIN
      ...
      INSERT INTO KARDEX
         (TIPODOCTOID, DOCTOID, MOVTOID, FECHA, FECHAHORA,
          ALMACENID, PRODUCTOID, LOTE, FECHAVENCE, CANTIDAD, PRECIO, COSTO, ORIGENFISCALID, ESAPARTADO, LOTEIMPORTADO,DOCTOLOTEIMPORTADOID)
      VALUES
         (:TIPODOCTOIDKARDEX, :DOCTOIDKARDEX, :MOVTOID, :FECHA, :FECHAHORA,
          :AlmacenId, :PRODUCTOID, :LOTE, :FECHAVENCE, :CANTIDADDEFACTURA, :PRECIO, :COSTO,3, :ESAPARTADO, :LOTEIMPORTADO, :DOCTOLOTEIMPORTADOID);
   ...
   BEGIN
      ...
      VALUES
         (:TIPODOCTOIDKARDEX, :DOCTOIDKARDEX, :MOVTOID, :FECHA, :FECHAHORA,
          :AlmacenId, :PRODUCTOID, :LOTE, :FECHAVENCE, :CANTIDADDEREMISION, :PRECIO, :COSTO,2, :ESAPARTADO, :LOTEIMPORTADO, :DOCTOLOTEIMPORTADOID);
   ...
   BEGIN
      ...
      VALUES
         (:TIPODOCTOIDKARDEX, :DOCTOIDKARDEX, :MOVTOID, :FECHA, :FECHAHORA,
          :AlmacenId, :PRODUCTOID, :LOTE, :FECHAVENCE, :CANTIDADDEINDEFINIDO, :PRECIO, :COSTO,1,:ESAPARTADO, :LOTEIMPORTADO, :DOCTOLOTEIMPORTADOID);
