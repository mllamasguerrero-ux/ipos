...
BEGIN
    ...
    WHERE DOCTO.FECHA >= :FECHAINICIAL AND DOCTO.FECHA <= :FECHAFINAL
        ...
        UNION
         ...
         SELECT
            ...
            CASE WHEN (COALESCE(:DOCTOMAXIMOID, 0) = 0 or COALESCE(DOCTODEV.doctorefid, 0) <= COALESCE(:DOCTOMAXIMOID, 0) ) THEN 'S' ELSE 'N' END APLICADO
   ...
   BEGIN
      ...
      IF(COALESCE(:TIPODOCTOID,0) = 21 AND COALESCE(:OMITIRVALES,'N') = 'S' and COALESCE(:VERSIONCFDI,'') = '3.3') THEN
      BEGIN     
            SELECT SUM(COALESCE(DOCTOPAGO.IMPORTE,0)) FROM DOCTOPAGO WHERE DOCTOID = :ID
            AND DOCTOPAGO.TIPOPAGOID = 1  AND DOCTOPAGO.formapagoid = 5 INTO :DESCUENTOCONVALE;

            DESCUENTOCONVALE = COALESCE(:DESCUENTOCONVALE,0);
