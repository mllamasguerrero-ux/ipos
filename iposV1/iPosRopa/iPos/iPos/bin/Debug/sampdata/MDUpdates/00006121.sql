create or alter procedure POLIZALINEAV2_DET_PARTE3A (
   ...
   CUADRE = :CUADREINICIAL;
    ...
    LEYENDA = :LEYENDAINICIAL;
        ...
        begin
             ...
             HDR_CONCEPTO1 = 'IEPS VENTA,CRED/CONT: CRED,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ'  ;
             HDR_CONCEPTO2 = '';
             NUMLINEA = :NUMERO + 1; DTL_ORDEN = 0;
              ...
              BUFFERMONTO1 = 0;
                --suma todos los ieps    
              for
                select (coalesce(movto.ieps ,0.00) * case when docto.tipodoctoid = 21 then 1.00 else -1.00 end)  subtotal     
               ...
               do
               begin      
                    DTL_NIV1_CONCEPTO =  'IEPS VENTA,CRED/CONT: CRED,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
              ...
              for      
                select (coalesce(movtodev.ieps ,0.00) )   subtotal       
               ...
               do
               begin      
                    DTL_NIV1_CONCEPTO =  'IEPS DEVOLUCION MISMO MES, FECHA:DEVOLUCION, CRED/CONT: CONT,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
                    ...
                    DTL_NIV2_CONCEPTO =  'IEPS VENTA MISMO MES, FECHA:DEVOLUCION, CRED/CONT: CONT,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
              ...
              for
               ...
               do
               begin    
                    DTL_NIV1_CONCEPTO = 'IEPS DEVOLUCION DIF MES, FECHA:VENTA, CRED/CONT: CRED,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
                    DTL_NIV1_NOMBREID1 = 'DEVOLUCION ID';
                    ...
                    DTL_NIV2_CONCEPTO =  'IEPS VENTA DIF MES, FECHA:VENTA, CRED/CONT: CRED,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
                    DTL_NIV2_NOMBREID1 = 'VENTA ID';
                    DTL_NIV2_NOMBREFECHA1 = 'FECHA VENTA';
                    DTL_NIV2_NOMBREMONTO1 = 'TOTAL';
                    DTL_NIV2_MONTO1  = :AUXMONTOSUMARIZADO1;
              ...
              for
                select (coalesce(movto.ieps ,0.00) )  subtotal    
                           ,docto.id , docto.fecha ,  docto.folio, docto.referencia  
                           ,coalesce(docto.esfacturaelectronica,'N') || ' - ' || coalesce(docto.timbradouuid,'') esfactura
                           ,docto.hatenidocredito
                           ,coalesce(plazo.clave, '') plazo
               ...
               do
               begin    
                    DTL_NIV1_CONCEPTO = 'IEPS TRASLADO, FECHA:TRSLADO, CRED/CONT: TODOS,ESTATUS:COMP,FACT/REM:' || CASE WHEN COALESCE(:ESFACT,'N') = 'S' THEN 'FACT' ELSE 'REM' END || ', PZO: NO TEQ';
                    DTL_NIV1_NOMBREID1 = 'TRASLADO ID';
                    DTL_NIV1_NOMBREFECHA1 = 'TRASLADO VENTA';
                    DTL_NIV1_NOMBREMONTO1 = 'IEPS';
                    DTL_NIV1_MONTO1  = :AUXMONTOSUMARIZADO1 * -1;
                    DTL_NIV1_NOMBREID2 = 'FOLIO';
                    DTL_NIV1_NOMBREREFERENCIA1 = 'REFERENCIA';
