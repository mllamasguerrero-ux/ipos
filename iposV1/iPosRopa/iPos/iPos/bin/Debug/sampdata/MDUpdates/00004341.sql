  ...
  --Checar producto descontinuado en venta
   ...
   BEGIN
        ...
        BEGIN       
               ...
               END
        END 
        ELSE IF (:TIPODOCTOID in ( 121)) THEN    -- OTRAS SALIDAS
        BEGIN       
               IF(:PRECIO IS NOT NULL) THEN
               BEGIN
                    SELECT FIRST 1 ID FROM MOVTO
   ...
   BEGIN
    ...
    select clave, ESKIT from producto where id = :productoid into :PRODUCTOCLAVE, :ESKIT;
      ...
      BEGIN
         ...
         IF ((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S') and (:TIPODOCTOID not in (321,821)))  THEN
         BEGIN
   ...
   BEGIN
      ...
      -- Validar existencia bajo cero.
      IF (((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S')) AND (:TIPODOCTOID not in (50,81,53, 321, 821))) THEN
      BEGIN
         -- Solo para documentos con tipo que sea SALIDA del inventario:
         -- Ventas, devoluciones, etc.
         IF (:TIPODOCTOID IN (SELECT ID FROM TIPODOCTO WHERE SENTIDOINVENTARIO = -1)
