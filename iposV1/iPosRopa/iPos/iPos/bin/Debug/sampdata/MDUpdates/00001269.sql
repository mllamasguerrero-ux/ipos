...
BEGIN
   ...
   SELECT S.precioenviotraslado FROM DOCTO D INNER JOIN SUCURSAL S ON D.SUCURSALTID = S.id
   where d.id = :doctoid
   ...
   select case
   when :precioenviotraslado = 0 then coalesce(costoreposicion,0)
   when :precioenviotraslado = 1 then coalesce(precio1,0) 
   when :precioenviotraslado = 2 then coalesce(precio2,0)
   when :precioenviotraslado = 3 then coalesce(precio3,0)
   when :precioenviotraslado = 4 then coalesce(precio4,0)
   when :precioenviotraslado = 5 then coalesce(precio5,0)
