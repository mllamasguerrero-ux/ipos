...
SELECT P.ID,
    ...
    || upper(COALESCE(TRIM(P.DESCRIPCION2), '')) producto,
     ...
     round((P.precio1  * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.00)) * (1 + (P.TASAIVA/100.00))),2) AS PRECIO_MAS_IMPUESTO  ,
     P.tasaieps TASAIEPS,
     ...
     P.PRECIO4,
     P.PRECIO5,
     (P.precio2 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.00))) AS PRECIO2_MAS_IMPUESTO ,
     (P.precio3 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.00))) AS PRECIO3_MAS_IMPUESTO ,
     (P.precio4 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.00))) AS PRECIO4_MAS_IMPUESTO ,
     (P.precio5 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.00))) AS PRECIO5_MAS_IMPUESTO ,
     (
      (
       ...
       * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.0000)) * (1 + (P.TASAIVA/100.00))
       )
      ) *  (CASE WHEN COALESCE(P.PZACAJA,1) > 0 THEN COALESCE(P.PZACAJA,1) ELSE 1 END) PRECIOCAJA_MAS_IMPUESTO
