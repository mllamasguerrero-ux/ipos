...
BEGIN
   ...
   begin
       ...
       BEGIN
          ...
          (CASE WHEN ((TIPOPAGOID = 1 AND FORMAPAGOID IN (1,2,3,4,5,6,14,15,16,17,18)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (7,19)))  THEN 1
          ...
          begin
                select coalesce(sum(p.importe *
                (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (1,2,3,4,5,7,14,15,16,17,19)) or (TIPOPAGOID = 1 AND FORMAPAGOID IN (6,18)))  THEN 1
                    when ((TIPOPAGOID = 1 AND FORMAPAGOID IN (7,19)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (6,18)))  THEN -1
                    ELSE 0 END)
                ),0) from doctopago p inner join formapago f on p.formapagoid = f.id
