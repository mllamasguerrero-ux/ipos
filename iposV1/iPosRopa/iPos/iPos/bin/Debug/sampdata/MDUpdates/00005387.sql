create or alter procedure MOVTO_INSERT (
    ...
    LOTEIMPORTADO D_FK,
    CARGOTARJPRECIOMANUAL D_BOOLCN,
...
BEGIN
   ...
   SELECT COALESCE(retieneisr,'N'),COALESCE(retieneiva,'N'), COALESCE(EXENTOIVA,'N'), PAGOTARJMENORPRECIOLISTA from PERSONA where id = :personaid into :PERSONARETIENEISR, :PERSONARETIENEIVA, :PERSONAEXENTOIVA, :PAGOTARJMENORPRECIOLISTA;
  ...
  --Checar producto descontinuado en venta
   ...
   BEGIN
      SELECT PRECIO1, TASAIVA, COSTOPROMEDIO, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION, SUM(COALESCE(PRODUCTOALMACEN.ENPROCESODESALIDA,0)) ENPROCESODESALIDA , TASAIEPS, TASAIMPUESTO, COSTOENDOLAR
      FROM PRODUCTO
      left join PRODUCTOALMACEN ON PRODUCTO.ID = PRODUCTOALMACEN.PRODUCTOID AND PRODUCTOALMACEN.ALMACENID = :ALMACENID
      WHERE PRODUCTO.ID = :PRODUCTOID
      group by PRECIO1, TASAIVA, COSTOPROMEDIO, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION,  TASAIEPS, TASAIMPUESTO, COSTOENDOLAR
      INTO :PRECIOLISTA, :TASAIVA, :COSTOPROMEDIO, :COSTOREPOSICION, :PRECIOMAXIMOPUBLICO, :NEGATIVOS, :INVENTARIABLE, :UNIDADPRODUCTO, :COMISIONXUNIDAD, :ENPROCESODESALIDA, :TASAIEPS, :TASAIMPUESTO,:COSTOENDOLAR;
   ...
   ELSE
   BEGIN
      SELECT PRECIO1, coalesce(TASAIVA,0), COSTOREPOSICION, PRECIOMAXIMOPUBLICO, COSTOPROMEDIO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION , SUM(COALESCE(PRODUCTOALMACEN.ENPROCESODESALIDA,0)) ENPROCESODESALIDA , TASAIEPS, TASAIMPUESTO , COSTOENDOLAR
      FROM PRODUCTO   
      left join PRODUCTOALMACEN ON PRODUCTO.ID = PRODUCTOALMACEN.PRODUCTOID  AND PRODUCTOALMACEN.ALMACENID = :ALMACENID
      WHERE PRODUCTO.ID = :PRODUCTOID
      group by PRECIO1, TASAIVA, COSTOREPOSICION, PRECIOMAXIMOPUBLICO, COSTOPROMEDIO, NEGATIVOS, INVENTARIABLE , UNIDAD, COMISION  , TASAIEPS, TASAIMPUESTO ,COSTOENDOLAR
      INTO :PRECIOLISTA, :TASAIVA, :COSTOREPOSICION, :PRECIOMAXIMOPUBLICO, :COSTOPROMEDIO, :NEGATIVOS, :INVENTARIABLE, :UNIDADPRODUCTO, :COMISIONXUNIDAD, :ENPROCESODESALIDA , :TASAIEPS, :TASAIMPUESTO, :COSTOENDOLAR;
   END
         ...
         BEGIN
            IF(COALESCE(:PAGOCONTARJETA,'N') = 'S') THEN
                BEGIN
                    ...
                    SELECT  PAGOTARJMENORPRECIOLISTA
                    FROM PERSONA
                    WHERE ID = :PERSONAID
                    INTO  :PAGOTARJMENORPRECIOLISTA;

                    --si el precio ya supera el precio de menudeo volver a poner el precio de menudeo
                    IF(:PRECIO > :PRECIOLISTA AND COALESCE(:PAGOTARJMENORPRECIOLISTA,'N') = 'S' ) THEN
                    BEGIN
                        PRECIO = :PRECIOLISTA;
                    END
                END
   ...
   IF(COALESCE(:AGRUPAPORPARAMETRO,'N') = 'S' AND :AGRUPACIONVENTAID IS NOT NULL AND COALESCE(:AGRUPACIONVENTAID,1) IN (2,3) ) THEN
   BEGIN
                  MOVTOACTUALID = NULL;
   END
   ...
   BEGIN
    ...
    select clave, ESKIT from producto where id = :productoid into :PRODUCTOCLAVE, :ESKIT;
      ...
      BEGIN
         ...
         IF ((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S') and (:TIPODOCTOID not in (321,821,901,902)))  THEN
         BEGIN
            ...
            IF (:TIPODOCTOID IN (SELECT ID FROM TIPODOCTO WHERE SENTIDOINVENTARIO = -1)
                  AND COALESCE(:SUBTIPODOCTOID,0) <> 22
            ...
            BEGIN
               ...
               BEGIN
                    ...
                    FROM GET_EXISTENCIAPARAKIT(:PRODUCTOID, :ALMACENID, :CANTIDAD - (COALESCE(:EXISTENCIAACTUAL,0) - COALESCE(:ENPROCESODESALIDA,0)))
                    INTO  :HAYEXISTENCIAPARAARMAR , :ERRORCODE;
                    
                    NUEVOSENPROCESOPARTES =  :CANTIDAD - (COALESCE(:EXISTENCIAACTUAL,0) - COALESCE(:ENPROCESODESALIDA,0));
               ...
               IF((:TIPODOCTOID IN (/*21,*/25,12,31, 121) or (:TIPODOCTOID = 21 AND :SUBTIPODOCTOID <> 8 ) ) AND
               ...
               END
            END
            IF(:TIPODOCTOID IN (501) AND :SUBTIPODOCTOID <> 10) THEN
            begin
         ...
         IF (:TIPODOCTOID in (11,50,51,52,53,54,55,41,16,17, 501,502,503,504, 32, 82)) THEN
         BEGIN
            PRECIOLISTA = :COSTO;
         ...
         IF (:TIPODOCTOID = 22 AND :SUBTIPODOCTOID <> 13/* y no es una nc x rebaja*/) THEN
         BEGIN
         ...
         IF (:TIPODOCTOID = 22 AND :SUBTIPODOCTOID = 13/* y  es una nc x rebaja*/) THEN
         BEGIN
               PRECIONUEVO = :PRECIO;
               PRECIOLISTA = :PRECIO;
               PRECIOMANUAL = :PRECIO;
         ...
         -- Para tipo 21=Venta
         IF (:TIPODOCTOID  in (21,25,22, 321,821) AND :SUBTIPODOCTOID NOT IN (13)/* y no es una nc x rebaja*/) THEN
         BEGIN
            ...
            BEGIN
                ...
                IF( COALESCE(:PRECIOYAVALIDADO,'N') = 'N' AND  :SUBTIPODOCTOID <> 15 AND :SUBTIPODOCTOID <> 22) THEN  /*Si no es una venta movil*/
                BEGIN
      ...
      IF(:CANTIDAD < 0 AND :TIPODOCTOID IN (11,12,21,22,25,16,17, 321, 821, 111, 121)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
      ...
      IF(:TIPODOCTOID IN (11,12,13,14,15,16,17,18, 111, 113)) THEN
      BEGIN
   ...
   BEGIN
      ...
      -- Validar existencia bajo cero.
      IF (((:NEGATIVOS = 'N') AND (:INVENTARIABLE = 'S')) AND (:TIPODOCTOID not in (50,81,53, 321, 821, 901,902))) THEN
      BEGIN
         -- Solo para documentos con tipo que sea SALIDA del inventario:
         -- Ventas, devoluciones, etc.
         IF (:TIPODOCTOID IN (SELECT ID FROM TIPODOCTO WHERE SENTIDOINVENTARIO = -1)
                  AND COALESCE(:SUBTIPODOCTOID,0) <> 22 ) THEN
         BEGIN
            ...
            IF (:EXISTENCIAACTUAL < (:CANTIDADACTUAL + :CANTIDAD)
                AND COALESCE(:HAYEXISTENCIAPARAARMAR,'N') <> 'S') THEN
            BEGIN
               ERRORCODE = 1081 ;
               SUSPEND;
      ...
      /* no puede haber cantidades negativas en compras , ventas, devoluciones de compras y devoluciones de ventas*/
      IF((:CANTIDAD + :CANTIDADACTUAL) < 0 AND :TIPODOCTOID IN (11,12,21,22,25,16,17, 321, 821, 111, 121)) THEN
      BEGIN 
            ERRORCODE = 3005;
            SUSPEND;
            EXIT;
      ...
      SET
         ...
         ENPROCESOPARTES = ENPROCESOPARTES + :NUEVOSENPROCESOPARTES
      WHERE ID = :MOVTOID
