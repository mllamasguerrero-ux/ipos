...
select P.ID,
    ...
    (
      ...
      ((100.00 + COALESCE(PM.COMISIONPORTARJETA,0.00))/100) * cast((P.precio3 * (1.00 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0.00 END)/100.00)) * (1.00 + (P.TASAIVA/100.00))) as d_precio)
       AS PRECIOMEDIOMAYOREOCONTARJETA,
      CASE WHEN COALESCE(P.PRECIO6,0) > 0 THEN PRECIO6
           ELSE ((100 + COALESCE(PM.COMISIONPORTARJETA,0.00))/100.0) * cast((P.precio4 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.00)) * (1 + (P.TASAIVA/100.00))) as d_precio)
      END PRECIOMAYOREOCONTARJETA
