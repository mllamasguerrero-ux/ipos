...
basico.doctoimporteendolar ,
 ...
 SUM(COALESCE(MKITREF.cantidad,(COALESCE(MOVTO.CANTIDAD,0) * COALESCE(KITDEFINICION.cantidadparte,0)))) KITCANTIDAD
...
(
    ...
    List(distinct (CASE WHEN COALESCE(mlp.LOTEIMPORTADO,0) > 0 THEN ' Ped.: ' || COALESCE(lotesimportados.PEDIMENTO,'') || ' fec.: ' || CAST( coalesce(lotesimportados.FECHAIMPORTACION, current_date) AS VARCHAR(20)) || ' Adu. ' ||  coalesce( lotesimportados.aduana, '') || ' Cant. '  || CAST(CASE WHEN (docto.tipodoctoid in  (41)) THEN coalesce(mlp.cantidadsurtida,0) ELSE coalesce(mlp.cantidad,0) END AS VARCHAR(20)) else '' end)) strpedimento ,
    ...
    left join movto mlp on mlp.doctoid = coalesce(docto.loteimportadodoctoid, docto.id) and mlp.productoid = movto.productoid  and coalesce(mlp.lote,'') = coalesce(movto.lote,'') and coalesce(mlp.fechavence, current_date) = coalesce(movto.fechavence, current_date) and mlp.precio = movto.precio
