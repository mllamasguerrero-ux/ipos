  ...
  /* Trigger text */
   ...
   BEGIN    
        SELECT TIPODOCTOID , SUBTIPODOCTOID FROM DOCTO WHERE ID = NEW.DOCTOID INTO :TIPODOCTOID, :SUBTIPODOCTOID;
         ...
         IF(:tipodoctoid IN (/*21,*/25,12,31, 121) or (:TIPODOCTOID = 21 AND coalesce(:SUBTIPODOCTOID,0) <> 8 )  or (:TIPODOCTOID = 501 AND coalesce(:SUBTIPODOCTOID,0) <> 10 )
           AND (coalesce(:SUBTIPODOCTOID,0) <> 28)) THEN
         BEGIN
