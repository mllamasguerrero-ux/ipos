...
select P.ID,
    ...
    (
      ...
      CASE WHEN COALESCE(P.PRECIO6,0) > 0 THEN ((100.0000 + COALESCE(PM.COMISIONPORTARJETA,0.00))/100.0000) * cast((P.precio6 * (1 + ((CASE WHEN COALESCE(PM.desgloseieps,'N') = 'S' THEN P.TASAIEPS ELSE 0 END)/100.00)) * (1 + (P.TASAIVA/100.00))) as d_precio)
