...
sum(trans.efectivo * cortecalculo.sentidoefectivo) efectivo,
...
sum(case when cortecalculo.sentidoefectivo = 1 then trans.efectivo else 0 end ) efectivo_ingreso,  
sum(case when cortecalculo.sentidoefectivo = -1 then trans.efectivo else 0 end ) efectivo_egreso,
...
sum(coalesce(trans.subtotalsiniva,0)) subtotalsiniva,
sum(coalesce(trans.subtotalconiva,0)) subtotalconiva,   
sum(coalesce(trans.montoiva,0)) montoiva
