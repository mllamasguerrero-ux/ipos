...
declare variable PROMOCIONMULTIDETALLEID D_FK;
BEGIN
   SELECT
      D.ESTATUSDOCTOID, M.ESTATUSMOVTOID, M.PROMOCION  , parametro.esvendedormovil , M.TIPODOCTOID,
      PARAMETRO.aplicarmayoreoporlinea, /*D.tipomayoreoid ,*/ D.ID , M.PRODUCTOID , M.PROMOCIONMULTIDETALLEID
   FROM MOVTO M
      LEFT JOIN DOCTO D
         ON D.ID = M.DOCTOID
   ...
   WHERE
      M.ID = :MOVTOID
   INTO
      :ESTATUSDOCTOID, :ESTATUSMOVTOID, :PROMOCION, :ESVENDEDORMOVIL, :TIPODOCTOID, :APLICARMAYOREOPORLINEA, /*:TIPOMAYOREOID,*/ :DOCTOID, :PRODUCTOID, :PROMOCIONMULTIDETALLEID;
   ...
   BEGIN
      ...
      IF (:TIPODOCTOID in (21) AND :ESTATUSDOCTOID = 0 AND COALESCE(:PROMOCIONMULTIDETALLEID,0) <> 0) THEN
      BEGIN
           SELECT  ERRORCODE
           FROM APLICAPROMOSMULTIDETALLE(:DOCTOID,:PRODUCTOID)
           INTO :ERRORCODE;
           IF (:ERRORCODE > 0) THEN
           BEGIN
               SUSPEND;
