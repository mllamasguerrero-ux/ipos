create or alter procedure REP_VENTA_CONIEPS_MARCADIA (
    FECHAINI D_FECHA,
    FECHAFIN D_FECHA,
...
BEGIN
  ...
  FOR SELECT
   ...
   WHERE (d.TIPODOCTOID = :TIPODOCTOID  OR (:TIPODOCTOID = -21 AND d.TIPODOCTOID IN (21,31,23,81))
     ...
     GROUP BY D.FECHA,D.ESFACTURAELECTRONICA  ,R.MARCAID,P.CLAVE ,P.NOMBRE
   INTO
      :FECHA ,
      :SUBTOTAL,
      :IVA ,
   ...
   BEGIN
        ...
        BEGIN     
            ...
            INNER JOIN PRODUCTO P ON P.ID = MOVTO.PRODUCTOID   
            LEFT JOIN LINEA ON LINEA.ID = P.LINEAID
            WHERE (D.TIPODOCTOID = :TIPODOCTOID  OR (:TIPODOCTOID = -21 AND D.TIPODOCTOID IN (21,31,23,81))
                OR (:TIPODOCTOID = -31 AND D.TIPODOCTOID IN (31,81)) )
                AND D.FECHA BETWEEN :FECHAINI AND :FECHAFIN
                AND D.ESTATUSDOCTOID = 1
                ...
                AND D.FECHA = :FECHA
                AND COALESCE(P.MARCAID,0) = COALESCE(:MARCAID,0)
