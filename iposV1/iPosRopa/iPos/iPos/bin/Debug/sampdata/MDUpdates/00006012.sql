...
BEGIN
    ...
    BEGIN
       ...
       IF( NEW.TIPODOCTOID in (11,15) ) THEN
       BEGIN
         UPDATE PERSONA SET SALDO = coalesce(SALDO,0) + NEW.total WHERE ID = NEW.PERSONAID;
       END


       IF( NEW.TIPODOCTOID IN (12,1012)) THEN
       BEGIN
         UPDATE PERSONA SET SALDO = coalesce(SALDO,0) - NEW.total WHERE ID = NEW.PERSONAID;
      ...
      END  */
       ...
       IF( NEW.TIPODOCTOID = 21 ) THEN
       BEGIN
         UPDATE PERSONA SET ULTIMAVENTA = NEW.fecha WHERE ID = NEW.PERSONAID;
       END
