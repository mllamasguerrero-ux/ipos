...
declare variable LINEAAPLICAMAYOREOPORLINEA D_BOOLCN;
BEGIN

    TIPOMAYOREOID = 1;

    SELECT LINEA.aplicamayoreoxlinea FROM PRODUCTO
    LEFT JOIN LINEA ON LINEA.ID = PRODUCTO.LINEAID WHERE PRODUCTO.ID = :PRODUCTOID INTO  :LINEAAPLICAMAYOREOPORLINEA;
    ...
    IF(COALESCE(:APLICARMAYOREOPORLINEA  ,'N') = 'N' OR COALESCE(:LINEAAPLICAMAYOREOPORLINEA  ,'N') = 'N') THEN
    BEGIN
         TIPOMAYOREOID = 1;
         ERRORCODE = 0;
         SUSPEND;
    ...
    WHERE MOVTO.DOCTOID = :DOCTOID AND COALESCE(LINEA.aplicamayoreoxlinea,'N') = 'S'  --AND PRODUCTO.LINEAID =  :LINEAID
    INTO  :TOTALCANTIDAD;
