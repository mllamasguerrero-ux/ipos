...
BEGIN
   SELECT CORTEID, CAJEROID, FECHACORTE, TURNOID
   FROM CAJA
   WHERE ID = :CAJAID
   INTO :CORTEID, :CAJEROID, :FECHACORTE, :TURNOID;

   SELECT ACTIVO, SALDOINICIAL, APORTACION, RETIRO
   FROM CORTE
   ...
   INTO :ACTIVO, :SALDOINICIAL, :APORTACION, :RETIRO;
   ...
   SELECT COALESCE(SUM(doctopago.importerecibido), 0.00)
   FROM DOCTO
