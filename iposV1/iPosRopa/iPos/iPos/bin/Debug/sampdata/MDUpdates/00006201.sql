...
BEGIN
   ...
   REFERENCIAS = NULL;
   SUCURSALTID = COALESCE(:SUCURSALTID,0);
   ALMACENTID = 0;
   ...
   -- Contar si hay registros para reportar.
        ...
        AND COALESCE(D.subtipodoctoid,0) NOT IN (22,24)

        INTO :PRODUCTOCUENTA;

   END
   ELSE IF(:SUBTIPODOCTOID = 2) THEN
   BEGIN
   ...
   END
        ...
        IF ( COALESCE(:FORZARPEDIDO,'N') = 'N' AND ((:PRODUCTOCUENTA IS NULL) OR (:PRODUCTOCUENTA <= 0))) THEN
        BEGIN
            ERRORCODE = 1058;
            SUSPEND;
            EXIT;
   ...
   BEGIN
        ...
        END,
          ...
          and ( coalesce(PRODUCTO.stock,0)
              - (coalesce(PRODUCTO.existencia,0) + (CASE :INCLUIRAPARTADO WHEN 'S' THEN coalesce(PRODUCTO.existenciaapartado,0) ELSE 0 END ))) > 0 ;
          
   END


   --delete from movto where doctoid = :DOCTOID AND productoid not in ( select id from producto);
    IF(:SUBTIPODOCTOID IN (1,2)) THEN
    BEGIN
      UPDATE DOCTO
      ...
      AND ((DOCTO.ESTATUSDOCTOPEDIDOID IS NULL) OR (DOCTO.ESTATUSDOCTOPEDIDOID = 0) OR (:FORZADO = 'S'))
        AND COALESCE(DOCTO.subtipodoctoid,0) NOT IN (22,24) ;
