...
BEGIN
   ...
   begin
       ...
       BEGIN
          ...
          (CASE WHEN ((TIPOPAGOID = 1 AND FORMAPAGOID IN (1,2,3,4,5,6,9,14)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (7/*,8*/)))  THEN 1
       ...
       BEGIN
          select coalesce(sum(p.importe *
           (CASE WHEN ((TIPOPAGOID = 2 AND FORMAPAGOID IN (1,2,3,4,5,7/*,8*/,14)) or (TIPOPAGOID = 1 AND FORMAPAGOID IN (6,9)))  THEN 1
                when ((TIPOPAGOID = 1 AND FORMAPAGOID IN (7/*,8*/)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (6,8/*9*/)))  THEN -1
                ELSE 0 END)
          ),0) from doctopago p inner join formapago f on p.formapagoid = f.id
