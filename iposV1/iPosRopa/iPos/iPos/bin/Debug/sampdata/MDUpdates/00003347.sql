...
BEGIN
   ...
   FOR SELECT
         ...
         coalesce((DOCTOPAGO.importe *
          (CASE WHEN ((DOCTOPAGO.TIPOPAGOID = 1 AND DOCTOPAGO.FORMAPAGOID IN (1,2,3,5,6,14,15,16)) or (TIPOPAGOID = 2 AND FORMAPAGOID IN (7)))  THEN 1
                when ((DOCTOPAGO.TIPOPAGOID = 1 AND DOCTOPAGO.FORMAPAGOID IN (7)) or (DOCTOPAGO.TIPOPAGOID = 2 AND FORMAPAGOID IN (6))) THEN -1
                ELSE 0 END)
          ),0) as ABONO  ,
        ...
        UNION
         ...
         SELECT
            ...
            coalesce((DOCTOPAGO.importe *
           (CASE WHEN ((DOCTOPAGO.TIPOPAGOID = 2 AND DOCTOPAGO.FORMAPAGOID IN (1,2,3,5,14,15,16)) )  THEN -1
                ELSE 0 END)),0) ABONO ,
          FORMAPAGO.nombre FORMADEPAGO,
          BANCOS.NOMBRE BANCONOMBRE,
          DOCTOPAGO.referenciabancaria,
        ...
        FROM    DOCTO  inner join
                DOCTOPAGO ON DOCTOPAGO.DOCTOSALIDAID = DOCTO.id AND DOCTOPAGO.formapagoid IN (1,2,3,5,15)
                LEFT JOIN TIPODOCTO ON TIPODOCTO.ID = DOCTO.TIPODOCTOID
                LEFT JOIN PERSONA ON PERSONA.ID = DOCTO.PERSONAID  
                LEFT JOIN FORMAPAGO ON FORMAPAGO.ID = DOCTOPAGO.FORMAPAGOID
                LEFT JOIN BANCOS ON BANCOS.ID = DOCTOPAGO.banco
