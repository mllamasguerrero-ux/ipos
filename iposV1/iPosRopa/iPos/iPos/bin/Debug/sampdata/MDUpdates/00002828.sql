create or alter procedure TTL_REP_PROD_COMPRA_COMPARACION (
    PRODUCTOID_P D_FK,
    ANIO1 D_DIAS,
    ANIO2 D_DIAS,
...
((COALESCE(D2.CANTIDAD ,0) -  COALESCE(D1.CANTIDAD ,0)) /  CASE WHEN COALESCE(D1.CANTIDAD,0)  <> 0 THEN D1.TOTAL ELSE COALESCE(D2.CANTIDAD,1) END) *100  PORCDIFERENCIA
...
select
        ...
        ANIO,
        MES,
        SUM(CASE WHEN TIPODOCTOID IN (11,41,15) THEN COALESCE(CANTIDAD,0) ELSE COALESCE(CANTIDAD,0) * -1 END) CANTIDAD,
        SUM(CASE WHEN TIPODOCTOID IN (11,41,15) THEN COALESCE(TOTAL,0) ELSE COALESCE(TOTAL,0) * -1 END) TOTAL,
        SUM(CASE WHEN TIPODOCTOID IN (11,41,15) THEN COALESCE(UTILIDAD,0) ELSE COALESCE(UTILIDAD,0) * -1 END) UTILIDAD
      from ttl_prod_tipo_mes
      where
          ANIO = :ANIO1   AND PRODUCTOID = :PRODUCTOID_P AND
          ( TIPODOCTOID IN (11,12,13,15) OR   (:INCLUIRTRASL = 'S' AND TIPODOCTOID IN (41,42,43) )    )
