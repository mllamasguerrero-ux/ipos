...
namespace iPos
{
    public partial class WFPagoServiciosEmida : IposForm
    {
        ...
        string m_strType = "AN";
        string m_amountType = "Variable";
        string m_nameReference = "Referencia";
        int m_length = 127;

        bool m_habServicioChangedEvent = true;

        public WFPagoServiciosEmida()
        {
            InitializeComponent();
        ...
        private CEMIDAREQUESTBE ObtenerDatosCapturados()
        {
            ...
            if(TBMonto.Text != "")
            {
                emidaRequest.IAMOUNT = ((decimal)TBMontoAplicar.NumericValue).ToString();//((decimal)TBMonto.NumericValue).ToString();
            }

            if(txtNum1.Text != "" && txtNum1.Text.Trim().Length == m_length && txtNum2.Text == txtNum1.Text)
            {
            ...
            emidaRequest.ISITEID = DBEmida.ObtenerTerminalServicio();

            CCLERKPAGOSERVICIOD clerkD = new CCLERKPAGOSERVICIOD();
            CCLERKPAGOSERVICIOBE clerkBE = new CCLERKPAGOSERVICIOBE();
            clerkBE.IID = CurrentUserID.CurrentUser.ICLERKSERVICIOS;
            ...
            int iConsecutivo = DBEmida.ObtenerConsecutivoTerminalServicio(ref strCometnario);
            if (iConsecutivo == -1)
            {
            ...
            try
            {
                emidaRequest.ICOMISION = decimal.Parse(TBComision.NumericValue.ToString());
            }
            catch
            {
        ...
        private bool AgregarBorradorVenta(CEMIDAREQUESTBE emidaReq, ref int auxMovtoId)
        {
            ...
            if (!eReq.EMIDA_GENERARVENTA((int)prodEmidaBE.IID, CurrentUserID.CurrentUser.IID, m_ClienteId, auxPrdouctId, decimal.Parse(emidaReq.IAMOUNT) + emidaReq.ICOMISION, emidaReq.IID, emidaReq.IINVOICENO, emidaReq.IACCOUNTID, "S",emidaReq.ICOMISION, ref auxDoctoId, ref auxMovtoId, null))
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (  m_strType == "NM"  && !ValidateTxtNum())
                {
                    MessageBox.Show("El campo de referencia solo puede contener números");
                    this.txtNum1.Focus();
                    return;
                }
                else if (txtNum1.Text == "" || txtNum1.Text.Trim().Length != m_length)
                {
                    MessageBox.Show("Porfavor ingrese una referencia de "  + m_length.ToString() + "  digitos , no vacio");
                    return;
                }
                else if (txtNum1.Text != txtNum2.Text)
                {
                    MessageBox.Show("Las referencias ingresadas no coinciden el uno al otro, ingrese un mismo número");
                    return;
                }
            ...
            {
                MessageBox.Show("No se pudo realizar la operación, verifique su conexión a internet o llame a Soporte 11 \n\n Exception: " + exc + exc.Message +  exc.StackTrace);
                CancelarVentaActual();
                this.Close();
        ...
        {
            string strCometnario = "";
            if (!DBEmida.RefrescarCarriersServicios(ref strCometnario) || !DBEmida.RefrescarEmidaProductsServicios(ref strCometnario) || !DBEmida.RefrescarEmidaComisionesServicios(ref strCometnario) )
            {
                MessageBox.Show(strCometnario);
            }
            else
            {
                ...
                parametroBE.IFECHAACTUALIZACIONEMIDASERV = DateTime.Today;
                if (!parametroD.CambiarFECHAACTUALIZACIONEMIDASERV(parametroBE, parametroBE, null))
                {
                ...
                else
                {
                    ...
                    CurrentUserID.CurrentParameters.IFECHAACTUALIZACIONEMIDASERV = parametroBE.IFECHAACTUALIZACIONEMIDASERV;
                    MessageBox.Show("Carriers y otra información de pago de servicio ha sido actualizada");
                }
        ...
        private void WFPagoServiciosEmida_Load(object sender, EventArgs e)
        {
            ...
            if (CurrentUserID.CurrentParameters.IFECHAACTUALIZACIONEMIDASERV == null || CurrentUserID.CurrentParameters.IFECHAACTUALIZACIONEMIDASERV.Date < DateTime.Today )
            {
                actualizaEmidaProductosYCarriers();
            }
            this.cARRIERTableAdapter.Fill(this.dSPuntoDeVentaAux2.CARRIER,"S");
            this.cbCantidadEmida.Enabled = true;
            try
            {
                if (CBCompaniaEmida.SelectedValue != null)
                {
                    this.eMIDAPRODUCTTableAdapter.Fill(this.dSPuntoDeVentaAux2.EMIDAPRODUCT, CBCompaniaEmida.SelectedValue.ToString(),"S");
                }
            }
            catch(Exception ex)
            {
        ...
        private void CBCompaniaEmida_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                m_habServicioChangedEvent = false;
                this.cbCantidadEmida.Enabled = true;
                this.eMIDAPRODUCTTableAdapter.Fill(this.dSPuntoDeVentaAux2.EMIDAPRODUCT, CBCompaniaEmida.SelectedValue.ToString(), "S");
                //TBMonto.Text = this.eMIDAPRODUCTTableAdapter.GetAmountProduct(cbCantidadEmida.SelectedValue.ToString());
                cbCantidadEmida.SelectedIndex = -1;
                ResetConfig();

                m_habServicioChangedEvent = true;
            }
            catch(Exception ex)
            {
                m_habServicioChangedEvent = true;
        ...
        {
            m_strType = "NM";
            m_amountType = "Variable";
            m_nameReference = "Referencia";
            m_length = 127;

            txtNum1.Text = "";
            txtNum2.Text = "";
            txtNum1.MaxLength = 127;
            txtNum1.MaxLength = 127;
            lblReferencia1.Text = m_nameReference;
            lblReferencia2.Text = m_nameReference;
            TBMontoAplicar.Text = "0";
            TBMontoAplicar.Enabled = true;
            TBComision.Text = "0";
            TBMonto.Text = "0";
        }
        ...
        private void cbCantidadEmida_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                CEMIDAPRODUCTD prodD = new  CEMIDAPRODUCTD();
                CEMIDAPRODEXTBE prodBE = new CEMIDAPRODEXTBE();
                prodBE.IPRODUCTID = cbCantidadEmida.SelectedValue.ToString();
                prodBE.IESSERVICIO = "S";

                prodBE = prodD.seleccionarEMIDAPRODUCTxEMIDAPRODUCTID(prodBE, null);

                if(prodBE != null)
                {
                    string strConfig = prodBE.IREFERENCE;

                    if(strConfig != null && strConfig.Length > 0)
                    {

                        //string[] strConfigArray = strConfig.Split(new string[] { "Type=", "Length=", "Amount Type=" }, StringSplitOptions.None);
                        string[] strConfigArray = strConfig.Split(new string[] { "Type=" }, StringSplitOptions.None);
                        if(strConfigArray.Length > 0)
                        {
                            string strBuffer = strConfigArray[1];
                            string[] strBufferArray = strBuffer.Split(new string[] { "Type=", "Length=", "Amount Type=", "Name=" }, StringSplitOptions.None);
                            string strVal = strBufferArray[0];
                            m_strType = strVal.Trim();
                        }

                        strConfigArray = strConfig.Split(new string[] { "Length=" }, StringSplitOptions.None);
                        if (strConfigArray.Length > 0)
                        {
                            string strBuffer = strConfigArray[1];
                            string[] strBufferArray = strBuffer.Split(new string[] { "Type=", "Length=", "Amount Type=", "Name=" }, StringSplitOptions.None);
                            ...
                            try{
                                m_length = int.Parse(strVal.Trim());
                            }
                            catch{
                                m_length = 127;
                            }
                        ...
                        strConfigArray = strConfig.Split(new string[] { "Name=" }, StringSplitOptions.None);
                        if (strConfigArray.Length > 0)
                        {
                            string strBuffer = strConfigArray[1];
                            string[] strBufferArray = strBuffer.Split(new string[] { "Type=", "Length=", "Amount Type=", "Name=" }, StringSplitOptions.None);
                            string strVal = strBufferArray[0];
                            m_nameReference = strVal.Trim();

                        }


                        strConfigArray = strConfig.Split(new string[] { "Amount Type=" }, StringSplitOptions.None);
                        if (strConfigArray.Length > 0)
                        {
                            string strBuffer = strConfigArray[1];
                            string[] strBufferArray = strBuffer.Split(new string[] { "Type=", "Length=", "Amount Type=", "Name=" }, StringSplitOptions.None);
                            string strVal = strBufferArray[0];
                            m_amountType = strVal.Trim();

                        }


                        if (m_amountType == "Variable")
                        {
                            TBMontoAplicar.Text = "0";
                            TBMontoAplicar.Enabled = true;
                        }
                        else
                        {
                            ...
                            TBMontoAplicar.Text = prodBE.IAMOUNT;
                            TBMontoAplicar.Enabled = false;
                        ...
                        txtNum1.MaxLength = m_length;
                        txtNum2.MaxLength = m_length;

                        if(m_strType.Equals("NM"))
                        {
                           lblDigits2.Text = lblDigits1.Text = m_length.ToString() + " digitos";
                        }
                        else
                        {
                            lblDigits2.Text = lblDigits1.Text = m_length.ToString() + " carácteres";
                        }


                        CEMIDACOMISIOND comisionD = new CEMIDACOMISIOND();
                        CEMIDACOMISIONBE comisionBE = new CEMIDACOMISIONBE();
                        comisionBE.IPRODUCTID = cbCantidadEmida.SelectedValue.ToString();
                        comisionBE = comisionD.seleccionarEMIDACOMISIONxEMIDAPRODUCTID(comisionBE, null);

                        if (comisionBE != null)
                        {
                            try
                            {
                                TBComision.NumericValue =  decimal.Parse( comisionBE.IPRODUCTUFEE.ToString());
                            }
                            catch
                            {
                                TBComision.Text = "0";
        ...
        private void calcularTotal()
        {
            ...
            try
            {
                dMontoAplicar = decimal.Parse(TBMontoAplicar.NumericValue.ToString());
            }
            catch
            {
            ...
            try
            {
                dComision = decimal.Parse(TBComision.NumericValue.ToString());
            }
            catch
            {
            ...
            dTotal = dMontoAplicar + dComision;

            TBMonto.NumericValue = dTotal;
        ...
        private void TBMontoAplicar_Validated(object sender, EventArgs e)
        {
            calcularTotal();
        }
        ...
        public WFPagoServiciosEmida( bool bFocus, bool bSeleccionarCompania)
        {
            InitializeComponent();
            m_dMontoRecarga = 0;
            m_bFocus = bFocus;
        ...
        public WFPagoServiciosEmida(bool bFocus, bool bSeleccionarCompania, decimal defaultValue) : this(bFocus, bSeleccionarCompania)
        {
