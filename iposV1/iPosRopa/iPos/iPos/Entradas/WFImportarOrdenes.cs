...
namespace iPos
{
    public partial class WFImportarOrdenes : IposForm
    {
        private long m_TipoDoctoID;



        public WFImportarOrdenes()
        {
            InitializeComponent();
            m_TipoDoctoID = 11;
        }

        public WFImportarOrdenes(long TipoDoctoID) : this()
        {
            m_TipoDoctoID = TipoDoctoID;
        }
        private void WFImportarOrdenes_Load(object sender, EventArgs e)
        {
        ...
        private void iMP_FILESDataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    WFImportarOrdenesDet fic;

                    fic = new WFImportarOrdenesDet((long)iMP_FILESDataGridView["GR_ID", e.RowIndex].Value, m_TipoDoctoID);
