...
{
    public partial class WFVehiculos : IposForm
    {
        ...
        public WFVehiculos(string palabrasBusqueda)
            : this()
        {
            m_bRetornarValor = true;
        }

        private void WFVehiculos_Load(object sender, EventArgs e)
        {
        ...
        private void MostrarRegistro(string opc)
        {
            try
            {
                ...
                long iID = long.Parse(TableDataGridView.Rows[iRetornoRowIndex].Cells["dgwID"].Value.ToString());
                WFVehiculoEdit fPi = new WFVehiculoEdit();
                fPi.ReCargar(opc, iID);
                fPi.ShowDialog();
                fPi.Dispose();
        ...
        private void LlenarGrid()
        {
            ...
            try
            {
                ...
                numeroRegistros = this.TableTableAdapter.Fill(this.dSCatalogos.VEHICULO_VIEW, strABuscar);
                if (this.dSCatalogos.VEHICULO_VIEW.Rows.Count > 0)
