...
{
    public partial class FRptDetVentas : Form
    {
        public int m_tipodoctoId;
        public bool m_ieps;

        public FRptDetVentas()
        {
            InitializeComponent();
            m_ieps = false;

        }
        public FRptDetVentas(bool ieps): this()
        {
            m_ieps = ieps;

        }

        private void FRptDetVentas_Load(object sender, EventArgs e)
        {
        ...
        private void LlenarDataSourceVentas()
        {
            ...
            if (!m_ieps || this.CBReporteTabular.Checked)
            {
                ...
                try
                {
                    ...
                    this.rEP_VENTA_DETTableAdapter.Fill(this.dSReportIpos.REP_VENTA_DET, DTPFrom.Value, DTPTo.Value, (long)m_tipodoctoId, (CBSoloAlmacen.Checked) ? int.Parse((comboBoxAlmacen.SelectedValue.ToString())) : 0);
            ...
            else
            {
                ...
                report1.SetParameterValue("DESENSAMBLADO", CBKitsDesensamblados.Checked ? "S" : "N");
        ...
        private void RecargarReport()
        {
            ...
            report1.Load(FastReportsConfigReport.getPathForFile("InformeDiarioVentasIEPSDETALLEEXP.frx", FastReportsConfigTipoFile.desistema));
