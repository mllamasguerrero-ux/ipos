...
{
    public class CMBANKDROIDD
    {
        private string iComentario;
        private string sCadenaConexion;
        private List<CBANKDROIDBE> bancos;
        ...
        public CMBANKDROIDD()
        {
            ...
            bancos = new List<CBANKDROIDBE>();
        }

        public CMBANKDROIDD(string conexion)
        {
        ...
        public bool ObtenerRegistros(FbTransaction st)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    CBANKDROIDBE banco = new CBANKDROIDBE();

                    if (dr["CLAVE"] != System.DBNull.Value)
                    {
                        banco.Clave = (string)(dr["CLAVE"]);
                    }

                    if (dr["NOMBRE"] != System.DBNull.Value)
                    {
                        banco.Nombre = (string)(dr["NOMBRE"]);
                    }
        ...
        public bool LimpiarTabla(FbTransaction st)
        {
            try
            {
                ...
                string commandText = @"DELETE FROM BANKDROID;";
        ...
        public bool InsertarBanco(CBANKDROIDBE banco, FbTransaction st)
        {
            try
            {
                sCadenaConexion = conexion;
                ...
                auxPar.Value = banco.Clave;
                parts.Add(auxPar);

                auxPar = new FbParameter("@NOMBRE", FbDbType.VarChar);
                auxPar.Value = banco.Nombre;
                parts.Add(auxPar);

                string commandText = @"INSERT INTO BANKDROID(
                                            CLAVE,
                                            NOMBRE
                                       )
                                       Values(
        ...
        public bool LlenarTabla()
        {
            ...
            try
            {
                ...
                foreach (CBANKDROIDBE banco in bancos)
                {
                    if (!InsertarBanco(banco, fbTransaction))
                        throw new Exception(iComentario);
                }
